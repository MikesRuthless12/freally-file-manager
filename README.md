...
- **Stays lean by design.** The repository is synchronous, `#![forbid(unsafe_code)]`, and adds **zero** new dependencies — it is the shared content + timeline layer the async history / sync engines record *into*, not a wrapper that drags their databases (or `copythat-audit`'s platform `unsafe`) into the chunk crate. The unified "Library" tab (Live / Snapshots / Versions) that replaces the separate History / Sync / Totals views is a scoped UI follow-up; the storage core ships first.

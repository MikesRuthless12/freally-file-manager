...
- **Deferred follow-ups (documented, not built):** the `copythat migrate <restic|borg|kopia>` / `copythat export` CLI, the sibling `cdr-py` PyPI parser, and upstream-adoption PRs need third-party repo binaries, a separately published package, and community engagement — tracked in the roadmap. This phase ships the spec + the format contract they all build on.

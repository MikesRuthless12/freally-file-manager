...
- **One content-addressed repository for copy, sync, and versioning.** `copythat_chunk::Repository` wraps the Phase 27 chunk store (now a sub-component) and adds a single **snapshot timeline**: every copy job, sync run, and version checkpoint is recorded as a `Snapshot` (kind `Copy` / `Sync` / `Version` / `Backup`) over a set of content-defined chunks, so every byte is stored once and every past state is queryable from one place. `snapshots()` lists the whole timeline; `snapshot_at(path, ts)` resolves the newest version of a file at or before a moment; `restore()` materialises it back from chunks.
...
- **Stays lean by design.** The repository is synchronous, `#![forbid(unsafe_code)]`, and adds **zero** new dependencies — it is the shared content + timeline layer the async history / sync engines record *into*, not a wrapper that drags their databases (or `copythat-audit`'s platform `unsafe`) into the chunk crate.

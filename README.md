...
- **Library tab.** A unified "Library" drawer (Live / Snapshots / Versions sub-views) ships alongside the existing History / Sync / Totals drawers, with a chunk-savings hero number — backed by `repository_stats` / `repository_snapshots` Tauri IPC over the in-`AppState` `Repository` handle, and 22 new `repo-*` / `library-*` Fluent keys across all 18 locales.
...
- **Cross-tool migration (`copythat migrate` / `export`).** A repository **detector** (restic / Borg / Kopia / CDR-0 from on-disk markers) + a **real restic importer** ship in `copythat_chunk::migrate` and the CLI. `copythat migrate restic <repo> <cdr> --password <pw>` decrypts a restic **v1/v2** repository (scrypt KDF → AES-256-CTR + Poly1305-AES → zstd, all via crates already in the workspace lockfile), reconstructs every file, and re-ingests it into a CDR-0 repository (re-chunked with FastCDC + BLAKE3 — restic chunk IDs aren't portable). It's validated by `tests/smoke/phase_50b_restic.rs` against a committed, PII-free restic fixture — byte-identical restore, no restic binary needed in CI. **Borg** and **Kopia** importers return a typed error naming exactly what each still needs (Kopia: binary-index build-out; Borg: a MessagePack dependency). The `cdr-py` PyPI parser and upstream-adoption PRs remain ecosystem follow-ups. See [`docs/spec/CDR-0.md`](docs/spec/CDR-0.md) §11.

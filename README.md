...
- **Parallel multi-chunk** copy for large files (default 4 chunks at matched memory budget). Opt-in via `FREALLY_PARALLEL_CHUNKS`; see [`docs/BENCHMARKS.md`](docs/BENCHMARKS.md).
...
- **Shell integration** — Windows IExplorerCommand, macOS Finder Sync, Linux `.desktop` / KDE ServiceMenu / Thunar UCA all route into the same queue via `freally --enqueue <verb> <paths…> [--destination <dst>]`.
- **Global paste hotkey** — press Ctrl+Shift+V (Windows / Linux) or Cmd+Shift+V (macOS) anywhere on the system to paste files copied from Explorer / Finder / Files through Freally File Manager's staging dialog. Combo is user-configurable; the shortcut can be toggled off entirely.
...
- **Read-through-snapshot for locked files** — when another process holds the source open for exclusive write, Freally File Manager can pull a read-only filesystem snapshot (VSS on Windows, ZFS / Btrfs on Linux, APFS local snapshot on macOS) and copy from there instead of surfacing the "file in use" error. Opt-in via Settings → Transfer → "When a file is locked". The Windows path spawns a sibling `freally-helper-vss.exe` via UAC so the main app never needs elevation of its own.
- **Crash / reboot resume** — every 50 ms the copy engine fsync's the destination and writes a checkpoint to a redb-backed journal at `<data-dir>/freally-journal.redb` (carrying the running BLAKE3 of consumed source bytes). On the next launch Freally File Manager detects unfinished jobs, BLAKE3-verifies the partial destination's prefix, and seeks both source and destination past the verified offset — power-cut at 96 % of a 2 TB transfer no longer means starting over. Toggle "Auto-resume interrupted jobs without prompting" in Settings → General to skip the resume modal.
...
- **Settings → Remotes tab** for add / remove / test-connection; secrets live in the OS keychain (Apple Keychain / Windows Credential Manager / Secret Service / kwallet) under `freally-cloud/<backend-name>`, never in `settings.toml`.
...
- **Resume journal awareness**: `JobRecord::remote_backend_name` distinguishes cloud-dst from local-dst at startup-resume time; cross-process MPU resume is intentionally not handled (Freally File Manager's remote-destination model is idempotent re-copy).

### Mount the archive (Phase 33)

- **Cross-platform read-only mount** of the Freally File Manager snapshot history: `by-date/YYYY-MM-DD/HH-MM-SS/<job>` + `by-source/<escaped-src-root>/<timestamp>` + `by-job-id/<row-id>/<job>`. The mount exposes historical copies as plain folders — `cat` / Explorer / Finder all work.
...
- **USER ACTION** to validate real kernel mounts: run `cargo test -p freally-mount --features fuse` on Linux/macOS, or `cargo test -p freally-mount --features winfsp` on Windows (requires the WinFsp driver + LLVM libclang installed — see [`docs/ROADMAP.md`](docs/ROADMAP.md) Phase 33g for the exact `winget` commands).
...
- **Five wire formats** ship simultaneously: **CSV** (eight-column stable header for spreadsheet forensics), **JSON-lines** (default, pipe-friendly), **Syslog RFC 5424** (structured-data block `[freally@32473 jobId="..." user="..."]`), **ArcSight CEF v0** (`CEF:0|Freally|Freally|<ver>|<sig>|<name>|<sev>|<ext>`), and **IBM QRadar LEEF 2.0** (tab-separated extension, `LEEF:2.0|Freally|Freally|<ver>|<eventID>|<ext>`).
- **Tamper-evident chain**: every record embeds `prev_hash = BLAKE3(previous_record_bytes)` as a hex column; one edit anywhere in the file cascades into every subsequent record's chain step. The Settings → *Verify chain* button + the Phase 36 CLI's `freally audit verify <log>` catch single-byte tampering.
...
- **Tracing fan-out**: `tracing_subscriber::Layer` captures any `target = "freally::audit"` log from the rest of the workspace into the sink so ad-hoc `warn!` calls auditors care about land in the same file.
...
- **Destination encryption via [age](https://age-encryption.org/)** — passphrase, X25519 (`age1…`), or SSH ed25519 / RSA recipients. Output is bit-for-bit compatible with the upstream `rage` CLI's binary format, so a future user can decrypt with the official `rage` binary without Freally File Manager in the loop.
...
- **Decryption is symmetric**: `freally_crypt::decrypted_reader` accepts a passphrase, X25519 secret key, or SSH private key — same `Identity` bag, same age-format input. The runner's auto-decrypt-on-copy-FROM-`.age` flow lands in a Phase 35 follow-up.
...
- **PWA over PeerJS WebRTC.** The phone-side app at [`apps/freally-mobile/`](apps/freally-mobile/) is a Vite + Svelte 5 Progressive Web App. Users open the deployed URL in their phone browser, scan the desktop's pairing QR with the system camera, and tap "Add to Home Screen" — the manifest reuses the desktop's icon so the home-screen launcher matches the desktop tray. **No App Store gate.**
...
### `freally` CLI (Phase 36)

- **`freally <SUBCOMMAND>`** — a real command-line interface suitable for CI/CD pipelines, automation scripts, and headless servers. Top-level commands: `copy`, `move`, `sync`, `shred`, `verify`, `history`, `stack`, `remote`, `mount`, `audit`, `plan`, `apply`, `schedule`, `version`, `config`, `provenance`, `serve` (Phase 48 — headless WebDAV/HTTP server), `migrate`, `export`, `repo`, `key`, `replicate`, and `completions`.
- **Repository commands run headless (Phases 49n / 50h).** `freally repo verify [--deep]` re-checks the repository and exits `4` on damage so cron can branch on "damaged" separately from "the command broke"; `repo repair` is dry-run by default so an unattended caller cannot delete snapshots by typo. `freally key add|remove|list|recovery` manages credential slots. `freally replicate <dst>` is the dedup-aware 3-2-1 push — only chunks the destination lacks transfer, and a re-run copies nothing. `freally mount --snapshot <id>` mounts a repository snapshot read-only. All of them resolve the repository the same way the GUI does, so a **portable install operates on its own repository beside the binary rather than the host's**.
...
- **Shell completions** for bash / zsh / fish / pwsh / elvish via `freally completions <SHELL>`. Redirect `stdout` to the shell's per-user completion location; the CLI itself never writes files.
...
- **`freally serve`** runs Freally headless as a **WebDAV / plain-HTTP / S3 / SFTP** file server over a directory: `freally serve --webdav --bind 127.0.0.1:8080 --root /data --token s3cr3t` (swap `--webdav` for `--http`, `--s3`, or `--sftp`). WebDAV / HTTP are backed by `dav-server`'s local filesystem, with `--readonly` (write methods → `403` before touching disk) and optional **bearer** or **HTTP Basic** auth (constant-time credential compare; `/metrics` stays open for scrapers). The startup banner prints `sftp://<addr>` for an SFTP server (SSH — no `/metrics`) and `http://<addr>` plus the metrics URL for every other protocol; a non-loopback bind with no auth prints a loud warning.
- **Prometheus `/metrics`** endpoint exposes `freally_jobs_total`, `freally_files_copied_total`, `freally_bytes_copied_total`, `freally_errors_total`, and a `freally_active_jobs` gauge in text-exposition format; writes increment the counters live.
- **SFTP transport** (`--sftp`, russh + russh-sftp) serves the same `--root` over SSH, path-jailed against the canonicalised root (rejects `..` / absolute / drive-prefix escapes, never follows symlinks), with `--readonly` enforcement, constant-time auth, and a 256 KiB per-read cap.
- **S3-compatible API** (`--s3`) — path-style `ListObjectsV2` (recursive, prefix-aware XML) plus GET / HEAD / PUT / DELETE on objects. **AWS SigV4** request verification: `--user` / `--password` map to access-key-id / secret-key and the `AWS4-HMAC-SHA256` signature is recomputed and constant-time-compared (bearer auth is rejected for S3). Object keys are percent-decoded then confined under the canonicalised `--root` (the same lexical jail as SFTP); PUT / DELETE are refused under `--readonly`.
- **OpenTelemetry export** (`--otel-endpoint http://localhost:4318/v1/traces`) ships traces over OTLP / HTTP-protobuf (no gRPC). Best-effort: a failed pipeline build is logged and the server still starts.
...
- **Settings → Server panel** brings the whole surface into the GUI: per-protocol toggles (WebDAV / HTTP / S3 / SFTP), bind address + served root, read-only, auth mode (none / bearer / basic) + credentials, OpenTelemetry endpoint, webhook destinations, and a Start / Stop control with live status and the `/metrics` URL — backed by `ServerSettings` persistence and the `server_start` / `server_stop` / `server_status` Tauri IPC commands. The panel **persists and edits** the webhook destinations and OTel endpoint, but a GUI-started server does not yet fire those webhooks or export those traces (its `serve()` consumes neither) — live webhook delivery / trace export remains the CLI's `--otel-endpoint` + `WebhookSink::deliver` path, the one remaining Phase 48 follow-up.

### Unified sync + backup repository (Phase 49 — moonshot)

- **One content-addressed repository for copy, sync, and versioning.** `freally_chunk::Repository` wraps the Phase 27 chunk store (now a sub-component) and adds a single **snapshot timeline**: every copy job, sync run, and version checkpoint is recorded as a `Snapshot` (kind `Copy` / `Sync` / `Version` / `Backup`) over a set of content-defined chunks, so every byte is stored once and every past state is queryable from one place. `snapshots()` lists the whole timeline; `snapshot_at(path, ts)` resolves the newest version of a file at or before a moment; `restore()` materialises it back from chunks.
- **Reference-counted garbage collection.** `gc()` is mark-and-sweep: it computes the reachable chunk set from **every manifest source** — the snapshots *and* the chunk store's own Phase 27 manifests, so a chunk a delta-resume / mount / recovery manifest still needs is never swept — then drops the index entries of unreachable chunks and deletes whole dead pack files, never touching data reachable from a surviving snapshot. GC takes an exclusive lease while recording snapshots share a concurrent one, so it can't run mid-record (which would otherwise sweep a not-yet-committed snapshot's chunks). Reference counts are derived from the catalog on demand rather than kept in a drift-prone side table. Large files stream through the rolling chunker (`snapshot_files`) so a multi-gigabyte backup is never buffered whole in RAM, and the timeline listing reads a lightweight summaries index without touching chunk manifests.
- **Stays lean by design.** The repository is synchronous, `#![forbid(unsafe_code)]`, and adds **zero** new dependencies — it is the shared content + timeline layer the async history / sync engines record *into*, not a wrapper that drags their databases (or `freally-audit`'s platform `unsafe`) into the chunk crate.
- **One shared repository handle.** The unified `Repository` and its underlying chunk store are opened **once** at startup and held in `AppState`, then shared by every consumer — the Library IPC, the recovery web UI, the mount backend, and the delta-resume sink — so the chunk store's single redb file lock is taken once instead of raced by per-call opens. Copy, sync, and version-on-overwrite recording into the timeline are each opt-in toggles on `ChunkStoreSettings` (default off, since recording re-reads the destination): a finished copy/move records a `Copy` snapshot, a sync round records a `Sync` snapshot of just the files it touched, and overwriting a file first captures the old bytes as a `Version` snapshot — all sharing chunks, so the dedup hero ratio reflects real savings.
- **Library tab — backup-manager workflows.** A unified "Library" drawer ships alongside the existing History / Sync / Totals drawers, with a chunk-savings hero number. Four sub-views: **Live** (stored vs effective bytes + dedup ratio, with a *Reclaim space* button that runs `gc()`), **Snapshots** (the copy / sync / version / backup timeline; each row offers *Restore…* and *Forget*), **Versions** (per-file rolling history), and **Sources** — designate folders for backup and *Back up now* to record an on-demand `Backup` snapshot honouring exclude globs (a repeat backup of an unchanged tree dedups to ~zero stored bytes). The **restore browser** lists a snapshot's files, picks a destination, and on existing-file collisions presents the engine's overwrite / skip / keep-both vocabulary before writing — behind a lexical traversal guard that can never escape the chosen root. Backed by `repository_*` / `sources_*` / `backup_now` / `snapshot_tree` / `restore_*` Tauri IPC and 29 new `backup-*` / `restore-*` / `library-*` Fluent keys across all 18 locales.

### Cross-tool repository portability — CDR-0 (Phase 50 — moonshot)

- **A public-domain interchange format.** [`docs/spec/CDR-0.md`](docs/spec/CDR-0.md) defines the **Common Dedup Repository, version 0** (CC0-1.0): on-disk layout, the FastCDC-2020 + BLAKE3-256 chunk algorithm, the framed pack-file format, the canonical-CBOR manifest + snapshot schemas, the encryption envelope (AES-256-GCM + scrypt), and the mandatory version-refusal rule. The goal: any compliant tool can read any compliant repo, so migrating in from restic / Borg / Kopia becomes a one-shot manifest translation that reuses existing chunk bytes instead of a multi-day re-ingest.
- **Rust reference implementation.** `freally_chunk::cdr` implements the manifest layer of CDR-0: `CdrManifest` ↔ internal `Manifest` conversion, deterministic CBOR encode/decode (via the already-in-tree `ciborium` + `serde_bytes` — no new third-party crate), a `validate()` that enforces every invariant (algorithm tag, 32-byte digests, contiguous chunk tiling, declared size), and `ensure_readable()` — the spec's hard rule that a reader **must refuse** a repository whose `spec_version` is newer than it understands.
- **Cross-tool migration (`freally migrate` / `export`).** A repository **detector** (restic / Borg / Kopia / CDR-0 from on-disk markers) + **real, validated importers for all three major deduplicating-backup tools** ship in `freally_chunk::migrate` and the CLI. `freally migrate <restic|borg|kopia> <repo> <cdr> --password <pw>` decrypts the source repository, reconstructs every file, and re-ingests it into a CDR-0 repository (re-chunked with FastCDC + BLAKE3, since source chunk IDs aren't portable): **restic** v1/v2 (scrypt → AES-256-CTR + Poly1305-AES → zstd), **Borg** repokey (PBKDF2 → AES-256-CTR + HMAC-SHA256, MessagePack + LZ4), and **Kopia** filesystem (scrypt format blob + v2 index + HMAC-keyed AES-256-GCM + object/directory resolution). Each is validated **byte-identical** by a smoke test (`tests/smoke/phase_50{b,c,d}_*.rs`) against a committed, PII-free fixture — no external binary needed in CI — and adds **no new crate** (every cipher + codec was already in the workspace lockfile). See [`docs/spec/CDR-0.md`](docs/spec/CDR-0.md) §11.

### End-to-end encrypted collaboration (Phase 51)

- **Per-file envelope crypto, no shared passphrase.** `freally-collab` keeps a `Roster` of the people who can read a shared item. `encrypt_for` mints a fresh data key per file and wraps it to every member's X25519 public key through `age` (via the existing `freally-crypt`), so each member decrypts with their own secret and no shared secret ever exists. Compression-before-encryption is disabled on both paths, so a CRIME-style oracle can't correlate plaintext with ciphertext length.
- **Removals are signed.** Removing someone mints a `RevocationToken` signed by the roster's ed25519 admin key, and `verify_revocations()` runs on every load, so a removal cannot be forged without that key. The roster is **trusted local state, not authenticated peer input**: verification uses the roster's own `admin_public` field, which proves internal consistency rather than origin, and the member list carries no signature. That is sufficient while the file only ever lives in the per-user config directory — anyone who can write there can already load a plugin with self-granted capabilities. A peer-import path would first need a signature over the roster body and `admin_public` pinned against the locally held key; the crate header says so where the code is.
- **Key substitution is defeated out of band.** New members are confirmed with a short authentication string: two people read the same short code aloud, and matching codes prove each holds the other's real key rather than an attacker's substitute.
- **Revocation is forward-only, and the API says so.** `age` wraps a *per-file* key to the recipients present at encrypt time. Removing a member bumps the roster epoch so every subsequent encryption excludes them, but it cannot claw back files they could already read — those bytes and the wrapped key inside them were already in their hands. There is deliberately **no** `revoke_access_to_existing_files`, because that function cannot be honestly implemented.
- **Known limits.** The roster carries labels and public keys, so it is metadata about who collaborates with whom — encrypt the roster file too if that matters. The crate is a building block: it is **not yet wired into the copy engine or the repository**. 7 IPC commands, a Settings tab, 13 `collab-*` Fluent keys across all 18 locales, and `tests/smoke/phase_51_collab.rs`.

### Three-tree state machine for P2P bidirectional sync (Phase 52)

- **LocalTree / SyncedTree / RemoteTree.** `freally_sync::tri_tree` decides one path from three inputs — what is here, what was last agreed, and what is there — rather than inferring intent from a single baseline record. The result is translated back into the existing `SyncAction` vocabulary, so the executor, the event stream, and the conflict UI are all unchanged; only the decision changes.
- **Opt-in, and two-way only.** Reachable via `SyncOptions::tri_tree` / `SyncSettings.tri_tree`, **default off**. The matrix is symmetric and has no concept of a forbidden direction, so `sync()` routes only `SyncMode::TwoWay` through it — every one-way mirror stays on the Phase 25 engine, which enforces direction and contribute's never-delete rule itself.
- **Not yet the crash-safe engine.** The module's `SyncedTree` prepare/commit journal is written and tested but has no production consumer: the engine currently adopts the decision matrix only.

### Interactive 3-way merge for non-text files (Phase 53)

- **A comparable rendering per format.** A text merge shows three columns of lines; a PNG, PSD, WAV, MP4 or PDF has no such view. `freally-mergeview` (36 tests) produces something a merge UI can place side by side: a blended image, a difference heatmap, a layer list, a waveform, a sample timeline, a page summary.
- **Honest about what each format supports.** Images (`image`) and PSD (`psd`) decode fully, so they get real per-pixel heatmaps and per-layer added/removed/changed diffs. Audio (`symphonia`) decodes to a peak/RMS waveform and a coarse band spectrogram. **Video (`mp4`) is a demuxer only** — duration, tracks, dimensions and a sample timeline, no decoded frames. **PDF (`pdf`) is a parser only** — page count and per-page size/text extent, no rasterised pages. A thumbnail strip and a page-pair image view would need ffmpeg and pdfium/mupdf, large native dependencies `cargo-deny`'s allowlist would have to be reopened for.
- **ffmpeg is opt-in.** With it enabled in Settings, video gets a real thumbnail strip spread across the clip using the demuxer's duration. Disabled or missing, the structural diff still works and says so — a re-encode, a page insertion, or a duration change all show up.
- **Hand off to a real editor.** `merge_handoff` writes the base / local / remote versions out so the user can resolve the merge in whatever tool actually edits that format. 12 IPC commands and a Settings tab.

### SMB compression + cloud-VM offload helper (Phase 40)

- **SMB 3.1.1 traffic compression badge** — when the destination is a UNC path on Windows, Freally File Manager requests `COPY_FILE_REQUEST_COMPRESSED_TRAFFIC` on every `CopyFileExW` / `CopyFile2` invocation and surfaces a teal "🗜 SMB compress: \<algo\>" pill in the header bar while the job runs. The flag is a *request* to the kernel — the SMB 3.1.1 server independently decides per-share whether to compress, and Windows does not surface the negotiated chained-compression algorithm (`XPRESS_LZ77` / `XPRESS_HUFFMAN` / `LZNT1`) to user mode, so today's badge labels a successful flag-pass rather than a confirmed compressed wire. Free win on slow remote links; ignored on local destinations.
...
### WASM plugin runtime (Phase 46 — shipped, 46.1 → 46.7)

- **Engine + ABI shipped.** New `freally-plugin` workspace crate wraps a real `wasmtime::Engine` (cranelift JIT, pruned feature set: `cranelift` / `runtime` / `wat` / `async`). `PluginHost::load_plugin(<path>)` reads `.wasm` (or `.wat`) bytes from disk and compiles them; `PluginHandle::call_hook(kind, ctx)` dispatches through a JSON-over-linear-memory ABI — the plugin must export `memory`, `alloc(size: i32) -> i32` (returns a pointer to a fresh buffer), and `hook(ctx_ptr: i32, ctx_len: i32) -> i64` (reads the JSON `HookCtx`, returns a packed `(out_ptr << 32) | out_len` pointing at the JSON `HookOutcome` response). Each call uses a fresh `wasmtime::Store` so plugins are stateless across calls.
- **Lifecycle hooks the engine dispatches.** `BeforeJob`, `BeforeFile`, `AfterFile`, `AfterJob`, `OnError`. Plugin response is one of `Continue`, `SkipFile`, `AbortJob`, or `Notify { message }`; the engine acts on the decoded outcome (skip the current file, abort the job, raise a tray/log notification, or proceed normally). `HookKind` serialises as snake_case and `HookOutcome` uses an internally-tagged `{"kind":"…"}` JSON shape so the ABI plugins compile against is stable.
- **Per-call sandbox budgets enforced.** Defaults: 1,000,000 fuel (CPU), 64 MiB linear memory, 50 ms wall-clock. Custom budgets via `PluginConfig { fuel_per_call, max_memory_bytes, wall_time_budget }` + `PluginHost::with_config(cfg)`. Fuel exhaustion (`Trap::OutOfFuel`) → `PluginError::FuelExhausted`; memory growth past the cap (rejected by a `wasmtime::ResourceLimiter`) → `PluginError::MemoryExceeded { wanted_bytes, max_bytes }`; wall-clock overrun (`epoch_interruption(true)` driven by a 1 ms `EpochTicker` thread the host owns) → `PluginError::WallTimeExceeded`. All three trap cooperatively because dispatch goes through `*_async` engine variants.
- **Manifest + capability gating.** Each plugin ships an adjacent `plugin.toml` declaring `name`, semver-shaped `version`, lifecycle `hooks`, and required `capabilities` (`read_fs:<scope>` / `write_fs:<scope>` / `network`). The user grants each capability per-plugin in the Settings UI; the per-call gate (`PluginHandle::call_hook`) consults the grant *before* the WASM instance is built, denying unauthorised dispatches with `PluginError::CapabilityDenied { plugin, capability }`. Stale grants outside the manifest are stripped on every read so a manifest downgrade can't leave a phantom permission live.
- **Four sample plugins ship in-tree** under `apps/freally-ui/plugins/`: `organize-by-exif` (proposes `<dest_root>/YYYY/MM/<basename>` from EXIF DateTaken), `notify-discord` (Discord-webhook envelope), `notify-ntfy` (ntfy.sh with priority/header sanitisation), `dedup-warning` (fires when `duplicates_found > threshold`). Each is a standalone `cdylib` with its own `Cargo.toml` (empty `[workspace]`) so the host's x86_64 build never tries to compile them; build them with `xtask build-sample-plugins`, which compiles each to `wasm32-unknown-unknown` release and stages `plugin.toml` next to the `.wasm`.
- **Settings → Plugins UI** (`apps/freally-ui/src/lib/settings/PluginsTab.svelte`). Lists installed plugins from `<config_dir>/plugins/<name>/{plugin.toml, plugin.wasm, state.toml}` with per-plugin enable toggle, manifest viewer (name, version, hooks), and a capability grant matrix (one row per manifest-declared capability with a per-row toggle). Install-from-file uses a Tauri dialog filtered to `.wasm`; Install-from-URL is hash-pinned across two distinct fetches — a preview call returns the BLAKE3 hex + parsed manifest without writing, then a commit call with `expected_hash = <preview hex>` re-downloads and refuses on drift before copying the staged files into the plugin store. Backed by 7 IPC commands in `plugin_commands.rs` (`plugin_list` / `plugin_enable` / `plugin_disable` / `plugin_install_from_file` / `plugin_install_from_url` / `plugin_grant_capability` / `plugin_revoke_capability`).
- **Wrap pass (46.7) shipped.** `/review` + `/security-review` audit closed with one defensive fix: `PluginHandle::call_hook` now clamps the plugin-returned response length to `PluginConfig::max_memory_bytes` before allocating the host-side decode buffer, so a malicious plugin returning `out_len = 0xFFFFFFFF` is rejected with `PluginError::OutOfBounds` rather than triggering a multi-GiB allocation. The cumulative `tests/smoke/phase_46_plugin.{rs,sh}` smoke exercises the manifest gate (denied capability blocks dispatch), the JSON-over-linear-memory ABI (the WAT skip-file plugin round-trips through `PluginHost`), the per-call sandbox (an infinite-loop plugin trips fuel exhaustion), and the new pre-read clamp end-to-end. WASI preview1 imports against granted capabilities (so a plugin with `read_fs:source` could call `fd_read` against the source root inside the sandbox) stay deferred — the four sample plugins emit declarative envelopes the engine forwards instead, which fits the post-copy-hook ABI without dragging the full WASI surface into the runtime.

### Drag-progress-merge + named queues + F2 + tray destinations (Phase 45)

- **Per-drive named queues.** `freally-core::QueueRegistry` partitions queued jobs by physical destination drive (`freally_platform::volume_id` on Windows VolumeSerialNumber, `st_dev` on Unix) so two transfers to the same disk share one queue and never compete for spindle / NAND head time, while transfers to *different* drives spawn parallel queues that actually run in parallel. The legacy `state.queue` stays in place as the back-compat default queue (`QueueId::DEFAULT`) so every existing entry point keeps working without changes.
...
- **Window-to-window drag-merge — deferred.** Tauri 2's `WindowEvent::DragDrop` doesn't expose HTML5 cross-window drag transfer; the title-bar drag-source / cross-process IPC pattern would need a custom protocol with native-level event interception. The single-instance-plugin design also means there's only ever one Freally window per session today. Parked until a Tauri capability lands or a multi-window multi-instance use case materialises.
...
`crates/freally-secure-delete/src/opal.rs` module behind the
...
via the safe-FFI seam in `freally-platform`; Tauri's device
...
trait surface — the `freally-secure-delete` crate now ships
...
- **Whole-drive sanitize via firmware** — NVMe Sanitize Crypto Erase (instant; rotates the drive's media key), NVMe Sanitize Block Erase (every cell), NVMe Format with Secure Erase (FSE bit), ATA Secure Erase (legacy SATA SSDs), and TCG OPAL Crypto Erase (Self-Encrypting Drives). The new `SsdSanitizeMode` enum + `whole_drive_sanitize` async API + `sanitize_capabilities` probe live in `freally-secure-delete::sanitize`. The actual privileged command runs through a pluggable `SanitizeHelper` trait — `NoopSanitizeHelper` ships as a safe-by-default fallback that refuses every call; the real Linux (`nvme-cli` / `hdparm` via `freally-helper`) and Windows (`DeviceIoControl(IOCTL_STORAGE_SECURITY_PROTOCOL_OUT)`) impls land in a Phase 44.1 follow-up.
...
- **Signed BLAKE3 + ed25519 manifest** per copy job, written as canonical CBOR (RFC 8949) at `<dst-root>/.freally-provenance.cbor`. The new `freally-provenance` crate (pure Rust, permissive licenses: `bao` 0.13 MIT/Apache-2.0, `ed25519-dalek` 2 BSD-3-Clause, `ciborium` 0.2 Apache-2.0) computes a per-file BLAKE3 root + Bao verified-streaming outboard during the source-read pass, aggregates them into a manifest with a Merkle root over the per-file roots, and detached-signs that manifest with a user-managed ed25519 key. **Reviewers re-hash the destination tree months later and prove no byte changed since the copy** — `freally provenance verify <MANIFEST>` re-hashes every file against its claimed root, validates the Merkle root, validates the signature, and reports `tampered: M (paths…)` on any divergence.
...
- **CLI surface**: `freally provenance verify <MANIFEST> [--trusted-key <PEM>]` (exit 0 clean / 4 tampered / 9 parse error) and `freally provenance keygen --out <PATH> [--write-public]` (PKCS#8 PEM private key + optional SPKI public-key sidecar).
...
- **Beats every measured competitor on Windows 11 NVMe** (10 GiB · same-volume copy):
  ...
  | Tool | MiB/s | vs Freally CLI |
  | --- | ---: | ---: |
  | **Freally CLI / engine** | **2429** | — |
  | **Freally UI (Phase 40)** | **2198** | −9 % |
  | Robocopy | 1305 | **−46 %** |
  | FastCopy | 1006 | **−59 %** |
  | cmd copy | 940-1147 | **−53-61 %** |
  | TeraCopy | 855 | **−65 %** |

  Freally's UI also beats every competitor by 68-157 % on the same workload. Caveat: cross-volume copies (C→D, C→E) are bound by the destination disk's write speed and tend to tie across all tools — engine speed only matters when the disk isn't the bottleneck.
...
- **Phase 40 named-pipe broker** — `freally-ui --enqueue` invocations forward argv to the running first instance via named pipe in **110 ms** instead of booting a fresh ~5 second Tauri runtime per call.
- **Phase 41 + 42 + 46 topology-driven overlapped-IOCP engagement** — large copies route through the overlapped pipeline whenever the destination's bus class benefits from a deep queue: NVMe at ≥256 MiB on **any** volume pairing (Phase 46 dropped the same-volume gate — modern NVMe sustained bandwidth often beats DRAM-to-DRAM coalescing, so the page cache stops being a useful staging buffer past a few hundred MB); SMB / iSCSI / RAID / VHDX at ≥1 GiB regardless of volume; SATA SSD / USB / SD / HDD only on cross-volume copies ≥1 GiB (the Phase 41 USB-enclosure rule). Each topology gets its own buffer / queue-depth / `NO_BUFFERING` profile via `IOCTL_STORAGE_QUERY_PROPERTY`. Override via `FREALLY_DISABLE_AUTO_OVERLAPPED=1` if the same-volume engagement regresses on specific hardware.
...
- **Phase 42 OpenZFS 2.2.x corruption warning** — one-shot stderr warning (suppressible via `FREALLY_SUPPRESS_ZFS_WARNING=1`) if the host runs OpenZFS 2.2.0-2.2.6 with `zfs_bclone_enabled=1` (openzfs/zfs#15526 data-corruption bug). Reflink path stays active.
...
- **Criterion benches** live at `crates/freally-core/benches/copy_bench.rs`: `single_huge_file`, `buffer_size_sweep`, `many_small_files` (10 KiB / 100 KiB / 1 MiB / 10 MiB mix), `mixed_tree` (10 KiB → 250 MiB).
- **Power-user env-var tunables** documented in [`docs/PERFORMANCE_TUNING.md`](docs/PERFORMANCE_TUNING.md): `FREALLY_PARALLEL_CHUNKS`, `FREALLY_OVERLAPPED_IO`, `FREALLY_OVERLAPPED_BUFFER_KB`, `FREALLY_OVERLAPPED_SLOTS`, `FREALLY_OVERLAPPED_NO_BUFFERING`, `FREALLY_NO_BUFFERING_THRESHOLD_MB`, `FREALLY_SKIP_ZERO_FILL` (Phase 46: defaults on when elevated; set to `0` to opt out), `FREALLY_DISABLE_AUTO_OVERLAPPED`.
...
├── crates/
│   ├── freally-core/           # async copy engine + streaming tree walk
│   ├── freally-hash/           # verify hashes
│   ├── freally-secure-delete/  # multi-pass shredding
│   ├── freally-history/        # SQLite history
│   ├── freally-platform/       # OS fast paths + shell hooks
│   ├── freally-settings/       # TOML settings + JSON profile store
│   ├── freally-i18n/           # Fluent loader
│   ├── freally-plugin/         # WASM plugin runtime (wasmtime-based, Phase 46)
│   ├── freally-chunk-remote/   # object-store pack backend + local cache (Phase 50)
│   ├── freally-collab/         # end-to-end encrypted collaboration (Phase 51)
│   └── freally-mergeview/      # 3-way merge previews for non-text files (Phase 53)
├── apps/freally-ui/            # Tauri 2.x + Svelte 5 shell
├── cdr-py/                      # CDR-0 reference reader for Python (Phase 50)
├── xtask/                       # workspace automation + benches
├── locales/<code>/freally.ftl  # 18 Fluent locale files
├── tests/smoke/                 # per-phase smoke tests
└── docs/                        # architecture, changelog, benchmarks, roadmap
    └── spec/CDR-0.md            # Common Dedup Repository v0 (CC0, Phase 50)
```
...
cargo bench -p freally-core --bench copy_bench
...
cargo test -p freally-core --test phase_01_core_copy -- --nocapture

# Phase 2 — 500-file tree copy + move
cargo test -p freally-core --test phase_02_tree_queue -- --nocapture

# Phase 3 — 500 MiB copy+verify across all 8 hash algorithms
cargo test -p freally-hash --test phase_03_verify --release -- --nocapture

# Phase 4 — 10 MiB shred across every ShredMethod
cargo test -p freally-secure-delete --test phase_04_shred -- --nocapture
...
cargo test -p freally-platform --test phase_06_fast_paths -- --nocapture

# Phase 13 — throughput floor (20 MiB/s on a 100 MiB single-file copy)
cargo test -p freally-ui --test phase_13_bench -- --nocapture

# Phase 16 — free-first packaging tripwire (no paid signing, free runners only)
cargo test -p freally-ui --test phase_16_package -- --nocapture

# Phase 17a — red-team `..` traversal rejection at every trust boundary
cargo test -p freally-core --test phase_17_security -- --nocapture
...
FREALLY_PHASE18_FULL=1 cargo test -p freally-ui --test phase_18_e2e --release

# Phase 23 — 100 MiB sparse copy: byte-exact + dst allocated <= 8 MiB
cargo test -p freally-platform --test phase_23_sparse -- --nocapture
```
...
| -------- | --------------------------- | ------------------------------------------------------------ |
| Windows  | MSI + NSIS (`.exe`)          | Double-click, or `winget install Freally.Freally` (after the winget-pkgs PR merges) |
| macOS    | `.app` + `.dmg` (x64 + arm64)| Right-click → Open on first launch, or `brew install --cask freally` |
| Linux    | AppImage + `.deb` + `.rpm`   | `sudo apt install ./freally-ui_<ver>_amd64.deb`, or Flathub / AUR |

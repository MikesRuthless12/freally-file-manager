...
# MT
settings-tab-bugreport = Relatar um erro

# Phase 51 — end-to-end encrypted collaboration
# MT
settings-tab-collab = Colaboração
collab-intro = Os arquivos compartilhados são criptografados para que só as pessoas desta lista consigam abri-los. Cada uma guarda a própria chave; não há senha compartilhada.
collab-forward-only = Remover alguém muda quem poderá abrir os arquivos criptografados a partir de então. Não recupera os que a pessoa já podia ler.
collab-members = Pessoas
collab-none = Ninguém foi adicionado ainda.
collab-label = Nome
collab-recipient = Chave pública dela
collab-add = Adicionar
collab-remove = Remover
collab-generate = Gerar uma chave para alguém
collab-identity-once = Copie agora — é exibida só uma vez e é a única forma de abrir os arquivos compartilhados:
collab-sas-label = Código de verificação
collab-sas-hint = Leia o código em voz alta para a outra pessoa. Se coincidirem, cada um tem a chave real do outro.

# Phase 53 — optional decoded video frames (user-supplied ffmpeg)
# MT
merge-heading = Prévias de vídeo
merge-ffmpeg-hint = O Freally pode mostrar quadros de um vídeo ao comparar versões. Isso exige o ffmpeg, que não é incluído — instale-o e o Freally o usará. Todo o resto funciona sem ele.
merge-ffmpeg-enable = Usar ffmpeg para quadros de vídeo
merge-ffmpeg-path = Caminho do ffmpeg (vazio para procurar no PATH)
merge-ffmpeg-found = Encontrado
merge-ffmpeg-missing = O ffmpeg não foi encontrado. A comparação de vídeo continua funcionando: mostra duração, tamanho e faixas em vez de imagens.
merge-ffmpeg-disabled = Os quadros de vídeo estão desativados. Ative o ffmpeg nas Configurações para comparar imagens.

collab-revoked = Removidas
collab-no-roster = Ainda não há uma lista de pessoas. Adicione alguém antes de compartilhar arquivos.
collab-admin-key-corrupt = Não foi possível ler a chave de administrador salva. Ela pode estar danificada.

bugreport-intro = Os relatórios são anônimos e nunca são enviados automaticamente. Você escolhe o que enviar e clica em Enviar — não mantemos nenhum servidor.
bugreport-pending = Há um relatório de falha de uma execução anterior aguardando.
bugreport-description-label = O que aconteceu?
bugreport-include-crash = Incluir o relatório de falha
bugreport-preview-label = Mostrar exatamente o que será enviado
bugreport-send-github = Abrir uma issue no GitHub
bugreport-send-gmail = Abrir o Gmail
bugreport-send-email = Abrir meu aplicativo de e-mail
bugreport-dismiss-crash = Descartar o relatório de falha
bugreport-simulate = Simular um relatório de falha
...
# MT
power-badge-paused = Pausado
power-badge-capped = Limitado
power-badge-tooltip = Uma política de energia está limitando as transferências. Abra Configurações → Energia.
...
snapshot-field-description = Descrição
snapshot-field-tags = Etiquetas (separadas por vírgulas)
...
# MT
repo-keys-title = Chaves de acesso
repo-keys-empty = Este repositório não tem bloqueio por senha.
repo-key-label = Rótulo
repo-key-password = Nova senha
repo-key-auth = Senha atual
repo-action-add-key = Adicionar chave
repo-action-remove-key = Revogar
repo-action-recovery = Gerar chave de recuperação

# Phase 50h — repository replication (3-2-1)
# MT
repo-action-replicate = Replicar para…

# Phase 50g — per-repository pack storage backend
# MT
repo-remote-label = Armazenamento de pacotes
repo-remote-local = Disco local
repo-remote-restart = As alterações valem na próxima inicialização.
repo-remote-unknown = Esse remoto não está configurado.
repo-remote-not-empty = Este repositório já contém dados. O armazenamento de pacotes só pode ser escolhido enquanto ele estiver vazio — use Replicar para copiar para um novo repositório.

repo-replicate-done = Replicado: { $copied } instantâneo(s) copiados, { $skipped } já presentes
repo-replicate-same-path = O destino é o mesmo repositório da origem.

repo-recovery-once = Guarde agora — é exibida apenas uma vez:
...
footer-hash = Hash
hash-inspector-title = Inspetor de hash
hash-inspector-algo-label = Algoritmo
hash-inspector-add-files = Adicionar arquivos…
hash-inspector-add-folders = Adicionar pastas…
hash-inspector-run = Calcular hash
hash-inspector-hashing = Calculando…
hash-inspector-clear = Limpar
hash-inspector-close = Fechar
hash-inspector-empty = Escolha arquivos ou pastas e calcule o hash.
hash-inspector-compare-label = Comparar com um hash
hash-inspector-compare-placeholder = Cole um hash, uma linha de checksum ou uma linha SFV
hash-inspector-compare-invalid = Isso não parece um hash.
hash-inspector-pair-identical = Os dois arquivos são idênticos
hash-inspector-pair-different = Os dois arquivos são diferentes
hash-inspector-truncated = Mostrando os primeiros { $rows } arquivos; o restante da seleção foi ignorado.

# FFM-M10 — copy-verification certificate export.
history-export-certificate = Certificado
history-export-certificate-hint = Exporte este trabalho como certificado de verificação imprimível em HTML / CSV / JSON
certificate-sign-prompt = Assinar o certificado com uma chave privada ed25519? Cancelar exporta sem assinatura.
certificate-key-picker-title = Escolha a chave de assinatura ed25519 (PKCS#8 PEM)
toast-certificate-exported = Certificado exportado ({ $files } arquivo(s))
toast-certificate-exported-signed = Certificado assinado exportado ({ $files } arquivo(s))

# FFM-M11 — verify-only re-audit.
footer-reaudit = Reverificar
history-reverify = Reverificar
history-reverify-hint = Recalcula o hash da origem e do destino deste trabalho e relata divergências (não escreve nada)
reaudit-title = Auditoria somente de verificação
reaudit-hint = Recalcula o hash de um par origem/destino e relata divergências — não escreve nada
reaudit-pick-source = Escolha a pasta de origem a reverificar
reaudit-pick-destination = Escolha a pasta de destino para comparar
reaudit-clean-title = Sem divergências — as árvores coincidem
reaudit-drift-title = Divergência detectada
reaudit-summary = { $ok } iguais, { $differs } divergentes, { $missing } ausentes, { $extra } excedentes, { $errors } ilegíveis ({ $algo })
reaudit-status-differs = Divergente
reaudit-status-missing = Ausente no destino
reaudit-status-extra = Somente no destino
reaudit-status-error = Ilegível
reaudit-close = Fechar

# FFM-M12 — cross-filesystem filename doctor.
doctor-title = Doutor de nomes de arquivo
doctor-summary = { $flagged } de { $scanned } nome(s) que o destino { $target } não aceita
doctor-issue-reserved-name = Nome de dispositivo reservado
doctor-issue-illegal-char = Caractere inválido
doctor-issue-trailing-dot-space = Ponto ou espaço no final
doctor-issue-component-too-long = Nome longo demais
doctor-issue-path-too-long = Caminho longo demais
doctor-issue-case-collision = Colisão de maiúsculas/minúsculas
doctor-no-rename = Sem renomeação segura
doctor-apply = Renomear { $count } arquivo(s)
doctor-toast-applied = { $renamed } renomeado(s), { $failed } com falha

# FFM-M13 — file-list import as a copy source.
action-import-list = Importar lista…
filelist-title = Importar lista de arquivos
filelist-hint = Use uma lista de caminhos TXT / CSV / JSON como conjunto de origem do trabalho
filelist-picker-title = Escolha uma lista de arquivos (TXT / CSV / JSON)
filelist-summary = { $found } arquivo(s) encontrado(s), { $missing } ausente(s)
filelist-missing-header = { $count } caminho(s) da lista não existem mais
filelist-preserve-structure = Preservar a estrutura de pastas em relação a uma raiz
filelist-relative-root-placeholder = Raiz relativa
filelist-toast-queued = { $files } arquivo(s) da lista na fila
filelist-toast-empty = Essa lista não continha caminhos

# FFM-M14 — preflight / dry-run report export.
preflight-report-action = Exportar plano
preflight-report-hint = Salve esta simulação como HTML / CSV / JSON antes de copiar qualquer coisa
preflight-report-toast = Plano exportado — { $files } arquivo(s), { $findings } achado(s) de nome

# FFM-M15 — damaged-media salvage.
footer-salvage = Resgatar
salvage-title = Resgatar de mídia danificada
salvage-hint = Recupera tudo o que for legível de um arquivo defeituoso, mapeia o ilegível e na próxima vez tenta apenas as lacunas
salvage-pick-source = Escolha o arquivo a resgatar
salvage-pick-destination = Onde gravar a cópia resgatada
salvage-toast-complete = Resgate concluído — { $bytes } byte(s), sem lacunas
salvage-toast-gaps = Resgatados { $bytes } byte(s); { $missing } byte(s) ilegíveis em { $gaps } lacuna(s)

# FFM-M16 — whole-job elevated batch with a consent ledger.
history-elevate-batch = Elevar tudo
history-elevate-batch-hint = Tente novamente todos os caminhos deste trabalho com permissão negada usando um único consentimento
elevate-batch-none = Nenhum caminho com permissão negada para elevar
elevate-batch-confirm = Conceder uma elevação para { $count } caminho(s)? ({ $skipped } outra(s) falha(s) não são resolvidas elevando.) O auxiliar copiará estes caminhos:
elevate-batch-toast = Lote elevado — { $copied } copiados, { $failed } com falha

# ===== Build 3 (FFM-M17..M24) =====

# FFM-M23
err-source-changed = O arquivo de origem foi alterado durante a leitura
job-source-changed = Origem alterada durante a cópia
settings-source-stability-label = Se um arquivo de origem mudar durante a cópia
settings-source-stability-description = Reverifica o tamanho e a data de cada origem após a leitura. A verificação sozinha não detecta isso.
settings-source-stability-off = Não verificar
settings-source-stability-warn = Avisar e marcar o arquivo
settings-source-stability-recopy = Copiar o arquivo novamente
settings-source-stability-fail = Marcar o arquivo como falho

# FFM-M17
err-schedule-id-invalid = Identificador de agendamento inválido
err-schedule-label-invalid = Nome de agendamento inválido
err-schedule-verb-invalid = Um agendamento só pode copiar ou mover
err-schedule-trigger-invalid = Horário de agendamento inválido
err-schedule-too-many = Agendamentos demais
...
settings-schedule-trigger-hourly = A cada hora
settings-schedule-trigger-daily = Todos os dias
settings-schedule-trigger-weekly = Toda semana
settings-schedule-next-run = Próxima execução
settings-schedule-missed = Executar assim que possível um agendamento perdido
settings-schedule-missed-unsupported = Este sistema sempre executa um agendamento perdido ao despertar.
settings-schedule-not-installed = Ausente do agendador do sistema

# FFM-M18
err-affinity-too-many = Grupos de filas de unidade em excesso
err-affinity-name-invalid = Nome de grupo inválido
err-affinity-name-duplicate = Outro grupo já usa este nome
err-affinity-prefixes-invalid = Um grupo precisa de pelo menos uma pasta
err-affinity-workers-invalid = Número de processos inválido
settings-affinity-title = Filas de unidade
settings-affinity-description = Defina quais destinos compartilham uma fila de transferência quando a detecção de unidades erra.
settings-affinity-add = Adicionar grupo
settings-affinity-workers = Processos
...
# FFM-M20
err-favorite-label-invalid = Nome de favorito inválido
err-favorite-kind-invalid = Tipo de favorito inválido
err-favorite-hotkey-invalid = Atalho inválido
err-favorite-too-many = Favoritos demais
settings-favorites-title = Favoritos
settings-favorites-empty = Nenhum favorito ainda.
settings-favorites-add = Adicionar favorito
favorites-recent-title = Pares recentes
favorites-kind-source = Origem
favorites-kind-destination = Destino
favorites-kind-pair = Par

# FFM-M21
err-shell-portable = A integração com o shell não está disponível no modo portátil
settings-portable-title = Modo portátil
settings-portable-active = O modo portátil está ativo. Configurações, histórico e diários são guardados em { $path }.
settings-portable-keychain-warning = As credenciais de nuvem ficam em um arquivo criptografado na unidade portátil, desbloqueado por uma senha escolhida por você — elas não são gravadas neste computador. Se esquecer a senha, as credenciais salvas não podem ser recuperadas.

# FFM-M21 — portable credential store (age-encrypted file)
# MT
cloud-error-credentials-locked = As credenciais de nuvem estão bloqueadas. Desbloqueie o arquivo de credenciais desta unidade para usar este backend.
cloud-error-credentials-bad-passphrase = Essa senha não abre o arquivo de credenciais.
cloud-error-credentials-empty-passphrase = Digite uma senha.
cloud-error-credentials-not-portable = Esta instalação guarda as credenciais no chaveiro do sistema, então não há nada para desbloquear.
portable-credentials-title = Credenciais de nuvem portáteis
portable-credentials-unlock-body = Digite a senha que protege o arquivo de credenciais desta unidade.
portable-credentials-create-body = Escolha uma senha para proteger as credenciais de nuvem nesta unidade. Ela não pode ser recuperada se você esquecê-la.
portable-credentials-passphrase-label = Senha
portable-credentials-unlock-action = Desbloquear
portable-credentials-lock-action = Bloquear


# FFM-M24
err-autostart-portable = Iniciar ao fazer login não está disponível no modo portátil
err-autostart-failed = Não foi possível alterar o início ao fazer login
settings-autostart-label = Iniciar o Freally ao fazer login
settings-autostart-description = Inicia minimizado na bandeja para que integração com o shell, atalhos, monitoramentos e agendamentos fiquem ativos desde o login.
settings-autostart-unavailable-portable = Indisponível no modo portátil — uma entrada de inicialização apontaria para esta unidade removível.
settings-autostart-unavailable-platform = Iniciar ao fazer login não está disponível neste sistema.
...
settings-tab-queues = Filas de unidade
settings-tab-favorites = Favoritos
settings-schedule-name = Nome
settings-schedule-verb = Ação
settings-schedule-verb-copy = Copiar
settings-schedule-verb-move = Mover
settings-schedule-manifest = Lista de arquivos
settings-schedule-destination = Destino
settings-schedule-when = Frequência
settings-schedule-weekday = Dia da semana (0 = domingo)
settings-schedule-hour = Hora
settings-schedule-minute = Minuto
settings-affinity-name = Nome do grupo
settings-affinity-paths = Pastas
settings-affinity-path-add = Adicionar pasta…
settings-favorites-name = Nome
settings-favorites-kind = Tipo
settings-favorites-path = Pasta
settings-favorites-destination = Destino
settings-favorites-hotkey = Atalho

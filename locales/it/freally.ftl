...
toast-clipboard-files-detected = File negli appunti — premi la scorciatoia di incolla per copiarli con Freally File Manager
...
# MT
settings-tab-bugreport = Segnala un bug

# Phase 51 — end-to-end encrypted collaboration
# MT
settings-tab-collab = Collaborazione
collab-intro = I file condivisi sono cifrati perché solo le persone elencate qui possano aprirli. Ognuna conserva la propria chiave; non esiste una password condivisa.
collab-forward-only = Rimuovere qualcuno cambia chi potrà aprire i file cifrati da quel momento. Non recupera quelli che poteva già leggere.
collab-members = Persone
collab-none = Non è stato ancora aggiunto nessuno.
collab-label = Nome
collab-recipient = La sua chiave pubblica
collab-add = Aggiungi
collab-remove = Rimuovi
collab-generate = Genera una chiave per qualcuno
collab-identity-once = Copiala ora: viene mostrata una sola volta ed è l’unico modo per aprire i file condivisi:
collab-sas-label = Codice di verifica
collab-sas-hint = Leggi il codice ad alta voce all’altra persona. Se coincidono, ciascuno ha la chiave reale dell’altro.

# Phase 53 — optional decoded video frames (user-supplied ffmpeg)
# MT
merge-heading = Anteprime video
merge-ffmpeg-hint = Freally può mostrare fotogrammi di un video durante il confronto. Serve ffmpeg, che non è incluso: installalo tu e Freally lo userà. Tutto il resto funziona anche senza.
merge-ffmpeg-enable = Usa ffmpeg per i fotogrammi
merge-ffmpeg-path = Percorso di ffmpeg (vuoto per cercare nel PATH)
merge-ffmpeg-found = Trovato
merge-ffmpeg-missing = ffmpeg non è stato trovato. Il confronto video funziona comunque: mostra durata, dimensioni e tracce invece delle immagini.
merge-ffmpeg-disabled = I fotogrammi video sono disattivati. Attiva ffmpeg nelle Impostazioni per confrontare le immagini.

collab-revoked = Rimosse
collab-no-roster = Non c'è ancora un elenco di persone. Aggiungi qualcuno prima di condividere i file.
collab-admin-key-corrupt = Non è stato possibile leggere la chiave di amministratore salvata. Potrebbe essere danneggiata.

bugreport-intro = Le segnalazioni sono anonime e non vengono mai inviate automaticamente. Scegli tu cosa inviare e premi Invia — non gestiamo alcun server.
bugreport-pending = È in attesa un rapporto di arresto anomalo da un’esecuzione precedente.
bugreport-description-label = Che cosa è successo?
bugreport-include-crash = Includi il rapporto di arresto anomalo
bugreport-preview-label = Mostra esattamente ciò che verrà inviato
bugreport-send-github = Apri una issue su GitHub
bugreport-send-gmail = Apri Gmail
bugreport-send-email = Apri la mia app di posta
bugreport-dismiss-crash = Elimina il rapporto di arresto anomalo
bugreport-simulate = Simula un rapporto di arresto anomalo
...
settings-paste-shortcut-hint = Premi questa combinazione ovunque sul sistema per incollare i file copiati da Explorer / Finder / Files tramite Freally File Manager. CmdOrCtrl corrisponde a Cmd su macOS e a Ctrl su Windows / Linux.
settings-clipboard-watcher = Monitora gli appunti per i file copiati
settings-clipboard-watcher-hint = Mostra un avviso quando negli appunti compaiono URL di file, suggerendo che puoi incollarli tramite Freally File Manager. Controlla ogni 500 ms quando è attivo.
...
settings-intercept-copy-hint = Quando è attivo, Ctrl+C / Ctrl+V di Explorer passa attraverso Freally File Manager. La registrazione arriverà nella Phase 14.
...
settings-updater-hint = Freally File Manager verifica gli aggiornamenti firmati al massimo una volta al giorno. Gli aggiornamenti si installano alla successiva chiusura dell'app.
...
settings-on-locked-hint = Elimina gli errori "file in uso da un altro processo". Freally File Manager crea uno snapshot del volume di origine (VSS su Windows, ZFS/Btrfs su Linux, APFS su macOS) e legge dalla copia snapshot.
snapshot-prompt-title = Questo file è in uso da un altro processo
snapshot-prompt-body = Un altro programma ha { $path } aperto in scrittura esclusiva. Scegli come Freally File Manager deve gestire questo file e altri simili sullo stesso volume.
...
snapshot-vss-needs-elevation = La lettura da uno snapshot VSS richiede autorizzazioni di amministratore. Freally File Manager ti chiederà di consentirla.
...
resume-prompt-body = Freally File Manager ha rilevato { $count } trasferimento/i incompleto/i da una sessione precedente. Scegli cosa fare per ciascuno.
...
dropstack-show-tray-icon = Mostra l'icona di Freally File Manager nella barra delle applicazioni
...
# MT
power-badge-paused = In pausa
power-badge-capped = Limitato
power-badge-tooltip = Un criterio di alimentazione sta limitando i trasferimenti. Apri Impostazioni → Alimentazione.
...
settings-mount-on-launch-path-placeholder = es. C:\Mounts\freally
...
settings-audit-file-path-placeholder = es. C:\ProgramData\Freally\audit.log
...
cli-help-tagline = Freally File Manager CLI — copia di file byte-esatta, sincronizzazione, verifica e controllo per pipeline CI/CD.
...
cli-info-shape-recorded = Limite di banda "{ $rate }" registrato; l'applicazione avviene tramite freally-shape
...
settings-mobile-pair-active = Server di associazione attivo — scansiona il QR con l'app mobile di Freally File Manager
...
settings-dedup-hint = Quando origine e destinazione condividono un volume, Freally File Manager può clonare i file a livello di filesystem invece di copiare i byte. Il reflink è istantaneo e sicuro; l'hardlink è più veloce ma entrambi i nomi condividono lo stato.
...
plugin-hint = I plugin WASM in sandbox estendono Freally File Manager con hook personalizzati. Ogni plugin viene eseguito con limiti di CPU e memoria per chiamata e vede solo le capacità host che gli concedi.
...
server-hint = Esegui Freally File Manager come server di file headless. Scegli i protocolli da esporre, imposta l'indirizzo e la cartella da servire e, facoltativamente, richiedi l'autenticazione.
...
library-tab-compare = Confronta
repo-change-added = Aggiunto
repo-change-removed = Rimosso
repo-change-modified = Modificato
repo-change-unchanged = Nessuna modifica
repo-diff-summary = { $added } aggiunti · { $removed } rimossi · { $modified } modificati
repo-diff-bytes-added = { $bytes } nuovi
repo-diff-pick-two = Seleziona due snapshot da confrontare
# Phase 49r — statistics / reports.
library-tab-reports = Report
report-growth-title = Crescita dell'archiviazione
report-by-kind-title = Per tipo
report-top-files-title = File principali
report-dedup-ratio = { $pct }% deduplicato
report-export = Esporta report
report-exported = Report salvato in { $path }
report-file-versions = { $n } versioni
# Phase 49p — pinning / prune.
repo-pin = Fissa
repo-unpin = Annulla fissaggio
repo-pinned-badge = Fissato
repo-prune-title = Sfoltisci
repo-prune-keep-last = Mantieni i più recenti
repo-prune-removed = { $n } snapshot sfoltiti
repo-prune-none = Niente da sfoltire
...
backup-retention-keep-all = Mantieni tutto
backup-retention-last = Mantieni gli ultimi { $n }
backup-retention-days = Più vecchi di { $days } giorni
backup-retention-gfs = Rotazione GFS
backup-prune-now = Pota ora
backup-prune-none = Niente da potare
backup-prune-result = Rimosse { $removed } istantanee · recuperati { $bytes }
# Phase 49f — per-source scheduling.
backup-schedule = Pianificazione
backup-schedule-manual = Manuale
backup-schedule-hourly = Ogni ora
backup-schedule-daily = Giornaliero
backup-schedule-weekly = Settimanale
backup-next-run = Prossima esecuzione { $when }
backup-not-scheduled = Non pianificato
# Phase 49g — source filters.
backup-include-ph = Glob di inclusione (separati da virgole)
backup-skip-hidden = Salta i nascosti
# Phase 49q — notifications.
notify-title = Notifiche
notify-on-success = In caso di successo
notify-on-failure = In caso di errore
notify-test = Invia test
notify-test-sent = Test inviato a { $n } destinazione/i
...
# MT
snapshot-field-label = Nome
snapshot-field-description = Descrizione
snapshot-field-tags = Tag (separati da virgole)
...
library-compact = Compattazione completa
library-compact-started = Compattazione avviata — controlla Attività
# Phase 49h — compression.
library-stat-compression = Risparmiato con la compressione
storage-compression = Compressione
storage-compression-off = Disattivata
storage-compression-auto = Automatico (salta gli incomprimibili)
storage-compression-always = Sempre
storage-compression-restart = Si applica al prossimo avvio
# Phase 49j — tasks & progress center.
footer-tasks = Attività
...
task-state-running = In esecuzione
task-state-completed = Completato
task-state-failed = Non riuscito
task-state-cancelled = Annullato
# Phase 49k — repository setup/connect wizard.
repo-wizard-title = Connetti repository
repo-wizard-create-tab = Crea nuovo
repo-wizard-connect-tab = Connetti esistente
repo-field-name = Nome
repo-field-path = Posizione
repo-field-password = Passphrase (facoltativa)
repo-action-create = Crea
repo-action-connect = Connetti
repo-action-browse = Sfoglia…
repo-switcher-label = Repository
repo-action-forget = Dimentica
repo-action-change-pass = Cambia passphrase…

# Phase 50i — repository access key slots
# MT
repo-keys-title = Chiavi di accesso
repo-keys-empty = Questo repository non ha un blocco con passphrase.
repo-key-label = Etichetta
repo-key-password = Nuova password
repo-key-auth = Password esistente
repo-action-add-key = Aggiungi chiave
repo-action-remove-key = Revoca
repo-action-recovery = Genera chiave di ripristino

# Phase 50h — repository replication (3-2-1)
# MT
repo-action-replicate = Replica in…

# Phase 50g — per-repository pack storage backend
# MT
repo-remote-label = Archiviazione dei pack
repo-remote-local = Disco locale
repo-remote-restart = Le modifiche hanno effetto al prossimo avvio.
repo-remote-unknown = Quel remoto non è configurato.
repo-remote-not-empty = Questo repository contiene già dati. L'archiviazione dei pacchetti può essere scelta solo finché è vuoto: usa Replica per copiarlo in un nuovo repository.

repo-replicate-done = Replicato: { $copied } snapshot copiati, { $skipped } già presenti
repo-replicate-same-path = La destinazione è lo stesso repository dell’origine.

repo-recovery-once = Salvala ora: viene mostrata una sola volta:

repo-password-old = Passphrase attuale
repo-password-new = Nuova passphrase
repo-error-exists = Esiste già un repository in questa posizione
repo-error-not-found = Nessun repository trovato in questa posizione
repo-error-bad-pass = Passphrase errata
repo-note-no-encryption = La passphrase regola solo l'accesso; la crittografia a riposo arriverà in una versione successiva
repo-confirm-forget = Rimuovere "{ $name }" dall'elenco? I tuoi dati restano sul disco.
repo-toast-created = Repository "{ $name }" creato
repo-toast-connected = Connesso a "{ $name }"
repo-toast-pass-changed = Passphrase aggiornata
# Phase 49l — Sources dashboard.
library-tab-overview = Panoramica
library-source-empty = Ancora nessuna origine
library-source-unknown = (origine non specificata)
library-source-snapshots = { $n } istantanee
library-source-latest = Più recente { $when }
# Phase 49n — verify & repair.
repo-action-verify = Verifica
repo-action-verify-deep = Verifica (leggi tutti i dati)
repo-action-repair = Ripara…
repo-verify-clean = Verificati { $files } file / { $chunks } blocchi — nessun danno
repo-verify-damaged = { $missing } mancanti, { $corrupt } blocchi corrotti
repo-repair-confirm = Rimuovere { $n } istantanee che non è più possibile ripristinare?
repo-repair-removed = Rimosse { $n } istantanee danneggiate
repo-repair-none = Niente da riparare — il repository è integro
...
eula-intro = Leggi il contratto qui sotto. Devi accettarlo prima di usare Freally File Manager.
eula-scroll-hint = Scorri fino alla fine per abilitare "Accetto".
eula-thanks = Grazie per la lettura.
eula-agree = Accetto
eula-decline = Rifiuta ed esci
eula-error = Impossibile registrare l'accettazione: { $error }

# FFM-M01 — Explorer copy-verb takeover.
settings-intercept-copy-unsupported = L'intercettazione della copia è disponibile solo su Windows.
settings-intercept-copy-needs-menu = Attiva prima l'integrazione nel menu contestuale: il gestore di copia deve essere registrato prima che l'intercettazione possa subentrare.
settings-revert-copy-handler = Ripristina il gestore di copia di Windows
toast-copy-handler-reverted = Gestore di copia di Windows ripristinato
settings-context-menu-hint = Registra o rimuove il menu contestuale e il gestore di copia di Freally nel sistema (per utente, senza admin).
paste-chooser-title = Copia e incolla
paste-chooser-close = Chiudi
paste-chooser-files = { $count } file — scegli una destinazione
paste-chooser-system-copy = Copia di sistema
paste-chooser-system-move = Spostamento di sistema
paste-chooser-system-hint = Trasferimento semplice e veloce, senza verifica
paste-chooser-freally-copy = Copia Freally
paste-chooser-freally-move = Spostamento Freally
paste-chooser-freally-hint = Trasferimento verificato byte per byte
paste-chooser-replace-older = Freally — sostituisci i file più vecchi
paste-chooser-replace-older-hint = Verificato; sovrascrive solo se l'origine è più recente
paste-chooser-more = Altre opzioni…
toast-system-paste-done = { $items } elemento/i incollato/i

# FFM-M02 — transactional undo.
undo-title-copy = Annullare la copia — rimuovere i file copiati?
undo-title-move = Annullare lo spostamento — riportare indietro i file?
undo-summary = { $ready } di { $total } elementi possono essere annullati; il resto è cambiato, scomparso o in conflitto.
undo-action-trash = Nel cestino
undo-action-move-back = Riporta indietro
undo-status-ready = Pronto
undo-status-skip-missing = Mancante — saltato
undo-status-skip-changed = Modificato — saltato
undo-status-conflict = Percorso originale occupato
undo-cancel = Annulla
undo-confirm = Annulla { $count } elemento/i
toast-undo-done = Annullamento completato: { $done } fatti, { $skipped } saltati, { $failed } falliti
toast-undo-nothing = Niente da annullare
history-undo = Annulla
history-undo-hint = Inverte questo lavoro: i file copiati vanno nel cestino, quelli spostati tornano alla posizione originale

# FFM-M03 — trash-aware delete.
menu-trash-source = Elimina origine nel cestino
trash-confirm = Spostare nel cestino?
{ $path }
toast-trash-done = Spostato nel cestino: { $trashed } elemento/i, { $failed } falliti
settings-safety-confirm-trash = Conferma prima di eliminare nel cestino
settings-safety-move-to-trash = Sposta i file di origine nel cestino
settings-safety-move-to-trash-hint = Durante uno spostamento, invia l'origine al cestino invece di eliminarla — uno spostamento recuperabile.

# FFM-M04/M05 — eject + keep-awake.
menu-eject-destination = Espelli il volume di destinazione
toast-eject-done = Volume espulso — rimozione sicura
toast-eject-failed = Impossibile espellere: { $error }
settings-power-keep-awake = Mantieni il computer attivo durante i lavori
settings-power-keep-awake-hint = Mantiene un blocco di sistema (niente sospensione né screensaver) finché un lavoro copia.

# FFM-M06 — content-aware collision policies.
collision-policy-skip-identical-else-overwrite = Sovrascrivi solo se il contenuto è diverso
collision-policy-skip-identical-else-prompt = Salta se identico, altrimenti chiedi

# FFM-M07 — failed-file ledger + retry.
history-retry-failed = Riprova falliti
history-retry-failed-hint = Ricopia solo i file falliti in questo lavoro
history-export-failed = Esporta falliti
history-export-failed-hint = Salva l'elenco dei file falliti come CSV / TXT / JSON
toast-retry-failed-none = Nessun file fallito da riprovare
toast-retry-failed-queued = { $count } file falliti rimessi in coda
toast-failed-exported = Elenco falliti esportato

# FFM-M08 — checksum sidecars.
menu-create-checksums = Crea checksum (SHA-256)
toast-checksums-created = Checksum scritti per { $files } file
sidecar-verify-clean-title = Tutti i file verificati
sidecar-verify-bad-title = Verifica dei checksum fallita
sidecar-verify-summary = { $ok } OK, { $failed } falliti, { $missing } mancanti
...
# FFM-M23
err-source-changed = Il file di origine è cambiato durante la lettura
job-source-changed = Origine modificata durante la copia
settings-source-stability-label = Se un file di origine cambia durante la copia
settings-source-stability-description = Ricontrolla dimensione e data di ogni origine dopo averla letta. La verifica da sola non è in grado di rilevarlo.
settings-source-stability-off = Non controllare
settings-source-stability-warn = Avvisa e contrassegna il file
settings-source-stability-recopy = Copia di nuovo il file
settings-source-stability-fail = Segna il file come non riuscito

# FFM-M17
err-schedule-id-invalid = Identificatore di pianificazione non valido
err-schedule-label-invalid = Nome della pianificazione non valido
err-schedule-verb-invalid = Una pianificazione può solo copiare o spostare
err-schedule-trigger-invalid = Orario di pianificazione non valido
err-schedule-too-many = Troppe esecuzioni pianificate
...
settings-schedule-trigger-hourly = Ogni ora
settings-schedule-trigger-daily = Ogni giorno
settings-schedule-trigger-weekly = Ogni settimana
settings-schedule-next-run = Prossima esecuzione
settings-schedule-missed = Recupera al più presto un'esecuzione mancata
settings-schedule-missed-unsupported = Questo sistema recupera sempre un'esecuzione mancata alla riattivazione.
settings-schedule-not-installed = Assente dall'utilità di pianificazione del sistema

# FFM-M18
err-affinity-too-many = Troppi gruppi di code delle unità
err-affinity-name-invalid = Nome del gruppo non valido
err-affinity-name-duplicate = Un altro gruppo usa già questo nome
err-affinity-prefixes-invalid = Un gruppo richiede almeno una cartella
err-affinity-workers-invalid = Numero di processi non valido
settings-affinity-title = Code delle unità
settings-affinity-description = Stabilisci quali destinazioni condividono una coda di trasferimento quando il rilevamento delle unità sbaglia.
settings-affinity-add = Aggiungi gruppo
settings-affinity-workers = Processi
settings-affinity-workers-auto = Automatico
...
# FFM-M20
err-favorite-label-invalid = Nome del preferito non valido
err-favorite-kind-invalid = Tipo di preferito non valido
err-favorite-hotkey-invalid = Scorciatoia non valida
err-favorite-too-many = Troppi preferiti
settings-favorites-title = Preferiti
settings-favorites-empty = Nessun preferito.
settings-favorites-add = Aggiungi preferito
favorites-recent-title = Coppie recenti
favorites-kind-source = Origine
favorites-kind-destination = Destinazione
favorites-kind-pair = Coppia

# FFM-M21
err-shell-portable = L'integrazione con la shell non è disponibile in modalità portatile
settings-portable-title = Modalità portatile
settings-portable-active = La modalità portatile è attiva. Impostazioni, cronologia e journal sono archiviati in { $path }.
settings-portable-keychain-warning = Le credenziali cloud sono conservate in un file cifrato sull’unità portatile, sbloccato da una passphrase scelta da te: non vengono scritte su questo computer. Se dimentichi la passphrase, le credenziali salvate non sono recuperabili.

# FFM-M21 — portable credential store (age-encrypted file)
# MT
cloud-error-credentials-locked = Le credenziali cloud sono bloccate. Sblocca il file delle credenziali su questa unità per usare questo backend.
cloud-error-credentials-bad-passphrase = Questa passphrase non apre il file delle credenziali.
cloud-error-credentials-empty-passphrase = Inserisci una passphrase.
cloud-error-credentials-not-portable = Questa installazione conserva le credenziali nel portachiavi di sistema, quindi non c’è nulla da sbloccare.
portable-credentials-title = Credenziali cloud portatili
portable-credentials-unlock-body = Inserisci la passphrase che protegge il file delle credenziali su questa unità.
portable-credentials-create-body = Scegli una passphrase per proteggere le credenziali cloud su questa unità. Non è recuperabile se la dimentichi.
portable-credentials-passphrase-label = Passphrase
portable-credentials-unlock-action = Sblocca
portable-credentials-lock-action = Blocca


# FFM-M24
err-autostart-portable = L'avvio all'accesso non è disponibile in modalità portatile
err-autostart-failed = Impossibile modificare l'avvio all'accesso
settings-autostart-label = Avvia Freally all'accesso
settings-autostart-description = Si avvia ridotto a icona nella barra delle applicazioni, così integrazione con la shell, scorciatoie, monitoraggi e pianificazioni sono attivi fin dall'accesso.
settings-autostart-unavailable-portable = Non disponibile in modalità portatile: una voce di avvio punterebbe a questa unità rimovibile.
settings-autostart-unavailable-platform = L'avvio all'accesso non è disponibile su questo sistema.
...
settings-tab-queues = Code delle unità
settings-tab-favorites = Preferiti
settings-schedule-name = Nome
settings-schedule-verb = Azione
settings-schedule-verb-copy = Copia
settings-schedule-verb-move = Sposta
settings-schedule-manifest = Elenco file
settings-schedule-destination = Destinazione
settings-schedule-when = Frequenza
settings-schedule-weekday = Giorno della settimana (0 = domenica)
settings-schedule-hour = Ora
settings-schedule-minute = Minuto
settings-affinity-name = Nome del gruppo
settings-affinity-paths = Cartelle
settings-affinity-path-add = Aggiungi cartella…
settings-favorites-name = Nome
settings-favorites-kind = Tipo
settings-favorites-path = Cartella
settings-favorites-destination = Destinazione
settings-favorites-hotkey = Scorciatoia

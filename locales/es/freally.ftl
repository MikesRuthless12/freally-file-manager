...
toast-clipboard-files-detected = Hay archivos en el portapapeles — pulsa tu atajo de pegar para copiarlos con Freally File Manager
...
settings-paste-shortcut-hint = Pulsa esta combinación en cualquier parte del sistema para pegar archivos copiados desde Explorer / Finder / Files con Freally File Manager. CmdOrCtrl equivale a Cmd en macOS y a Ctrl en Windows / Linux.
settings-clipboard-watcher = Vigilar el portapapeles en busca de archivos copiados
settings-clipboard-watcher-hint = Muestra un aviso cuando aparecen URL de archivos en el portapapeles, sugiriendo que puedes pegarlos con Freally File Manager. Consulta cada 500 ms mientras está activado.
...
settings-intercept-copy-hint = Cuando está activado, el Ctrl+C / Ctrl+V de Explorer pasa por Freally File Manager. El registro llega en la Fase 14.
...
settings-updater-hint = Freally File Manager busca actualizaciones firmadas como mucho una vez al día. Las actualizaciones se instalan al cerrar la app.
...
settings-on-locked-hint = Elimina los errores de "archivo en uso por otro proceso". Freally File Manager toma una instantánea del volumen de origen (VSS en Windows, ZFS/Btrfs en Linux, APFS en macOS) y lee desde la copia de la instantánea.
snapshot-prompt-title = Este archivo está en uso por otro proceso
snapshot-prompt-body = Otro programa tiene { $path } abierto con escritura exclusiva. Elige cómo debe gestionar Freally File Manager este y otros archivos similares del mismo volumen.
...
snapshot-vss-needs-elevation = Leer desde una instantánea VSS requiere permisos de administrador. Freally File Manager te pedirá que lo autorices.
...
resume-prompt-body = Freally File Manager detectó { $count } transferencia(s) sin terminar de una sesión anterior. Elige qué hacer con cada una.
...
dropstack-show-tray-icon = Mostrar el icono de Freally File Manager en la bandeja
...
settings-mount-on-launch-path-placeholder = p. ej. C:\Mounts\freally
...
settings-audit-file-path-placeholder = p. ej. C:\ProgramData\Freally\audit.log
...
cli-help-tagline = Freally File Manager CLI — copia, sincronización, verificación y auditoría de archivos byte a byte para pipelines de CI/CD.
...
cli-info-shape-recorded = Forma de ancho de banda "{ $rate }" registrada; la aplicación se realiza mediante freally-shape
...
settings-mobile-pair-active = Servidor de emparejamiento activo — escanea el QR con la app móvil de Freally File Manager
...
settings-dedup-hint = Cuando el origen y el destino comparten volumen, Freally File Manager puede clonar archivos a nivel del sistema de archivos en lugar de copiar bytes. El reflink es instantáneo y seguro; el enlace duro es más rápido, pero ambos nombres comparten estado.
...
plugin-hint = Los complementos WASM en espacio aislado amplían Freally File Manager con enlaces personalizados. Cada complemento se ejecuta con límites de CPU y memoria por llamada y solo ve las capacidades del host que le concedas.
...
server-hint = Ejecuta Freally File Manager como un servidor de archivos sin interfaz. Elige los protocolos que exponer, define la dirección y la carpeta que servir y, opcionalmente, exige autenticación.
...
repo-change-added = Añadido
repo-change-removed = Eliminado
repo-change-modified = Modificado
repo-change-unchanged = Sin cambios
repo-diff-summary = { $added } añadidos · { $removed } eliminados · { $modified } modificados
repo-diff-bytes-added = { $bytes } nuevos
repo-diff-pick-two = Elige dos instantáneas para comparar
# Phase 49r — statistics / reports.
library-tab-reports = Informes
report-growth-title = Crecimiento del almacenamiento
report-by-kind-title = Por tipo
report-top-files-title = Archivos principales
report-dedup-ratio = { $pct }% deduplicado
report-export = Exportar informe
report-exported = Informe guardado en { $path }
report-file-versions = { $n } versiones
# Phase 49p — pinning / prune.
repo-pin = Fijar
repo-unpin = Dejar de fijar
repo-pinned-badge = Fijada
repo-prune-title = Purgar
repo-prune-keep-last = Conservar las más recientes
repo-prune-removed = { $n } instantáneas purgadas
repo-prune-none = Nada que purgar
...
backup-retention-keep-all = Conservar todo
backup-retention-last = Conservar las últimas { $n }
backup-retention-days = Anteriores a { $days } días
backup-retention-gfs = Rotación GFS
backup-prune-now = Purgar ahora
backup-prune-none = Nada que purgar
backup-prune-result = Se eliminaron { $removed } instantáneas · se liberaron { $bytes }
# Phase 49f — per-source scheduling.
backup-schedule = Programación
backup-schedule-manual = Manual
backup-schedule-hourly = Cada hora
backup-schedule-daily = Diariamente
backup-schedule-weekly = Semanalmente
backup-next-run = Próxima ejecución { $when }
backup-not-scheduled = No programado
# Phase 49g — source filters.
backup-include-ph = Globs de inclusión (separados por comas)
backup-skip-hidden = Omitir ocultos
# Phase 49q — notifications.
notify-title = Notificaciones
notify-on-success = Al tener éxito
notify-on-failure = Al fallar
notify-test = Enviar prueba
notify-test-sent = Prueba enviada a { $n } destino(s)
...
library-compact = Compactación completa
library-compact-started = Compactación iniciada — consulta Tareas
# Phase 49h — compression.
library-stat-compression = Ahorrado por compresión
storage-compression = Compresión
storage-compression-off = Desactivada
storage-compression-auto = Automático (omitir incomprimibles)
storage-compression-always = Siempre
storage-compression-restart = Se aplica en el próximo inicio
# Phase 49j — tasks & progress center.
footer-tasks = Tareas
tasks-title = Tareas
tasks-empty = Aún no hay tareas
tasks-running = En curso
tasks-recent = Recientes
tasks-cancel = Cancelar
task-state-running = En curso
...
repo-wizard-title = Conectar repositorio
repo-wizard-create-tab = Crear nuevo
repo-wizard-connect-tab = Conectar existente
repo-field-name = Nombre
repo-field-path = Ubicación
repo-field-password = Frase de contraseña (opcional)
repo-action-create = Crear
repo-action-connect = Conectar
repo-action-browse = Examinar…
...
repo-error-not-found = No se encontró ningún repositorio en esta ubicación
repo-error-bad-pass = Frase de contraseña incorrecta
repo-note-no-encryption = La frase de contraseña solo controla el acceso; el cifrado en reposo llegará en una versión posterior
repo-confirm-forget = ¿Quitar "{ $name }" de la lista? Tus datos permanecen en el disco.
repo-toast-created = Repositorio "{ $name }" creado
repo-toast-connected = Conectado a "{ $name }"
repo-toast-pass-changed = Frase de contraseña actualizada
# Phase 49l — Sources dashboard.
library-tab-overview = Resumen
library-source-empty = Aún no hay orígenes
library-source-unknown = (origen sin especificar)
library-source-snapshots = { $n } instantáneas
library-source-latest = Más reciente { $when }
# Phase 49n — verify & repair.
repo-action-verify = Verificar
repo-action-verify-deep = Verificar (leer todos los datos)
repo-action-repair = Reparar…
repo-verify-clean = Verificados { $files } archivos / { $chunks } fragmentos — sin daños
repo-verify-damaged = { $missing } faltantes, { $corrupt } fragmentos dañados
repo-repair-confirm = ¿Eliminar { $n } instantáneas que ya no se pueden restaurar?
repo-repair-removed = { $n } instantáneas dañadas eliminadas
...
eula-intro = Revisa el acuerdo a continuación. Debes aceptarlo antes de usar Freally File Manager.
eula-scroll-hint = Desplázate hasta el final para habilitar "Acepto".
eula-thanks = Gracias por leer.
eula-agree = Acepto
eula-decline = Rechazar y salir
eula-error = No se pudo registrar la aceptación: { $error }

# FFM-M01 — Explorer copy-verb takeover.
settings-intercept-copy-unsupported = La interceptación de copia solo está disponible en Windows.
settings-intercept-copy-needs-menu = Activa primero la integración del menú contextual: el controlador de copia debe estar registrado antes de que la interceptación pueda tomar el control.
settings-revert-copy-handler = Restaurar el controlador de copia de Windows
toast-copy-handler-reverted = Se restauró el controlador de copia de Windows
settings-context-menu-hint = Registra o quita el menú contextual y el controlador de copia de Freally en el sistema (por usuario, sin admin).
paste-chooser-title = Copiar y pegar
paste-chooser-close = Cerrar
paste-chooser-files = { $count } archivo(s) — elige un destino
paste-chooser-system-copy = Copia del sistema
paste-chooser-system-move = Mover del sistema
paste-chooser-system-hint = Transferencia simple y rápida, sin verificación
paste-chooser-freally-copy = Copia de Freally
paste-chooser-freally-move = Mover con Freally
paste-chooser-freally-hint = Transferencia verificada byte a byte
paste-chooser-replace-older = Freally — reemplazar archivos antiguos
paste-chooser-replace-older-hint = Verificada; solo sobrescribe cuando el origen es más nuevo
paste-chooser-more = Más opciones…
toast-system-paste-done = { $items } elemento(s) pegado(s)

# FFM-M02 — transactional undo.
undo-title-copy = ¿Deshacer copia — quitar los archivos copiados?
undo-title-move = ¿Deshacer movimiento — devolver los archivos?
undo-summary = { $ready } de { $total } elemento(s) se pueden deshacer; el resto cambió, desapareció o tiene conflicto.
undo-action-trash = A la papelera
undo-action-move-back = Devolver
undo-status-ready = Listo
undo-status-skip-missing = Falta — omitido
undo-status-skip-changed = Cambiado — omitido
undo-status-conflict = Ruta original ocupada
undo-cancel = Cancelar
undo-confirm = Deshacer { $count } elemento(s)
toast-undo-done = Deshacer terminado: { $done } hechos, { $skipped } omitidos, { $failed } fallidos
toast-undo-nothing = Nada que deshacer
history-undo = Deshacer
history-undo-hint = Revierte este trabajo: los archivos copiados van a la papelera y los movidos vuelven a su ubicación original

# FFM-M03 — trash-aware delete.
menu-trash-source = Eliminar origen a la papelera
trash-confirm = ¿Enviar a la papelera?
{ $path }
toast-trash-done = Movido a la papelera: { $trashed } elemento(s), { $failed } fallidos
settings-safety-confirm-trash = Confirmar antes de eliminar a la papelera
settings-safety-move-to-trash = Enviar archivos de origen movidos a la papelera
settings-safety-move-to-trash-hint = Al mover, envía el origen a la papelera en vez de eliminarlo — un movimiento recuperable.

# FFM-M04/M05 — eject + keep-awake.
menu-eject-destination = Expulsar volumen de destino
toast-eject-done = Volumen expulsado — se puede quitar
toast-eject-failed = No se pudo expulsar: { $error }
settings-power-keep-awake = Mantener el equipo activo mientras se ejecutan trabajos
settings-power-keep-awake-hint = Mantiene un bloqueo del sistema (sin suspensión ni protector de pantalla) mientras haya un trabajo copiando.

# FFM-M06 — content-aware collision policies.
collision-policy-skip-identical-else-overwrite = Sobrescribir solo si el contenido difiere
collision-policy-skip-identical-else-prompt = Omitir si es idéntico; si no, preguntar

# FFM-M07 — failed-file ledger + retry.
history-retry-failed = Reintentar fallidos
history-retry-failed-hint = Volver a copiar solo los archivos que fallaron en este trabajo
history-export-failed = Exportar fallidos
history-export-failed-hint = Guardar la lista de archivos fallidos como CSV / TXT / JSON
toast-retry-failed-none = No hay archivos fallidos para reintentar
toast-retry-failed-queued = { $count } archivo(s) fallido(s) en cola de nuevo
toast-failed-exported = Lista de fallidos exportada

# FFM-M08 — checksum sidecars.
menu-create-checksums = Crear sumas de verificación (SHA-256)
toast-checksums-created = Sumas escritas para { $files } archivo(s)
sidecar-verify-clean-title = Todos los archivos verificados
sidecar-verify-bad-title = La verificación de sumas falló
sidecar-verify-summary = { $ok } OK, { $failed } fallidas, { $missing } faltantes
sidecar-verify-failed = No coincide
sidecar-verify-missing = Falta
sidecar-verify-close = Cerrar

# FFM-M09 — hash inspector + clipboard compare.
footer-hash = Hash
hash-inspector-title = Inspector de hash
hash-inspector-algo-label = Algoritmo
hash-inspector-add-files = Añadir archivos…
hash-inspector-add-folders = Añadir carpetas…
hash-inspector-run = Calcular hash
hash-inspector-hashing = Calculando…
hash-inspector-clear = Limpiar
hash-inspector-close = Cerrar
hash-inspector-empty = Elige archivos o carpetas y calcula su hash.
hash-inspector-compare-label = Comparar con un hash
hash-inspector-compare-placeholder = Pega un hash, una línea de suma de verificación o una fila SFV
hash-inspector-compare-invalid = Eso no parece un hash.
hash-inspector-pair-identical = Los dos archivos son idénticos
hash-inspector-pair-different = Los dos archivos son diferentes
hash-inspector-truncated = Se muestran los primeros { $rows } archivos; el resto de la selección se omitió.

# FFM-M10 — copy-verification certificate export.
history-export-certificate = Certificado
history-export-certificate-hint = Exporta este trabajo como certificado de verificación imprimible en HTML / CSV / JSON
certificate-sign-prompt = ¿Firmar el certificado con una clave privada ed25519? Cancelar lo exporta sin firmar.
certificate-key-picker-title = Elige la clave de firma ed25519 (PKCS#8 PEM)
toast-certificate-exported = Certificado exportado ({ $files } archivo(s))
toast-certificate-exported-signed = Certificado firmado exportado ({ $files } archivo(s))

# FFM-M11 — verify-only re-audit.
footer-reaudit = Reverificar
history-reverify = Reverificar
history-reverify-hint = Recalcula el hash del origen y el destino de este trabajo e informa de las diferencias (no escribe nada)
reaudit-title = Auditoría de solo verificación
reaudit-hint = Recalcula el hash de un par origen/destino e informa de las diferencias: no escribe nada
reaudit-pick-source = Elige la carpeta de origen a reverificar
reaudit-pick-destination = Elige la carpeta de destino con la que comparar
reaudit-clean-title = Sin diferencias: los árboles coinciden
reaudit-drift-title = Se detectaron diferencias
reaudit-summary = { $ok } coinciden, { $differs } difieren, { $missing } faltan, { $extra } sobran, { $errors } ilegibles ({ $algo })
reaudit-status-differs = Difiere
reaudit-status-missing = Falta en el destino
reaudit-status-extra = Solo en el destino
reaudit-status-error = Ilegible
reaudit-close = Cerrar

# FFM-M12 — cross-filesystem filename doctor.
doctor-title = Doctor de nombres
doctor-summary = { $flagged } de { $scanned } nombre(s) que el destino { $target } no admite
doctor-issue-reserved-name = Nombre de dispositivo reservado
doctor-issue-illegal-char = Carácter no permitido
doctor-issue-trailing-dot-space = Punto o espacio final
doctor-issue-component-too-long = Nombre demasiado largo
doctor-issue-path-too-long = Ruta demasiado larga
doctor-issue-case-collision = Colisión por mayúsculas/minúsculas
doctor-no-rename = Sin renombrado seguro
doctor-apply = Renombrar { $count } archivo(s)
doctor-toast-applied = { $renamed } renombrado(s), { $failed } fallido(s)

# FFM-M13 — file-list import as a copy source.
action-import-list = Importar lista…
filelist-title = Importar lista de archivos
filelist-hint = Usa una lista de rutas TXT / CSV / JSON como conjunto de origen del trabajo
filelist-picker-title = Elige una lista de archivos (TXT / CSV / JSON)
filelist-summary = { $found } archivo(s) encontrados, { $missing } no encontrados
filelist-missing-header = { $count } ruta(s) de la lista ya no existen
filelist-preserve-structure = Conservar la estructura de carpetas respecto a una raíz
filelist-relative-root-placeholder = Raíz relativa
filelist-toast-queued = { $files } archivo(s) de la lista en cola
filelist-toast-empty = Esa lista no contenía rutas

# FFM-M14 — preflight / dry-run report export.
preflight-report-action = Exportar plan
preflight-report-hint = Guarda esta simulación como HTML / CSV / JSON antes de copiar nada
preflight-report-toast = Plan exportado: { $files } archivo(s), { $findings } hallazgo(s) de nombres

# FFM-M15 — damaged-media salvage.
footer-salvage = Rescatar
salvage-title = Rescatar de un soporte dañado
salvage-hint = Recupera todo lo legible de un archivo defectuoso, mapea lo ilegible y reintenta solo los huecos la próxima vez
salvage-pick-source = Elige el archivo a rescatar
salvage-pick-destination = Dónde escribir la copia rescatada
salvage-toast-complete = Rescate completo: { $bytes } byte(s), sin huecos
salvage-toast-gaps = Rescatados { $bytes } byte(s); { $missing } byte(s) ilegibles en { $gaps } hueco(s)

# FFM-M16 — whole-job elevated batch with a consent ledger.
history-elevate-batch = Elevar todo
history-elevate-batch-hint = Reintenta con un solo consentimiento todas las rutas de este trabajo con permiso denegado
elevate-batch-none = No hay rutas con permiso denegado que elevar
elevate-batch-confirm = ¿Conceder una elevación para { $count } ruta(s)? ({ $skipped } fallo(s) más no se solucionan elevando.) El asistente copiará estas rutas:
elevate-batch-toast = Lote elevado: { $copied } copiadas, { $failed } fallidas

# ===== Build 3 (FFM-M17..M24) =====

# FFM-M23
err-source-changed = El archivo de origen cambió mientras se leía
job-source-changed = El origen cambió durante la copia
settings-source-stability-label = Si un archivo de origen cambia durante la copia
settings-source-stability-description = Vuelve a comprobar el tamaño y la fecha de cada origen tras leerlo. La verificación por sí sola no puede detectarlo.
settings-source-stability-off = No comprobar
settings-source-stability-warn = Avisar y marcar el archivo
settings-source-stability-recopy = Copiar el archivo de nuevo
settings-source-stability-fail = Marcar el archivo como fallido

# FFM-M17
err-schedule-id-invalid = Identificador de programación no válido
err-schedule-label-invalid = Nombre de programación no válido
err-schedule-verb-invalid = Una programación solo puede copiar o mover
err-schedule-trigger-invalid = Hora de programación no válida
err-schedule-too-many = Demasiadas ejecuciones programadas
...
settings-schedule-trigger-hourly = Cada hora
settings-schedule-trigger-daily = Cada día
settings-schedule-trigger-weekly = Cada semana
settings-schedule-next-run = Próxima ejecución
settings-schedule-missed = Ejecutar cuanto antes una programación perdida
settings-schedule-missed-unsupported = Este sistema siempre ejecuta una programación perdida al reanudarse.
settings-schedule-not-installed = No está en el programador del sistema

# FFM-M18
err-affinity-too-many = Demasiados grupos de colas de unidad
err-affinity-name-invalid = Nombre de grupo no válido
err-affinity-name-duplicate = Otro grupo ya usa este nombre
err-affinity-prefixes-invalid = Un grupo necesita al menos una carpeta
err-affinity-workers-invalid = Número de procesos no válido
settings-affinity-title = Colas de unidad
settings-affinity-description = Decide qué destinos comparten una cola de transferencia cuando la detección de unidades se equivoca.
settings-affinity-add = Añadir grupo
settings-affinity-workers = Procesos
settings-affinity-workers-auto = Automático
...
# FFM-M20
err-favorite-label-invalid = Nombre de favorito no válido
err-favorite-kind-invalid = Tipo de favorito no válido
err-favorite-hotkey-invalid = Atajo no válido
err-favorite-too-many = Demasiados favoritos
settings-favorites-title = Favoritos
settings-favorites-empty = Todavía no hay favoritos.
settings-favorites-add = Añadir favorito
favorites-recent-title = Pares recientes
favorites-kind-source = Origen
favorites-kind-destination = Destino
favorites-kind-pair = Par
...
err-autostart-failed = No se pudo cambiar el inicio al iniciar sesión
settings-autostart-label = Iniciar Freally al iniciar sesión
settings-autostart-description = Se inicia minimizado en la bandeja para que la integración con el shell, los atajos, los monitores y las programaciones estén activos desde el inicio de sesión.
settings-autostart-unavailable-portable = No disponible en modo portátil: una entrada de inicio apuntaría a esta unidad extraíble.
settings-autostart-unavailable-platform = El inicio al iniciar sesión no está disponible en este sistema.
...
settings-tab-queues = Colas de unidad
settings-tab-favorites = Favoritos
settings-schedule-name = Nombre
settings-schedule-verb = Acción
settings-schedule-verb-copy = Copiar
settings-schedule-verb-move = Mover
settings-schedule-manifest = Lista de archivos
settings-schedule-destination = Destino
settings-schedule-when = Frecuencia
settings-schedule-weekday = Día de la semana (0 = domingo)
settings-schedule-hour = Hora
settings-schedule-minute = Minuto
settings-affinity-name = Nombre del grupo
settings-affinity-paths = Carpetas
settings-affinity-path-add = Añadir carpeta…
settings-favorites-name = Nombre
settings-favorites-kind = Tipo
settings-favorites-path = Carpeta
settings-favorites-destination = Destino
settings-favorites-hotkey = Atajo

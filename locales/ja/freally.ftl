...
toast-clipboard-files-detected = クリップボードにファイルがあります — 貼り付けショートカットを押すと Freally File Manager でコピーできます
...
settings-paste-shortcut-hint = この組み合わせをシステム上のどこでも押すと、Explorer / Finder / Files でコピーしたファイルを Freally File Manager で貼り付けられます。CmdOrCtrl は macOS では Cmd、Windows / Linux では Ctrl に対応します。
settings-clipboard-watcher = コピーされたファイルをクリップボードで監視
settings-clipboard-watcher-hint = ファイルの URL がクリップボードに表示されたときにトーストを表示し、Freally File Manager で貼り付けられることを知らせます。有効な間は 500 ミリ秒ごとにポーリングします。
...
settings-intercept-copy-hint = 有効にすると、Explorer の Ctrl+C / Ctrl+V が Freally File Manager を経由します。登録は Phase 14 で実装されます。
...
settings-updater-hint = Freally File Manager は署名済みの更新を 1 日に最大 1 回確認します。更新は次回のアプリ終了時にインストールされます。
...
settings-on-locked-hint = 「他のプロセスがファイルを使用中」エラーを解消します。Freally File Manager がソースボリュームのスナップショットを作成し（Windows では VSS、Linux では ZFS/Btrfs、macOS では APFS）、スナップショットのコピーから読み取ります。
snapshot-prompt-title = このファイルは他のプロセスが使用中です
snapshot-prompt-body = 別のプログラムが { $path } を排他書き込みで開いています。Freally File Manager がこのファイルおよび同じボリューム上の同様のファイルをどう扱うかを選択してください。
snapshot-source-active = 📷 { $volume } の { $kind } スナップショットから読み取り中
snapshot-create-failed = ソースボリュームのスナップショットを作成できませんでした
snapshot-vss-needs-elevation = VSS スナップショットからの読み取りには管理者権限が必要です。Freally File Manager が許可を求めます。
...
resume-prompt-body = Freally File Manager は前回のセッションから { $count } 件の未完了の転送を検出しました。それぞれの処理を選択してください。
...
dropstack-show-tray-icon = Freally File Manager のトレイアイコンを表示
...
settings-mount-on-launch-path-placeholder = 例: C:\Mounts\freally
...
settings-audit-file-path-placeholder = 例: C:\ProgramData\Freally\audit.log
...
cli-help-tagline = Freally File Manager CLI — CI/CD パイプライン向けのバイト単位で正確なファイルコピー、同期、検証、監査。
...
cli-info-shape-recorded = 帯域シェイプ "{ $rate }" を記録しました。適用は freally-shape を通じて行われます
...
settings-mobile-pair-active = ペアサーバーが稼働中 — Freally File Manager モバイルアプリで QR をスキャンしてください
...
settings-dedup-hint = ソースとコピー先が同じボリュームを共有している場合、Freally File Manager はバイトをコピーする代わりにファイルシステムレベルでファイルをクローンできます。Reflink は即時かつ安全です。ハードリンクはより高速ですが、両方の名前が状態を共有します。
...
plugin-hint = サンドボックス化された WASM プラグインがカスタムフックで Freally File Manager を拡張します。各プラグインは呼び出しごとの CPU とメモリの制限下で動作し、付与したホスト機能のみを参照できます。
...
server-hint = Freally File Manager をヘッドレスのファイルサーバーとして実行します。公開するプロトコルを選び、アドレスと提供するフォルダーを設定し、必要に応じて認証を要求します。
...
eula-title = エンドユーザーライセンス契約
eula-version = バージョン { $version }
eula-intro = 以下の契約をご確認ください。Freally File Manager を使用する前に同意が必要です。
eula-scroll-hint = 最後までスクロールすると「同意する」が有効になります。
eula-thanks = お読みいただきありがとうございます。
eula-agree = 同意する
eula-decline = 同意せず終了
eula-error = 同意を保存できませんでした: { $error }

# FFM-M01 — Explorer copy-verb takeover.
settings-intercept-copy-unsupported = コピーの横取りは Windows でのみ利用できます。
settings-intercept-copy-needs-menu = 先にコンテキストメニュー統合を有効にしてください。横取りが引き継ぐ前にコピーハンドラーの登録が必要です。
settings-revert-copy-handler = Windows のコピーハンドラーに戻す
toast-copy-handler-reverted = Windows のコピーハンドラーに戻しました
settings-context-menu-hint = Freally の右クリックメニューとコピーハンドラーを OS に登録／削除します（ユーザーごと、管理者不要）。
paste-chooser-title = コピー＆ペースト
paste-chooser-close = 閉じる
paste-chooser-files = { $count } 個のファイル — 貼り付け先を選択
paste-chooser-system-copy = システムコピー
paste-chooser-system-move = システム移動
paste-chooser-system-hint = 高速なそのままの転送、検証なし
paste-chooser-freally-copy = Freally コピー
paste-chooser-freally-move = Freally 移動
paste-chooser-freally-hint = 検証付きのバイト単位で正確な転送
paste-chooser-replace-older = Freally — 古いファイルを置き換え
paste-chooser-replace-older-hint = 検証付き。ソースが新しい場合のみ上書きします
paste-chooser-more = その他のオプション…
toast-system-paste-done = { $items } 個の項目を貼り付けました

# FFM-M02 — transactional undo.
undo-title-copy = コピーを元に戻す — コピーしたファイルを削除しますか？
undo-title-move = 移動を元に戻す — ファイルを戻しますか？
undo-summary = { $total } 件中 { $ready } 件を元に戻せます。残りは変更・消失・競合しています。
undo-action-trash = ゴミ箱へ
undo-action-move-back = 元に戻す
undo-status-ready = 準備完了
undo-status-skip-missing = 見つからない — スキップ
undo-status-skip-changed = 変更あり — スキップ
undo-status-conflict = 元のパスが使用中
undo-cancel = キャンセル
undo-confirm = { $count } 件を元に戻す
toast-undo-done = 元に戻す完了: { $done } 件完了、{ $skipped } 件スキップ、{ $failed } 件失敗
toast-undo-nothing = 元に戻すものはありません
history-undo = 元に戻す
history-undo-hint = このジョブを逆転します。コピーされたファイルはゴミ箱へ、移動されたファイルは元の場所に戻ります

# FFM-M03 — trash-aware delete.
menu-trash-source = 元をゴミ箱へ削除
trash-confirm = ゴミ箱に送りますか？
{ $path }
toast-trash-done = ゴミ箱に移動: { $trashed } 件、{ $failed } 件失敗
settings-safety-confirm-trash = ゴミ箱へ削除する前に確認
settings-safety-move-to-trash = 移動元のファイルをゴミ箱へ送る
settings-safety-move-to-trash-hint = 移動時に、元を削除せずゴミ箱へ送ります — 復元可能な移動。

# FFM-M04/M05 — eject + keep-awake.
menu-eject-destination = 宛先ボリュームを取り出す
toast-eject-done = ボリュームを取り出しました — 安全に取り外せます
toast-eject-failed = 取り出せませんでした: { $error }
settings-power-keep-awake = ジョブ実行中はコンピューターをスリープさせない
settings-power-keep-awake-hint = ジョブがコピー中の間、システムのウェイクロックを保持します（スリープなし、スクリーンセーバーなし）。

# FFM-M06 — content-aware collision policies.
collision-policy-skip-identical-else-overwrite = 内容が異なる場合のみ上書き
collision-policy-skip-identical-else-prompt = 同一ならスキップ、それ以外は確認

# FFM-M07 — failed-file ledger + retry.
history-retry-failed = 失敗を再試行
history-retry-failed-hint = このジョブで失敗したファイルだけを再コピー
history-export-failed = 失敗をエクスポート
history-export-failed-hint = 失敗ファイル一覧を CSV / TXT / JSON で保存
toast-retry-failed-none = 再試行する失敗ファイルはありません
toast-retry-failed-queued = { $count } 件の失敗ファイルを再キュー
toast-failed-exported = 失敗一覧をエクスポートしました

# FFM-M08 — checksum sidecars.
menu-create-checksums = チェックサムを作成 (SHA-256)
toast-checksums-created = { $files } 件のファイルのチェックサムを書き込みました
sidecar-verify-clean-title = すべてのファイルを検証しました
sidecar-verify-bad-title = チェックサムの検証に失敗しました
sidecar-verify-summary = OK { $ok } 件、失敗 { $failed } 件、欠落 { $missing } 件

...
settings-mount-on-launch-path-placeholder = 例如 C:\Mounts\freally
...
settings-audit-file-path-placeholder = 例如 C:\ProgramData\Freally\audit.log
...
cli-info-shape-recorded = 带宽整形“{ $rate }”已记录；强制执行通过 freally-shape 接入
...
# 库抽屉（阶段 49）— 内容寻址仓库的统一视图。
footer-library = 库
library-title = 库
library-loading = 正在加载仓库…
library-unavailable = 仓库不可用
library-tab-live = 当前
library-tab-snapshots = 快照
library-tab-versions = 版本
library-hero-savings = 提供 { $effective } 有效数据 · 已节省 { $pct }
library-hero-empty = 已存储 { $chunks } 个块 — 暂无快照
library-stat-stored = 磁盘占用
library-stat-effective = 有效数据
library-stat-snapshots = 快照
library-stat-chunks = 唯一块
library-snapshot-empty = 暂无快照
library-snapshot-files = { $n } 个文件
library-version-path-ph = 目标路径…
library-version-load = 显示版本
library-version-empty = 此路径没有版本
repo-kind-copy = 复制
repo-kind-sync = 同步
...
server-root = 提供的文件夹
server-readonly = 只读（拒绝上传和删除）
server-auth-mode = 身份验证
server-auth-none = 无
server-auth-bearer = Bearer 令牌
server-auth-basic = 基本（用户名 + 密码）
server-auth-token = 令牌
server-auth-user = 用户名
server-auth-password = 密码
otel-endpoint = OpenTelemetry 端点
webhook-section = Webhook
webhook-url = Webhook URL
webhook-add = 添加 Webhook
webhook-remove = 移除
webhook-empty = 尚未配置 Webhook。
webhook-pushover-token = Pushover 令牌
webhook-pushover-user = Pushover 用户
server-start = 启动服务器
server-stop = 停止服务器
server-status-running = 正在 { $addr } 上运行

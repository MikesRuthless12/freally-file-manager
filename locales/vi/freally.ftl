...
toast-clipboard-files-detected = Có tệp trong bộ nhớ tạm — nhấn phím dán để sao chép qua Freally File Manager
...
settings-paste-shortcut-hint = Nhấn tổ hợp này ở bất kỳ đâu trên hệ thống để dán các tệp đã sao chép từ Explorer / Finder / Files qua Freally File Manager. CmdOrCtrl tương ứng với Cmd trên macOS, Ctrl trên Windows / Linux.
settings-clipboard-watcher = Theo dõi bộ nhớ tạm cho các tệp đã sao chép
settings-clipboard-watcher-hint = Hiển thị thông báo khi có URL tệp xuất hiện trong bộ nhớ tạm, gợi ý rằng bạn có thể dán qua Freally File Manager. Kiểm tra mỗi 500 ms khi bật.
...
settings-intercept-copy-hint = Khi bật, Ctrl+C / Ctrl+V của Explorer sẽ đi qua Freally File Manager. Việc đăng ký sẽ có ở Phase 14.
...
settings-updater-hint = Freally File Manager kiểm tra bản cập nhật đã ký tối đa một lần mỗi ngày. Cập nhật cài đặt vào lần thoát ứng dụng tiếp theo.
...
settings-on-locked-hint = Loại bỏ lỗi "tệp đang được tiến trình khác sử dụng". Freally File Manager chụp ảnh ổ nguồn (VSS trên Windows, ZFS/Btrfs trên Linux, APFS trên macOS) và đọc từ bản ảnh chụp.
snapshot-prompt-title = Tệp này đang được một tiến trình khác sử dụng
snapshot-prompt-body = Một chương trình khác đang mở { $path } để ghi độc quyền. Hãy chọn cách Freally File Manager xử lý tệp này và các tệp tương tự trên cùng ổ đĩa.
snapshot-source-active = 📷 Đang đọc từ ảnh chụp { $kind } của { $volume }
snapshot-create-failed = Không thể tạo ảnh chụp của ổ nguồn
snapshot-vss-needs-elevation = Đọc từ ảnh chụp VSS cần quyền Administrator. Freally File Manager sẽ xin phép bạn cho phép.
...
resume-prompt-body = Freally File Manager phát hiện { $count } lượt chuyển chưa hoàn tất từ phiên trước. Hãy chọn việc cần làm với từng lượt.
...
dropstack-show-tray-icon = Hiển thị biểu tượng khay Freally File Manager
...
settings-mount-on-launch-path-placeholder = ví dụ C:\Mounts\freally
...
settings-audit-file-path-placeholder = ví dụ C:\ProgramData\Freally\audit.log
...
cli-help-tagline = Freally File Manager CLI — sao chép, đồng bộ, xác minh và kiểm toán tệp chính xác từng byte cho pipeline CI/CD.
...
cli-info-shape-recorded = Đã ghi nhận hình băng thông "{ $rate }"; việc thực thi được nối qua freally-shape
...
settings-mobile-pair-active = Máy chủ ghép nối đang chạy — quét mã QR bằng ứng dụng di động Freally File Manager
...
settings-dedup-hint = Khi nguồn và đích dùng chung một ổ đĩa, Freally File Manager có thể nhân bản tệp ở cấp hệ thống tệp thay vì sao chép từng byte. Reflink tức thì + an toàn; hardlink nhanh hơn nhưng cả hai tên dùng chung trạng thái.
...
plugin-hint = Các plugin WASM trong hộp cát mở rộng Freally File Manager bằng các hook tùy chỉnh. Mỗi plugin chạy dưới giới hạn CPU và bộ nhớ cho từng lần gọi và chỉ thấy các năng lực máy chủ mà bạn cấp cho nó.
...
settings-tab-server = Máy chủ
server-hint = Chạy Freally File Manager như một máy chủ tệp không giao diện. Chọn các giao thức cần công khai, đặt địa chỉ và thư mục cần phục vụ, và tùy chọn yêu cầu xác thực.
server-protocols = Giao thức
server-bind-addr = Địa chỉ liên kết
server-root = Thư mục phục vụ
server-readonly = Chỉ đọc (từ chối tải lên và xóa)
server-auth-mode = Xác thực
server-auth-none = Không
server-auth-bearer = Token Bearer
server-auth-basic = Cơ bản (tên người dùng + mật khẩu)
server-auth-token = Token
server-auth-user = Tên người dùng
server-auth-password = Mật khẩu
otel-endpoint = Điểm cuối OpenTelemetry
webhook-section = Webhook
webhook-url = URL webhook
webhook-add = Thêm webhook
webhook-remove = Xóa
webhook-empty = Chưa cấu hình webhook nào.
webhook-pushover-token = Token Pushover
webhook-pushover-user = Người dùng Pushover
server-start = Khởi động máy chủ
server-stop = Dừng máy chủ
server-status-running = Đang chạy trên { $addr }
server-status-stopped = Đã dừng

...
toast-clipboard-files-detected = Fichiers dans le presse-papiers — appuyez sur votre raccourci coller pour copier via Freally File Manager
...
# MT
settings-tab-bugreport = Signaler un bug

# Phase 51 — end-to-end encrypted collaboration
# MT
settings-tab-collab = Collaboration
collab-intro = Les fichiers partagés sont chiffrés pour que seules les personnes listées ici puissent les ouvrir. Chacune garde sa propre clé ; il n’y a pas de mot de passe commun.
collab-forward-only = Retirer quelqu’un change qui pourra ouvrir les fichiers chiffrés ensuite. Cela ne reprend pas ceux qu’il pouvait déjà lire.
collab-members = Personnes
collab-none = Personne n’a encore été ajouté.
collab-label = Nom
collab-recipient = Sa clé publique
collab-add = Ajouter
collab-remove = Retirer
collab-generate = Générer une clé pour quelqu’un
collab-identity-once = Copiez-la maintenant — affichée une seule fois, c’est le seul moyen d’ouvrir les fichiers partagés :
collab-sas-label = Code de vérification
collab-sas-hint = Lisez ce code à voix haute. S’ils correspondent, vous détenez chacun la vraie clé de l’autre.

# Phase 53 — optional decoded video frames (user-supplied ffmpeg)
# MT
merge-heading = Aperçus vidéo
merge-ffmpeg-hint = Freally peut afficher des images d’une vidéo lors d’une comparaison. Cela nécessite ffmpeg, qui n’est pas fourni — installez-le et Freally l’utilisera. Tout le reste fonctionne sans.
merge-ffmpeg-enable = Utiliser ffmpeg pour les images vidéo
merge-ffmpeg-path = Chemin de ffmpeg (vide pour chercher dans PATH)
merge-ffmpeg-found = Trouvé
merge-ffmpeg-missing = ffmpeg est introuvable. La comparaison vidéo fonctionne toujours : elle affiche la durée, la taille et les pistes au lieu d’images.
merge-ffmpeg-disabled = Les images vidéo sont désactivées. Activez ffmpeg dans les Réglages pour comparer les images.

collab-revoked = Retirées
collab-no-roster = Il n'y a pas encore de liste de personnes. Ajoutez quelqu'un avant de partager des fichiers.
collab-admin-key-corrupt = La clé d'administrateur enregistrée n'a pas pu être lue. Elle est peut-être endommagée.

bugreport-intro = Les rapports sont anonymes et ne sont jamais envoyés automatiquement. Vous choisissez le contenu et cliquez sur Envoyer — nous n’hébergeons aucun serveur.
bugreport-pending = Un rapport de plantage d’une exécution précédente est en attente.
bugreport-description-label = Que s’est-il passé ?
bugreport-include-crash = Inclure le rapport de plantage
bugreport-preview-label = Afficher exactement ce qui sera envoyé
bugreport-send-github = Ouvrir un ticket GitHub
bugreport-send-gmail = Ouvrir Gmail
bugreport-send-email = Ouvrir mon client de messagerie
bugreport-dismiss-crash = Supprimer le rapport de plantage
bugreport-simulate = Simuler un rapport de plantage
...
settings-paste-shortcut-hint = Appuyez sur cette combinaison n'importe où sur votre système pour coller des fichiers copiés depuis Explorer / Finder / Files via Freally File Manager. CmdOrCtrl correspond à Cmd sur macOS, Ctrl sur Windows / Linux.
settings-clipboard-watcher = Surveiller le presse-papiers pour les fichiers copiés
settings-clipboard-watcher-hint = Affiche une notification lorsque des URL de fichiers apparaissent dans le presse-papiers, suggérant de les coller via Freally File Manager. Interrogation toutes les 500 ms lorsque l'option est activée.
...
settings-intercept-copy-hint = Lorsque cette option est activée, Ctrl+C / Ctrl+V d'Explorer passe par Freally File Manager. L'enregistrement arrive en Phase 14.
...
settings-updater-hint = Freally File Manager recherche des mises à jour signées au plus une fois par jour. Les mises à jour s'installent au prochain arrêt de l'application.
...
settings-on-locked-hint = Élimine les erreurs « fichier utilisé par un autre processus ». Freally File Manager crée un instantané du volume source (VSS sous Windows, ZFS/Btrfs sous Linux, APFS sous macOS) et lit à partir de la copie de l'instantané.
snapshot-prompt-title = Ce fichier est utilisé par un autre processus
snapshot-prompt-body = Un autre programme a ouvert { $path } en écriture exclusive. Choisissez comment Freally File Manager doit traiter ce fichier et les fichiers similaires sur le même volume.
...
snapshot-vss-needs-elevation = La lecture depuis un instantané VSS nécessite des droits d'administrateur. Freally File Manager vous demandera l'autorisation.
...
resume-prompt-body = Freally File Manager a détecté { $count } transfert(s) inachevé(s) d'une session précédente. Choisissez quoi faire de chacun.
...
dropstack-show-tray-icon = Afficher l'icône Freally File Manager dans la barre d'état
...
# MT
power-badge-paused = En pause
power-badge-capped = Limité
power-badge-tooltip = Une politique d’alimentation limite les transferts. Ouvrez Paramètres → Alimentation.
...
cli-help-tagline = Freally File Manager CLI — copie, synchronisation, vérification et audit de fichiers octet par octet pour les pipelines CI/CD.
...
cli-info-shape-recorded = Limite de bande passante « { $rate } » enregistrée ; l'application est gérée via freally-shape
...
settings-mobile-pair-active = Serveur d'association actif — scannez le QR avec l'application mobile Freally File Manager
...
settings-dedup-hint = Lorsque la source et la destination partagent un volume, Freally File Manager peut cloner les fichiers au niveau du système de fichiers au lieu de copier les octets. Le reflink est instantané et sûr ; le lien physique est plus rapide mais les deux noms partagent leur état.
...
plugin-hint = Des plugins WASM en bac à sable étendent Freally File Manager avec des hooks personnalisés. Chaque plugin s'exécute sous des limites de processeur et de mémoire par appel et ne voit que les capacités hôtes que vous lui accordez.
...
server-hint = Exécutez Freally File Manager comme un serveur de fichiers sans interface. Choisissez les protocoles à exposer, définissez l'adresse et le dossier à servir, et exigez éventuellement une authentification.
...
library-tab-compare = Comparer
repo-change-added = Ajouté
repo-change-removed = Supprimé
repo-change-modified = Modifié
repo-change-unchanged = Aucune modification
repo-diff-summary = { $added } ajoutés · { $removed } supprimés · { $modified } modifiés
repo-diff-bytes-added = { $bytes } nouveaux
repo-diff-pick-two = Choisissez deux instantanés à comparer
# Phase 49r — statistics / reports.
library-tab-reports = Rapports
report-growth-title = Croissance du stockage
report-by-kind-title = Par type
report-top-files-title = Fichiers principaux
report-dedup-ratio = { $pct }% dédupliqué
report-export = Exporter le rapport
report-exported = Rapport enregistré dans { $path }
report-file-versions = { $n } versions
# Phase 49p — pinning / prune.
repo-pin = Épingler
repo-unpin = Désépingler
repo-pinned-badge = Épinglé
repo-prune-title = Élaguer
repo-prune-keep-last = Conserver les plus récents
repo-prune-removed = { $n } instantanés élagués
repo-prune-none = Rien à élaguer
...
backup-retention-keep-all = Tout conserver
backup-retention-last = Conserver les { $n } derniers
backup-retention-days = Plus de { $days } jours
backup-retention-gfs = Rotation GFS
backup-prune-now = Élaguer maintenant
backup-prune-none = Rien à élaguer
backup-prune-result = { $removed } instantanés supprimés · { $bytes } récupérés
# Phase 49f — per-source scheduling.
backup-schedule = Planification
backup-schedule-manual = Manuel
backup-schedule-hourly = Toutes les heures
backup-schedule-daily = Tous les jours
backup-schedule-weekly = Toutes les semaines
backup-next-run = Prochaine exécution { $when }
backup-not-scheduled = Non planifié
# Phase 49g — source filters.
backup-include-ph = Motifs d'inclusion (séparés par des virgules)
backup-skip-hidden = Ignorer les cachés
# Phase 49q — notifications.
notify-title = Notifications
notify-on-success = En cas de succès
notify-on-failure = En cas d'échec
notify-test = Envoyer un test
notify-test-sent = Test envoyé à { $n } destination(s)
...
# MT
snapshot-field-label = Nom
snapshot-field-description = Description
snapshot-field-tags = Étiquettes (séparées par des virgules)
...
library-compact = Compactage complet
library-compact-started = Compactage démarré — voir Tâches
# Phase 49h — compression.
library-stat-compression = Économisé par compression
storage-compression = Compression
storage-compression-off = Désactivée
storage-compression-auto = Auto (ignorer les incompressibles)
storage-compression-always = Toujours
storage-compression-restart = S'applique au prochain lancement
# Phase 49j — tasks & progress center.
footer-tasks = Tâches
tasks-title = Tâches
tasks-empty = Aucune tâche pour l'instant
tasks-running = En cours
tasks-recent = Récentes
tasks-cancel = Annuler
task-state-running = En cours
task-state-completed = Terminé
task-state-failed = Échec
task-state-cancelled = Annulé
# Phase 49k — repository setup/connect wizard.
repo-wizard-title = Connecter un dépôt
repo-wizard-create-tab = Créer un dépôt
repo-wizard-connect-tab = Connecter un dépôt existant
repo-field-name = Nom
repo-field-path = Emplacement
repo-field-password = Phrase secrète (facultative)
repo-action-create = Créer
repo-action-connect = Connecter
repo-action-browse = Parcourir…
repo-switcher-label = Dépôt
repo-action-forget = Oublier
repo-action-change-pass = Changer la phrase secrète…

# Phase 50i — repository access key slots
# MT
repo-keys-title = Clés d’accès
repo-keys-empty = Ce dépôt n’a pas de verrou par phrase secrète.
repo-key-label = Libellé
repo-key-password = Nouveau mot de passe
repo-key-auth = Mot de passe existant
repo-action-add-key = Ajouter une clé
repo-action-remove-key = Révoquer
repo-action-recovery = Générer une clé de récupération

# Phase 50h — repository replication (3-2-1)
# MT
repo-action-replicate = Répliquer vers…

# Phase 50g — per-repository pack storage backend
# MT
repo-remote-label = Stockage des paquets
repo-remote-local = Disque local
repo-remote-restart = Les changements s’appliquent au prochain démarrage.
repo-remote-unknown = Ce dépôt distant n’est pas configuré.
repo-remote-not-empty = Ce dépôt contient déjà des données. Le stockage des paquets ne peut être choisi que tant qu'il est vide — utilisez plutôt Répliquer vers un nouveau dépôt.

repo-replicate-done = Répliqué : { $copied } instantané(s) copiés, { $skipped } déjà présents
repo-replicate-same-path = La destination est le même dépôt que la source.

repo-recovery-once = Conservez-la maintenant — affichée une seule fois :

repo-password-old = Phrase secrète actuelle
repo-password-new = Nouvelle phrase secrète
repo-error-exists = Un dépôt existe déjà à cet emplacement
repo-error-not-found = Aucun dépôt trouvé à cet emplacement
repo-error-bad-pass = Phrase secrète incorrecte
repo-note-no-encryption = La phrase secrète ne contrôle que l'accès ; le chiffrement au repos arrivera dans une version ultérieure
repo-confirm-forget = Retirer "{ $name }" de la liste ? Vos données restent sur le disque.
repo-toast-created = Dépôt "{ $name }" créé
repo-toast-connected = Connecté à "{ $name }"
repo-toast-pass-changed = Phrase secrète mise à jour
# Phase 49l — Sources dashboard.
library-tab-overview = Vue d'ensemble
library-source-empty = Aucune source pour l'instant
library-source-unknown = (source non spécifiée)
library-source-snapshots = { $n } instantanés
library-source-latest = Dernier { $when }
# Phase 49n — verify & repair.
repo-action-verify = Vérifier
repo-action-verify-deep = Vérifier (lire toutes les données)
repo-action-repair = Réparer…
repo-verify-clean = { $files } fichiers / { $chunks } blocs vérifiés — aucun dommage
repo-verify-damaged = { $missing } manquants, { $corrupt } blocs corrompus
repo-repair-confirm = Supprimer { $n } instantanés qui ne peuvent plus être restaurés ?
repo-repair-removed = { $n } instantanés endommagés supprimés
repo-repair-none = Rien à réparer — le dépôt est sain
...
eula-intro = Veuillez lire le contrat ci-dessous. Vous devez l'accepter avant d'utiliser Freally File Manager.
eula-scroll-hint = Faites défiler jusqu'à la fin pour activer « J'accepte ».
eula-thanks = Merci de votre lecture.
eula-agree = J'accepte
eula-decline = Refuser et quitter
eula-error = Impossible d'enregistrer l'acceptation : { $error }

# FFM-M01 — Explorer copy-verb takeover.
settings-intercept-copy-unsupported = L'interception de copie n'est disponible que sous Windows.
settings-intercept-copy-needs-menu = Activez d'abord l'intégration au menu contextuel : le gestionnaire de copie doit être enregistré avant que l'interception puisse prendre le relais.
settings-revert-copy-handler = Rétablir le gestionnaire de copie de Windows
toast-copy-handler-reverted = Gestionnaire de copie de Windows rétabli
settings-context-menu-hint = Enregistre ou supprime le menu contextuel et le gestionnaire de copie de Freally dans le système (par utilisateur, sans admin).
paste-chooser-title = Copier-coller
paste-chooser-close = Fermer
paste-chooser-files = { $count } fichier(s) — choisissez une destination
paste-chooser-system-copy = Copie système
paste-chooser-system-move = Déplacement système
paste-chooser-system-hint = Transfert simple et rapide, sans vérification
paste-chooser-freally-copy = Copie Freally
paste-chooser-freally-move = Déplacement Freally
paste-chooser-freally-hint = Transfert vérifié octet par octet
paste-chooser-replace-older = Freally — remplacer les fichiers plus anciens
paste-chooser-replace-older-hint = Vérifié ; n'écrase que si la source est plus récente
paste-chooser-more = Plus d'options…
toast-system-paste-done = { $items } élément(s) collé(s)

# FFM-M02 — transactional undo.
undo-title-copy = Annuler la copie — supprimer les fichiers copiés ?
undo-title-move = Annuler le déplacement — remettre les fichiers ?
undo-summary = { $ready } élément(s) sur { $total } peuvent être annulés ; le reste a changé, disparu ou est en conflit.
undo-action-trash = Vers la corbeille
undo-action-move-back = Remettre
undo-status-ready = Prêt
undo-status-skip-missing = Manquant — ignoré
undo-status-skip-changed = Modifié — ignoré
undo-status-conflict = Chemin d'origine occupé
undo-cancel = Annuler
undo-confirm = Annuler { $count } élément(s)
toast-undo-done = Annulation terminée : { $done } faits, { $skipped } ignorés, { $failed } échoués
toast-undo-nothing = Rien à annuler
history-undo = Annuler
history-undo-hint = Inverse ce travail : les fichiers copiés vont à la corbeille, les fichiers déplacés retournent à leur emplacement d'origine

# FFM-M03 — trash-aware delete.
menu-trash-source = Supprimer la source vers la corbeille
trash-confirm = Envoyer à la corbeille ?
{ $path }
toast-trash-done = Déplacé vers la corbeille : { $trashed } élément(s), { $failed } échoués
settings-safety-confirm-trash = Confirmer avant de supprimer vers la corbeille
settings-safety-move-to-trash = Envoyer les fichiers source déplacés à la corbeille
settings-safety-move-to-trash-hint = Lors d'un déplacement, envoie la source à la corbeille au lieu de la supprimer — un déplacement récupérable.

# FFM-M04/M05 — eject + keep-awake.
menu-eject-destination = Éjecter le volume de destination
toast-eject-done = Volume éjecté — retrait sûr
toast-eject-failed = Éjection impossible : { $error }
settings-power-keep-awake = Garder l'ordinateur éveillé pendant les tâches
settings-power-keep-awake-hint = Maintient un verrou système (pas de veille ni d'écran de veille) tant qu'une tâche copie.

# FFM-M06 — content-aware collision policies.
collision-policy-skip-identical-else-overwrite = Écraser seulement si le contenu diffère
collision-policy-skip-identical-else-prompt = Ignorer si identique, sinon demander

# FFM-M07 — failed-file ledger + retry.
history-retry-failed = Réessayer les échecs
history-retry-failed-hint = Recopier uniquement les fichiers en échec de cette tâche
history-export-failed = Exporter les échecs
history-export-failed-hint = Enregistrer la liste des fichiers en échec en CSV / TXT / JSON
toast-retry-failed-none = Aucun fichier en échec à réessayer
toast-retry-failed-queued = { $count } fichier(s) en échec remis en file
toast-failed-exported = Liste des échecs exportée

# FFM-M08 — checksum sidecars.
menu-create-checksums = Créer des sommes de contrôle (SHA-256)
toast-checksums-created = Sommes écrites pour { $files } fichier(s)
sidecar-verify-clean-title = Tous les fichiers vérifiés
sidecar-verify-bad-title = Échec de la vérification des sommes
sidecar-verify-summary = { $ok } OK, { $failed } échouées, { $missing } manquantes
...
settings-portable-keychain-warning = Les identifiants cloud sont conservés dans un fichier chiffré sur le lecteur amovible, déverrouillé par une phrase secrète que vous choisissez — ils ne sont pas écrits sur cet ordinateur. Si vous l’oubliez, les identifiants enregistrés sont irrécupérables.

# FFM-M21 — portable credential store (age-encrypted file)
# MT
cloud-error-credentials-locked = Les identifiants cloud sont verrouillés. Déverrouillez le fichier d’identifiants de ce lecteur pour utiliser ce backend.
cloud-error-credentials-bad-passphrase = Cette phrase secrète n’ouvre pas le fichier d’identifiants.
cloud-error-credentials-empty-passphrase = Saisissez une phrase secrète.
cloud-error-credentials-not-portable = Cette installation conserve les identifiants dans le trousseau système ; il n’y a rien à déverrouiller.
portable-credentials-title = Identifiants cloud portables
portable-credentials-unlock-body = Saisissez la phrase secrète qui protège le fichier d’identifiants de ce lecteur.
portable-credentials-create-body = Choisissez une phrase secrète pour protéger les identifiants cloud sur ce lecteur. Elle est irrécupérable en cas d’oubli.
portable-credentials-passphrase-label = Phrase secrète
portable-credentials-unlock-action = Déverrouiller
portable-credentials-lock-action = Verrouiller

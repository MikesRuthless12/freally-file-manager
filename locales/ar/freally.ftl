...
toast-clipboard-files-detected = توجد ملفات في الحافظة — اضغط اختصار اللصق لنسخها عبر Freally File Manager
...
settings-paste-shortcut-hint = اضغط هذه المجموعة في أي مكان على نظامك للصق الملفات المنسوخة من Explorer / Finder / Files عبر Freally File Manager. يُترجم CmdOrCtrl إلى Cmd على macOS وCtrl على Windows / Linux.
settings-clipboard-watcher = مراقبة الحافظة بحثًا عن الملفات المنسوخة
settings-clipboard-watcher-hint = إظهار إشعار عند ظهور عناوين ملفات في الحافظة، للتلميح بأنه يمكنك اللصق عبر Freally File Manager. يفحص كل 500 مل.ث أثناء التفعيل.
...
settings-intercept-copy-hint = عند التفعيل، يمر Ctrl+C / Ctrl+V في Explorer عبر Freally File Manager. سيصل التسجيل في المرحلة 14.
...
settings-updater-hint = يتحقق Freally File Manager من التحديثات الموقّعة مرة واحدة في اليوم على الأكثر. تُثبَّت التحديثات عند إنهاء التطبيق التالي.
...
settings-on-locked-hint = القضاء على أخطاء "الملف قيد الاستخدام من قبل عملية أخرى". يلتقط Freally File Manager لقطة لوحدة تخزين المصدر (VSS على Windows، وZFS/Btrfs على Linux، وAPFS على macOS) ويقرأ من نسخة اللقطة.
snapshot-prompt-title = هذا الملف قيد الاستخدام من قبل عملية أخرى
snapshot-prompt-body = برنامج آخر يبقي { $path } مفتوحًا للكتابة الحصرية. اختر كيف ينبغي لـ Freally File Manager التعامل مع هذا الملف والملفات المشابهة على نفس وحدة التخزين.
snapshot-source-active = 📷 جارٍ القراءة من لقطة { $kind } لـ { $volume }
snapshot-create-failed = تعذّر إنشاء لقطة لوحدة تخزين المصدر
snapshot-vss-needs-elevation = القراءة من لقطة VSS تتطلب إذن المسؤول. سيطلب منك Freally File Manager السماح بذلك.
...
resume-prompt-body = اكتشف Freally File Manager { $count } عملية نقل غير مكتملة من جلسة سابقة. اختر ما تريد فعله بكل منها.
...
dropstack-show-tray-icon = إظهار أيقونة Freally File Manager في علبة النظام
...
settings-mount-on-launch-path-placeholder = مثل C:\Mounts\freally
...
settings-audit-file-path-placeholder = مثل C:\ProgramData\Freally\audit.log
...
cli-help-tagline = Freally File Manager CLI — نسخ ومزامنة وتحقق وتدقيق دقيق على مستوى البايت للملفات لخطوط أنابيب CI/CD.
...
cli-info-shape-recorded = تم تسجيل شكل النطاق الترددي "{ $rate }"؛ يُفرض عبر freally-shape
...
settings-mobile-pair-active = خادم الاقتران نشط — امسح رمز QR بتطبيق Freally File Manager للجوال
...
settings-dedup-hint = عندما يتشارك المصدر والوجهة وحدة تخزين، يستطيع Freally File Manager استنساخ الملفات على مستوى نظام الملفات بدلًا من نسخ البايتات. الـ reflink فوري وآمن؛ والـ hardlink أسرع لكن الاسمين يتشاركان الحالة.
...
plugin-hint = توسّع إضافات WASM المعزولة Freally File Manager بخطافات مخصّصة. تعمل كل إضافة ضمن حدود لوحدة المعالجة والذاكرة لكل استدعاء، ولا ترى سوى إمكانات المضيف التي تمنحها لها.
...
settings-tab-server = الخادم
server-hint = شغّل Freally File Manager كخادم ملفات بلا واجهة. اختر البروتوكولات التي تريد إتاحتها، واضبط العنوان والمجلد المراد تقديمه، واطلب المصادقة اختياريًا.
server-protocols = البروتوكولات
server-bind-addr = عنوان الربط
server-root = المجلد المقدَّم
server-readonly = للقراءة فقط (رفض التحميل والحذف)
server-auth-mode = المصادقة
server-auth-none = لا شيء
server-auth-bearer = رمز Bearer
server-auth-basic = أساسية (اسم المستخدم + كلمة المرور)
server-auth-token = الرمز
server-auth-user = اسم المستخدم
server-auth-password = كلمة المرور
otel-endpoint = نقطة نهاية OpenTelemetry
webhook-section = Webhooks
webhook-url = عنوان Webhook
webhook-add = إضافة Webhook
webhook-remove = إزالة
webhook-empty = لم تُهيَّأ أي webhooks.
webhook-pushover-token = رمز Pushover
webhook-pushover-user = مستخدم Pushover
server-start = بدء الخادم
server-stop = إيقاف الخادم
server-status-running = يعمل على { $addr }
server-status-stopped = متوقف

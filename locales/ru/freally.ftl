...
toast-clipboard-files-detected = Файлы в буфере обмена — нажмите сочетание вставки, чтобы скопировать через Freally File Manager
...
collision-modal-hash-computing = Вычисление…
collision-modal-hash-identical = Идентичны
collision-modal-hash-different = Различаются
...
settings-paste-shortcut-hint = Нажмите это сочетание в любом месте системы, чтобы вставить файлы, скопированные из Explorer / Finder / Files, через Freally File Manager. CmdOrCtrl соответствует Cmd на macOS и Ctrl на Windows / Linux.
settings-clipboard-watcher = Отслеживать скопированные файлы в буфере обмена
settings-clipboard-watcher-hint = Показывать уведомление, когда в буфере обмена появляются URL файлов, подсказывая, что их можно вставить через Freally File Manager. Опрос каждые 500 мс, когда включено.
...
settings-force-parallel-chunks = Параллельное копирование частями (только RAID / массивы)
settings-force-parallel-chunks-hint = Разбивает каждое крупное копирование на параллельные части. Полезно только для чередующихся/RAID/сетевых назначений; ЗАМЕДЛЯЕТ одиночный SSD/NVMe (-25%…-76%). Оставьте выключенным, если назначение не является массивом из нескольких дисков.
...
settings-intercept-copy-hint = Когда включено, Ctrl+C / Ctrl+V в Explorer проходят через Freally File Manager. Регистрация появится в Phase 14.
...
settings-updater-hint = Freally File Manager проверяет наличие подписанных обновлений не чаще раза в день. Обновления устанавливаются при следующем закрытии приложения.
...
settings-on-locked-hint = Устраняет ошибки «файл используется другим процессом». Freally File Manager создаёт снимок тома источника (VSS в Windows, ZFS/Btrfs в Linux, APFS в macOS) и читает из копии снимка.
snapshot-prompt-title = Этот файл используется другим процессом
snapshot-prompt-body = Другая программа открыла { $path } для монопольной записи. Выберите, как Freally File Manager должен обрабатывать этот и подобные файлы на том же томе.
snapshot-source-active = 📷 Чтение из снимка { $kind } тома { $volume }
snapshot-create-failed = Не удалось создать снимок тома источника
snapshot-vss-needs-elevation = Чтение из снимка VSS требует прав администратора. Freally File Manager запросит разрешение.
...
resume-prompt-body = Freally File Manager обнаружил незавершённые передачи из предыдущего сеанса: { $count }. Выберите, что делать с каждой.
...
dropstack-show-tray-icon = Показывать значок Freally File Manager в трее
...
cli-help-tagline = Freally File Manager CLI — байт-в-байт копирование, синхронизация, проверка и аудит файлов для конвейеров CI/CD.
...
cli-info-shape-recorded = Ограничение полосы «{ $rate }» записано; применение выполняется через freally-shape
...
settings-mobile-pair-active = Сервер сопряжения активен — отсканируйте QR-код в мобильном приложении Freally File Manager
...
settings-dedup-hint = Когда источник и назначение находятся на одном томе, Freally File Manager может клонировать файлы на уровне файловой системы вместо копирования байтов. Reflink мгновенный и безопасный; жёсткая ссылка быстрее, но оба имени разделяют состояние.
...
library-tab-compare = Сравнить
repo-change-added = Добавлено
repo-change-removed = Удалено
repo-change-modified = Изменено
repo-change-unchanged = Без изменений
repo-diff-summary = { $added } добавлено · { $removed } удалено · { $modified } изменено
repo-diff-bytes-added = { $bytes } новых
repo-diff-pick-two = Выберите два снимка для сравнения
# Phase 49r — statistics / reports.
library-tab-reports = Отчёты
report-growth-title = Рост хранилища
report-by-kind-title = По типу
report-top-files-title = Топ файлов
report-dedup-ratio = Дедуплицировано { $pct }%
report-export = Экспорт отчёта
report-exported = Отчёт сохранён в { $path }
report-file-versions = { $n } версий
# Phase 49p — pinning / prune.
repo-pin = Закрепить
repo-unpin = Открепить
repo-pinned-badge = Закреплено
repo-prune-title = Очистить
repo-prune-keep-last = Оставить новейшие
repo-prune-removed = Очищено снимков: { $n }
repo-prune-none = Очищать нечего
...
plugin-hint = Изолированные WASM-плагины расширяют Freally File Manager пользовательскими хуками. Каждый плагин работает с ограничениями по ЦП и памяти на вызов и видит только те возможности хоста, которые вы ему предоставите.
...
bottleneck-source-io = Источник I/O
bottleneck-dest-io = Назначение I/O
bottleneck-network = Сеть
bottleneck-antivirus = Антивирус
bottleneck-cpu = CPU
bottleneck-thermal = Перегрев
bottleneck-unknown = Неизвестно
diag-aria = Узкое место: { $cause }
diag-tooltip = Ограничено: { $cause } · { $rate }
diag-spark-aria = Пропускная способность за последнюю минуту
diag-keeping-up = Справляется
diag-label = Диагностика

# Phase 48 — server mode + observability (Settings → Server).
settings-tab-server = Сервер
server-hint = Запустите Freally File Manager как файловый сервер без интерфейса. Выберите протоколы для публикации, задайте адрес и папку для раздачи и при необходимости включите аутентификацию.
server-protocols = Протоколы
server-bind-addr = Адрес привязки
server-root = Раздаваемая папка
server-readonly = Только чтение (запрет загрузки и удаления)
server-auth-mode = Аутентификация
server-auth-none = Нет
server-auth-bearer = Токен Bearer
server-auth-basic = Базовая (имя пользователя + пароль)
server-auth-token = Токен
server-auth-user = Имя пользователя
server-auth-password = Пароль
otel-endpoint = Конечная точка OpenTelemetry
webhook-section = Webhooks
webhook-url = URL вебхука
webhook-add = Добавить вебхук
webhook-remove = Удалить
webhook-empty = Вебхуки не настроены.
webhook-pushover-token = Токен Pushover
webhook-pushover-user = Пользователь Pushover
server-start = Запустить сервер
server-stop = Остановить сервер
server-status-running = Работает на { $addr }
server-status-stopped = Остановлен
...
backup-retention-keep-all = Хранить все
backup-retention-last = Хранить последние { $n }
backup-retention-days = Старше { $days } дней
backup-retention-gfs = Ротация GFS
backup-prune-now = Очистить сейчас
backup-prune-none = Нечего очищать
backup-prune-result = Удалено { $removed } снимков · освобождено { $bytes }
# Phase 49f — per-source scheduling.
backup-schedule = Расписание
backup-schedule-manual = Вручную
backup-schedule-hourly = Ежечасно
backup-schedule-daily = Ежедневно
backup-schedule-weekly = Еженедельно
backup-next-run = Следующий запуск { $when }
backup-not-scheduled = Не запланировано
# Phase 49g — source filters.
backup-include-ph = Включающие шаблоны (через запятую)
backup-skip-hidden = Пропускать скрытые
# Phase 49q — notifications.
notify-title = Уведомления
notify-on-success = При успехе
notify-on-failure = При сбое
notify-test = Отправить тест
notify-test-sent = Тест отправлен на { $n } назначений
...
library-compact = Полное уплотнение
library-compact-started = Уплотнение начато — смотрите Задачи
# Phase 49h — compression.
library-stat-compression = Сэкономлено сжатием
storage-compression = Сжатие
storage-compression-off = Выкл.
storage-compression-auto = Авто (пропускать несжимаемые)
storage-compression-always = Всегда
storage-compression-restart = Применяется при следующем запуске
...
task-state-cancelled = Отменено
# Phase 49k — repository setup/connect wizard.
repo-wizard-title = Подключить репозиторий
repo-wizard-create-tab = Создать новый
repo-wizard-connect-tab = Подключить существующий
repo-field-name = Имя
repo-field-path = Расположение
repo-field-password = Пароль (необязательно)
repo-action-create = Создать
repo-action-connect = Подключить
repo-action-browse = Обзор…
...
repo-error-not-found = В этом расположении репозиторий не найден
repo-error-bad-pass = Неверный пароль
repo-note-no-encryption = Пароль управляет только доступом; шифрование данных в покое появится в одном из следующих выпусков
repo-confirm-forget = Удалить "{ $name }" из списка? Ваши данные останутся на диске.
repo-toast-created = Репозиторий "{ $name }" создан
repo-toast-connected = Подключено к "{ $name }"
repo-toast-pass-changed = Пароль обновлён
# Phase 49l — Sources dashboard.
library-tab-overview = Обзор
library-source-empty = Пока нет источников
library-source-unknown = (источник не указан)
library-source-snapshots = { $n } снимков
library-source-latest = Последний { $when }
# Phase 49n — verify & repair.
repo-action-verify = Проверить
repo-action-verify-deep = Проверить (читать все данные)
repo-action-repair = Исправить…
repo-verify-clean = Проверено { $files } файлов / { $chunks } блоков — повреждений нет
repo-verify-damaged = { $missing } отсутствует, { $corrupt } повреждённых блоков
repo-repair-confirm = Удалить { $n } снимков, которые больше нельзя восстановить?
repo-repair-removed = Удалено { $n } повреждённых снимков
repo-repair-none = Исправлять нечего — репозиторий чист
...
eula-intro = Ознакомьтесь с соглашением ниже. Его необходимо принять перед использованием Freally File Manager.
eula-scroll-hint = Прокрутите до конца, чтобы активировать «Принимаю».
eula-thanks = Спасибо за прочтение.
eula-agree = Принимаю
eula-decline = Отклонить и выйти
eula-error = Не удалось сохранить принятие: { $error }

# FFM-M01 — Explorer copy-verb takeover.
settings-intercept-copy-unsupported = Перехват копирования доступен только в Windows.
settings-intercept-copy-needs-menu = Сначала включите интеграцию с контекстным меню — обработчик копирования должен быть зарегистрирован, прежде чем перехват сможет взять управление.
settings-revert-copy-handler = Вернуть обработчик копирования Windows
toast-copy-handler-reverted = Восстановлен обработчик копирования Windows
settings-context-menu-hint = Регистрирует или удаляет контекстное меню и обработчик копирования Freally в системе (для пользователя, без прав администратора).
paste-chooser-title = Копировать и вставить
paste-chooser-close = Закрыть
paste-chooser-files = { $count } файл(ов) — выберите назначение
paste-chooser-system-copy = Системное копирование
paste-chooser-system-move = Системное перемещение
paste-chooser-system-hint = Быстрая простая передача, без проверки
paste-chooser-freally-copy = Копирование Freally
paste-chooser-freally-move = Перемещение Freally
paste-chooser-freally-hint = Проверенная побайтово точная передача
paste-chooser-replace-older = Freally — заменить старые файлы
paste-chooser-replace-older-hint = Проверено; перезаписывает только если источник новее
paste-chooser-more = Дополнительные параметры…
toast-system-paste-done = Вставлено { $items } элемент(ов)

# FFM-M02 — transactional undo.
undo-title-copy = Отменить копирование — удалить скопированные файлы?
undo-title-move = Отменить перемещение — вернуть файлы?
undo-summary = { $ready } из { $total } элементов можно отменить; остальные изменились, исчезли или конфликтуют.
undo-action-trash = В корзину
undo-action-move-back = Вернуть
undo-status-ready = Готово
undo-status-skip-missing = Отсутствует — пропущено
undo-status-skip-changed = Изменён — пропущено
undo-status-conflict = Исходный путь занят
undo-cancel = Отмена
undo-confirm = Отменить { $count } элемент(ов)
toast-undo-done = Отмена завершена: { $done } выполнено, { $skipped } пропущено, { $failed } не удалось
toast-undo-nothing = Нечего отменять
history-undo = Отменить
history-undo-hint = Обращает эту задачу: скопированные файлы отправляются в корзину, перемещённые возвращаются на исходное место

# FFM-M03 — trash-aware delete.
menu-trash-source = Удалить источник в корзину
trash-confirm = Отправить в корзину?
{ $path }
toast-trash-done = Перемещено в корзину: { $trashed } элемент(ов), { $failed } не удалось
settings-safety-confirm-trash = Подтверждать перед удалением в корзину
settings-safety-move-to-trash = Отправлять перемещённые исходные файлы в корзину
settings-safety-move-to-trash-hint = При перемещении отправлять источник в корзину, а не удалять — перемещение с возможностью восстановления.

# FFM-M04/M05 — eject + keep-awake.
menu-eject-destination = Извлечь целевой том
toast-eject-done = Том извлечён — можно безопасно отключить
toast-eject-failed = Не удалось извлечь: { $error }
settings-power-keep-awake = Не давать компьютеру засыпать во время задач
settings-power-keep-awake-hint = Удерживает системную блокировку (без сна и заставки), пока идёт копирование.

# FFM-M06 — content-aware collision policies.
collision-policy-skip-identical-else-overwrite = Перезаписывать только при различии содержимого
collision-policy-skip-identical-else-prompt = Пропускать, если идентично, иначе спрашивать

# FFM-M07 — failed-file ledger + retry.
history-retry-failed = Повторить неуспешные
history-retry-failed-hint = Перекопировать только файлы, не удавшиеся в этом задании
history-export-failed = Экспорт неуспешных
history-export-failed-hint = Сохранить список неуспешных файлов как CSV / TXT / JSON
toast-retry-failed-none = Нет неуспешных файлов для повтора
toast-retry-failed-queued = Повторно поставлено в очередь { $count } неуспешных файлов
toast-failed-exported = Список неуспешных экспортирован

# FFM-M08 — checksum sidecars.
menu-create-checksums = Создать контрольные суммы (SHA-256)
toast-checksums-created = Контрольные суммы записаны для { $files } файл(ов)
sidecar-verify-clean-title = Все файлы проверены
sidecar-verify-bad-title = Проверка контрольных сумм не удалась
sidecar-verify-summary = { $ok } ОК, { $failed } не удалось, { $missing } отсутствует

...
toast-clipboard-files-detected = Файли в буфері обміну — натисніть комбінацію вставлення, щоб скопіювати через Copy That
...
settings-paste-shortcut-hint = Натисніть цю комбінацію будь-де в системі, щоб вставити файли, скопійовані з Explorer / Finder / Files, через Copy That. CmdOrCtrl відповідає Cmd на macOS, Ctrl на Windows / Linux.
settings-clipboard-watcher = Стежити за буфером обміну на копійовані файли
settings-clipboard-watcher-hint = Показувати сповіщення, коли в буфері обміну з'являються URL-адреси файлів, натякаючи, що можна вставити через Copy That. Опитує кожні 500 мс, коли ввімкнено.
...
settings-intercept-copy-hint = Коли ввімкнено, Ctrl+C / Ctrl+V у Explorer проходять через Copy That. Реєстрація з'явиться у Phase 14.
...
settings-updater-hint = Copy That перевіряє підписані оновлення не частіше ніж раз на день. Оновлення встановлюються під час наступного виходу із застосунку.
...
settings-on-locked-hint = Усуває помилки «файл використовується іншим процесом». Copy That робить знімок тому джерела (VSS на Windows, ZFS/Btrfs на Linux, APFS на macOS) і читає з копії знімка.
snapshot-prompt-title = Цей файл використовується іншим процесом
snapshot-prompt-body = Інша програма тримає { $path } відкритим для ексклюзивного запису. Виберіть, як Copy That має поводитися з цим та подібними файлами на тому ж томі.
snapshot-source-active = 📷 Читання зі знімка { $kind } тому { $volume }
snapshot-create-failed = Не вдалося створити знімок тому джерела
snapshot-vss-needs-elevation = Читання зі знімка VSS потребує прав адміністратора. Copy That попросить дозвіл.
...
resume-prompt-body = Copy That виявив { $count } незавершене(их) перенесення з попереднього сеансу. Виберіть, що робити з кожним.
...
dropstack-show-tray-icon = Показувати піктограму Copy That у треї
...
cli-help-tagline = Copy That CLI — побайтово точне копіювання, синхронізація, перевірка та аудит файлів для конвеєрів CI/CD.
...
cli-info-shape-recorded = Профіль пропускної здатності "{ $rate }" записано; застосування підключається через copythat-shape
...
settings-mobile-pair-active = Сервер підключення активний — відскануйте QR за допомогою мобільного застосунку Copy That
...
settings-dedup-hint = Коли джерело й призначення на одному томі, Copy That може клонувати файли на рівні файлової системи замість копіювання байтів. Reflink миттєвий і безпечний; hardlink швидший, але обидві назви спільно використовують стан.
...
plugin-hint = Ізольовані WASM-плагіни розширюють Copy That власними хуками. Кожен плагін працює в межах обмежень ЦП і пам'яті на виклик і бачить лише надані вами можливості хоста.
...
server-hint = Запустіть Copy That як файловий сервер без інтерфейсу. Виберіть протоколи для надання, задайте адресу та теку для роздавання й за потреби увімкніть автентифікацію.

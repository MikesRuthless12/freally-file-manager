 ...
 *
 * The `freally queue --watch` subcommand isn't built today (only
...
test.describe("§4.11g Phase 14f queue-while-locked", () => {
  test.fixme(
    ...
    async () => {
      // The `freally queue --watch` subcommand isn't built yet —
      // only `stack` exists in `freally --help`. Coverage for the
      // event shape itself is in `cargo test -p freally-platform`

 ...
 * the call log don't leak. See `apps/freally-ui/e2e/README.md` for
...
// fixtures/ → e2e/ → freally-ui/ → apps/ → repo root
...
function loadTranslations(locale: string): Record<string, string> {
  ...
  const path = resolve(REPO_ROOT, "locales", locale, "freally.ftl");
  if (!existsSync(path)) {
...
declare global {
  interface Window {
    __freally_e2e__?: FreallyE2EHandle;
  }
}
...
export const test = base.extend<Fixtures>({
  tauri: async ({ page }, use) => {
    ...
    await page.addInitScript(() => {
      if ((window as unknown as { __freally_e2e__?: unknown }).__freally_e2e__) return;
      ...
      const invoke = async (cmd: string, args?: Record<string, unknown>): Promise<unknown> => {
        ...
        if (!handler) {
          throw new Error("[freally e2e] no handler for invoke('" + cmd + "')");
        }
      ...
      (window as unknown as { __TAURI_INTERNALS__: unknown }).__TAURI_INTERNALS__ = {
        ...
        runtimeAuthToken: "freally-e2e-shim",
      };

      (window as unknown as { __freally_e2e__: unknown }).__freally_e2e__ = {
    ...
    await page.addInitScript((seed: { en: Record<string, string> }) => {
      const reg = window.__freally_e2e__ as unknown as {
    ...
    const fixture: TauriFixture = {
      async handle(cmd, handler) {
        ...
        await page.evaluate(
          ({ cmd, fnSrc }) => {
            ...
            window.__freally_e2e__?.setHandler(cmd, fn);
          },
          { cmd, fnSrc },
        );
      },
      ...
      async calls() {
        return await page.evaluate(() => window.__freally_e2e__?.calls() ?? []);
      },
      async callsFor(cmd) {
        return await page.evaluate(
          (cmd) =>
            (window.__freally_e2e__?.calls() ?? []).filter(
      ...
      async waitForCall(cmd, timeoutMs = 5000) {
        const handle = await page.waitForFunction(
          (cmd) => {
            const calls = window.__freally_e2e__?.calls() ?? [];
      ...
      async setDefault(handler) {
        ...
        await page.evaluate((fnSrc) => {
          const fn = new Function("args", `return (${fnSrc})(args);`) as InvokeHandler;
          window.__freally_e2e__?.setDefaultHandler(fn);
        }, fnSrc);
      },
      async handleValue(cmd, value) {
        await page.evaluate(
          ({ cmd, value }) => {
            const v = value;
            window.__freally_e2e__?.setHandler(cmd, () => v);
          },
          { cmd, value },
        );
      },
...
async function resetSafely(page: Page): Promise<void> {
  ...
  try {
    await page.evaluate(() => window.__freally_e2e__?.reset());
  } catch {

 ...
 *
 * The CLI is a separate binary — `freally` from `freally-cli`.
 ...
 *
 * The CLI smoke (`cargo test -p freally-cli --test phase_36_cli`)
...
function freallyBin(): string {
  // Prefer the release binary (faster, what users get); fall back
  // to debug if release isn't built yet.
  const candidates = [
    resolve(REPO_ROOT, "target/release/freally.exe"),
    resolve(REPO_ROOT, "target/release/freally"),
    resolve(REPO_ROOT, "target/debug/freally.exe"),
    resolve(REPO_ROOT, "target/debug/freally"),
  ];
...
test.describe("§4.10 CLI (Phase 36)", () => {
  test("`freally version --json` emits a parseable JSON object", async () => {
    const bin = freallyBin();
    test.skip(
      bin === "",
      "freally binary not built — run `cargo build -p freally-cli` first",
    );
  ...
  test("`freally copy <src> <dst> --json` emits one event per line", async () => {
    const bin = freallyBin();
    test.skip(bin === "", "freally binary not built");
    withTmp("freally-e2e-cp-", (dir) => {
  ...
  test("`freally plan --spec sample.toml` reports actions", async () => {
    const bin = freallyBin();
    test.skip(bin === "", "freally binary not built");
    withTmp("freally-e2e-plan-", (dir) => {
      const srcDir = resolve(dir, "src");
      const dstDir = resolve(dir, "dst");
      mkdirSync(srcDir, { recursive: true });
...
`.trim();
  ...
  test("`freally apply --spec sample.toml` runs once, second run exits 0", async () => {
    const bin = freallyBin();
    test.skip(bin === "", "freally binary not built");
    withTmp("freally-e2e-apply-", (dir) => {
      const srcDir = resolve(dir, "src");
      const dstDir = resolve(dir, "dst");
      mkdirSync(srcDir, { recursive: true });
      writeFileSync(resolve(srcDir, "a.bin"), Buffer.alloc(32));
...
`.trim();
  ...
  test("`freally verify <file> --algo blake3 --against <sidecar>` exits 4 on mismatch", async () => {
    const bin = freallyBin();
    test.skip(bin === "", "freally binary not built");
    withTmp("freally-e2e-verify-", (dir) => {

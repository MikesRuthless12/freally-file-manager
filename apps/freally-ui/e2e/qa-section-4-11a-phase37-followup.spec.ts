...
test.describe("§4.11a Phase 37 follow-up #2", () => {
  test("First-launch onboarding modal appears once → does not reappear after dismiss", async ({
    ...
    await page.addInitScript(() => {
      const reg = (window as unknown as {
        __freally_e2e__?: { setHandler: (cmd: string, h: (a: unknown) => unknown) => void };
      }).__freally_e2e__;
      if (!reg) return;
  ...
  test.fixme(
    ...
    async ({ page: _page, tauri: _tauri }) => {
      ...
      // `cargo test -p freally-mobile` (the snapshot derivation
  ...
  test.fixme(
    ...
    async ({ page: _page, tauri: _tauri }) => {
      ...
      // `cargo test -p freally-ui` plus the tauri-build CI job.
    },
  );
});

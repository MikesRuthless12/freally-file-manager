 ...
 *
 * The scheduling work happens inside `freally-cli`, not the Tauri
...
test.describe("§4.11f Phase 14d scheduled jobs", () => {
  test("CLI render — Windows: schtasks /Create form", async () => {
    ...
    withTmp("freally-e2e-sched-win-", (dir) => {
  ...
  test("CLI render — macOS: launchd plist form", async () => {
    const bin = freallyBin();
    test.skip(bin === "", "freally binary not built");
    withTmp("freally-e2e-sched-mac-", (dir) => {
      const specPath = writeSpec(dir);
      const r = spawnSync(
        bin,
  ...
  test("CLI render — Linux: systemd .service + .timer pair", async () => {
    const bin = freallyBin();
    test.skip(bin === "", "freally binary not built");
    withTmp("freally-e2e-sched-linux-", (dir) => {
      const specPath = writeSpec(dir);
      const r = spawnSync(
        bin,
  ...
  test("Phase 17a guard — `..`-laden source rejected with err-path-escape", async () => {
    const bin = freallyBin();
    test.skip(bin === "", "freally binary not built");
    withTmp("freally-e2e-sched-escape-", (dir) => {

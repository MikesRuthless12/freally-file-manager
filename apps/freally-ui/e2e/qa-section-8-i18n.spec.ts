 ...
 *    `crates/freally-cli/src/cli.rs`'s `after_help` block.
 *
...
test.describe("§8 i18n — CLI stays English", () => {
  for (const lang of ["fr_FR.UTF-8", "ja_JP.UTF-8", "ar_SA.UTF-8"]) {
    test(`freally --help stays English under LANG=${lang}`, async () => {
      ...
      // $LANG (per `crates/freally-cli/src/cli.rs`'s after_help

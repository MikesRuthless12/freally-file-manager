...
test.describe("§5 Performance + benchmarks", () => {
  test("Phase 13c parallel-chunk path stays env-var-gated by default", async () => {
    ...
    // freally-platform) verifies the parallel chunk path is OFF
    // unless `FREALLY_PARALLEL_CHUNKS=N` is set. Run it via cargo;
  ...
  test.fixme(
    ...
    async () => {
      ...
      // smoke lives at `cargo test -p freally-history --test
      // phase_19a_scan_db`.
    },
  );

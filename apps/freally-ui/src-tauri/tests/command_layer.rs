...
//! IPC DTO serialisation, and `freally_core::copy_file` with a
...
use freally_core::{CopyOptions, JobKind, JobState, Queue, copy_file};
use freally_ui_lib::ipc::{JobDto, job_state_name};
use freally_ui_lib::runner::build_globals;
...
fn list_jobs_impl_attributes_registry_jobs_to_their_queue_ids() {
    ...
    let dtos = freally_ui_lib::commands::list_jobs_impl(&state);
...
fn find_queue_for_job_locates_registry_routed_jobs() {
    ...
    let (legacy_jid, legacy_q) =
        freally_ui_lib::commands::find_queue_for_job(legacy_id.as_u64(), &state)
    ...
    let (routed_jid, routed_q) =
        freally_ui_lib::commands::find_queue_for_job(routed_id.as_u64(), &state)
    ...
    assert!(freally_ui_lib::commands::find_queue_for_job(9_999_999, &state).is_none());
}
...
fn build_globals_for_state_aggregates_legacy_and_registry() {
    ...
    let g = freally_ui_lib::runner::build_globals_for_state(&state);
...
fn remove_job_via_registry_prunes_empty_queue() {
    ...
    let (jid_resolved, queue) =
        freally_ui_lib::commands::find_queue_for_job(jid.as_u64(), &state).unwrap();
...
async fn pause_resume_cancel_cycle_runs_through_queue() {
    ...
    let opts = CopyOptions {
        buffer_size: freally_core::MIN_BUFFER_SIZE,
        ..Default::default()
    };
    ...
    let outcome = tokio::time::timeout(std::time::Duration::from_secs(10), async {
        while let Some(evt) = rx.recv().await {
            match evt {
                freally_core::CopyEvent::Paused => return Some("paused"),
                freally_core::CopyEvent::Completed { .. } => return Some("completed"),
                _ => continue,
            }
        }
        None
...
fn available_locales_covers_all_eighteen() {
    let locales = freally_ui_lib::i18n::available_locales();
...
fn translations_include_every_en_key_in_every_locale() {
    let en = freally_ui_lib::i18n::translations("en");
    for code in freally_ui_lib::i18n::available_locales() {
        let t = freally_ui_lib::i18n::translations(&code);
        for key in en.keys() {
...
fn icon_classifier_knows_common_buckets() {
    let dto = freally_ui_lib::icon::classify(std::path::Path::new("clip.mp4"));

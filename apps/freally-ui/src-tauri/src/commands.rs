...
use freally_core::{CopyOptions, JobKind};
...
pub async fn start_copy(
    ...
    // surface user paths on stderr; opt in via `RUST_LOG=freally=debug`.
    tracing::debug!(
        target: "freally::ipc",
...
/// concrete worker count. `Auto` asks `freally_platform` for a
...
fn platform_auto_concurrency_hint() -> u8 {
    // `freally-platform::recommend_concurrency` takes src + dst
...
fn resolve_filters(settings: &freally_settings::Settings) -> Option<freally_core::FilterSet> {
...
fn parse_collision_policy(raw: Option<&str>) -> Result<freally_core::CollisionPolicy, String> {
    use freally_core::CollisionPolicy;
    let Some(s) = raw else {
...
) -> freally_core::ErrorPolicy {
    use crate::ipc::ErrorPolicyDto;
    use freally_core::ErrorPolicy;
    match raw {
        ...
        None => match settings.advanced.error_policy {
            freally_settings::ErrorPolicyChoice::Ask => ErrorPolicy::Ask,
            freally_settings::ErrorPolicyChoice::Skip => ErrorPolicy::Skip,
            freally_settings::ErrorPolicyChoice::Abort => ErrorPolicy::Abort,
...
fn apply_options(
    ...
    let mut opts = CopyOptions {
        ...
        strategy: if settings.transfer.force_parallel_chunks {
            freally_core::CopyStrategy::ParallelChunks
        } else {
        ...
        on_locked: match settings.transfer.on_locked {
            freally_settings::LockedFilePolicyChoice::Ask => freally_core::LockedFilePolicy::Ask,
    ...
    opts.fast_copy_hook = Some(std::sync::Arc::new(freally_platform::PlatformFastCopyHook));
    ...
    if opts.preserve_sparseness {
        opts.sparse_ops = Some(std::sync::Arc::new(freally_platform::PlatformSparseOps));
    }
    ...
    if opts.preserve_security_metadata {
        opts.meta_ops = Some(std::sync::Arc::new(freally_platform::PlatformMetaOps));
    }
    ...
    if matches!(
        opts.on_locked,
        freally_core::LockedFilePolicy::Snapshot | freally_core::LockedFilePolicy::Ask
    ) {
        opts.snapshot_hook = Some(std::sync::Arc::new(
            freally_snapshot::FreallySnapshotHook::new(),
        ));
    }
...
fn resolve_verifier(
    dto: &CopyOptionsDto,
    settings: &freally_settings::Settings,
) -> Result<Option<freally_core::Verifier>, String> {
    ...
    let algo = freally_hash::HashAlgorithm::from_str(&name)
...
pub fn remove_job(id: u64, state: State<'_, AppState>) -> Result<(), String> {
    ...
    if q.id() != freally_core::QueueId::DEFAULT {
...
fn job_id(id: u64, state: &State<'_, AppState>) -> Result<freally_core::JobId, String> {
...
pub fn find_queue_for_job(
    id: u64,
    state: &AppState,
) -> Option<(freally_core::JobId, freally_core::Queue)> {
    if let Some(j) = state
        .queue
        .snapshot()
        .into_iter()
...
/// `freally-helper` ELEVATED through the OS consent flow (UAC /
...
pub async fn retry_elevated(id: u64, state: State<'_, AppState>) -> Result<u64, String> {
    ...
    let resp = if freally_helper::should_escalate(&resp) {
...
fn parse_error_action(action: &str) -> Result<freally_core::ErrorAction, String> {
    match action {
        "retry" => Ok(freally_core::ErrorAction::Retry),
        "skip" => Ok(freally_core::ErrorAction::Skip),
        "abort" => Ok(freally_core::ErrorAction::Abort),
...
fn parse_collision_resolution(
    resolution: &str,
    rename_to: Option<String>,
) -> Result<freally_core::CollisionResolution, String> {
    match resolution {
        "skip" => Ok(freally_core::CollisionResolution::Skip),
        "overwrite" => Ok(freally_core::CollisionResolution::Overwrite),
        "rename" => {
            let name =
        ...
        "abort" => Ok(freally_core::CollisionResolution::Abort),
...
/// [`freally_history::History::search`].
#[tauri::command]
pub async fn history_search(
    ...
    let f = freally_history::HistoryFilter {
...
pub async fn history_items(
    ...
    let history = require_history(&state)?;
    let id = freally_history::JobRowId(row_id);
...
pub async fn history_export_csv(
    ...
    let rows = history.search(f).await.map_err(|e| e.to_string())?;
    let body = freally_history::export_csv(&rows);
    let bytes = body.len() as u64;
...
pub async fn history_rerun(
    ...
    Ok(crate::shell::enqueue_jobs(
        ...
        copy_opts,
        None,
        freally_core::CollisionPolicy::default(),
        freally_core::ErrorPolicy::Ask,
        tree_concurrency,
        filters,
    ))
}
...
fn history_job_to_dto(s: freally_history::JobSummary) -> crate::ipc::HistoryJobDto {
...
fn totals_to_dto(t: freally_history::Totals) -> crate::ipc::TotalsDto {
    crate::ipc::TotalsDto {
        bytes: t.bytes,
        files: t.files,
        jobs: t.jobs,
...
fn history_item_to_dto(r: freally_history::ItemRow) -> crate::ipc::HistoryItemDto {
...
fn diff_setting_groups(
    before: &freally_settings::Settings,
    after: &freally_settings::Settings,
) -> String {
...
pub async fn destination_free_bytes(path: String) -> Result<u64, String> {
    ...
    let p = validate_ipc_path(&path).map_err(err_string)?;
    let moved = p.clone();
    tokio::task::spawn_blocking(move || freally_platform::free_space_bytes(&moved).unwrap_or(0))
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn path_metadata(paths: Vec<String>) -> Result<Vec<PathMetaDto>, String> {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        tracing::debug!(target: "freally::ipc", "path_metadata done");
        Ok(out)
    })
    .await
    .map_err(|e| e.to_string())?
...
pub fn discard_resume(row_id: u64, state: State<'_, AppState>) -> Result<(), String> {
    if let Some(j) = state.journal.as_ref() {
        j.delete_job(freally_journal::JobRowId(row_id))
            .map_err(|e| e.to_string())?;
    }
    state
        .startup_unfinished
...
pub fn validate_schedule_spec(spec: String) -> Result<usize, String> {
    freally_shape::Schedule::parse(&spec)
        .map(|s| s.rules().len())
        .map_err(|e| e.to_string())
}
...
pub fn add_conflict_rule(
    ...
    let rule = freally_settings::ConflictRule {
        pattern,
        resolution: freally_settings::ConflictRuleResolution::from_wire(&resolution),
    };
...
pub fn current_conflict_rules(
    job_id: u64,
    state: State<'_, AppState>,
) -> freally_settings::ConflictProfile {
...
pub fn save_conflict_profile(
    name: String,
    profile: freally_settings::ConflictProfile,
    state: State<'_, AppState>,
...
pub fn discard_all_resumes(state: State<'_, AppState>) -> Vec<String> {
    ...
    if let Some(j) = state.journal.as_ref() {
        for id in &ids {
            if let Err(e) = j.delete_job(freally_journal::JobRowId(*id)) {

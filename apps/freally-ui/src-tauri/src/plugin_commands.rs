...
//!
//! The runtime itself (`freally-plugin`) is concerned with one thing:
...
//! <config_dir>/plugins/
//!   organize-by-exif/
//!     plugin.toml      ← manifest, parsed by freally_plugin::PluginManifest
...
use freally_plugin::{Capability, PluginManifest};
...
const BUNDLE_HASH_DOMAIN: &[u8] = b"freally-plugin-bundle-v1\x00";
...
/// Resolve `<config_dir>/freally/plugins`. Mirrors the resolver
...
fn hook_kind_str(h: freally_plugin::HookKind) -> &'static str {
    use freally_plugin::HookKind::*;
    match h {
...
///
/// ```text
/// H = BLAKE3("freally-plugin-bundle-v1\0"

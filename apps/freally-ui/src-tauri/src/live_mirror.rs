...
//! Each instance owns:
//!
//! - A [`freally_watch::Watcher`] rooted at the pair's left side,
//!   whose debounced events act as "something changed" pulses.
//! - A tokio task that loops: (1) run a full `freally_sync::sync`
...
use freally_sync::{SyncControl, SyncEvent, SyncMode, SyncOptions, SyncPair, sync};
use freally_watch::{Watcher, WatcherOptions};
...
async fn run_loop(
    pair_id: String,
    cfg: freally_settings::SyncPairConfig,

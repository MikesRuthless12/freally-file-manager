...
use freally_settings::SyncPairConfig;
use freally_sync::{SyncControl, SyncEvent, SyncMode, SyncOptions, SyncPair, SyncReport, sync};
...
/// Remove a pair by id. The `.freally-sync.db` on disk is preserved
...
pub async fn start_sync(
    ...
    let mode = match cfg.mode {
        freally_settings::SyncModeChoice::TwoWay => SyncMode::TwoWay,
        freally_settings::SyncModeChoice::MirrorLeftToRight => SyncMode::MirrorLeftToRight,
        freally_settings::SyncModeChoice::MirrorRightToLeft => SyncMode::MirrorRightToLeft,
        freally_settings::SyncModeChoice::ContributeLeftToRight => SyncMode::ContributeLeftToRight,
    };
    ...
    tokio::spawn(async move {
        ...
        match result {
                    ...
                    if !rels.is_empty() {
                        ...
                        let _ = tokio::task::spawn_blocking(move || {
                            ...
                            if let Err(e) = repo.snapshot_files(
                                freally_chunk::SnapshotKind::Sync,
                                &sync_label,
                                now,
                                &pairs,
                            ) {
            ...
            Err(err) => {
                if matches!(err, freally_sync::SyncError::Cancelled) {
                    let _ = app_for_run.emit(
    ...
    while let Some(evt) = rx.recv().await {
        match evt {
            ...
            SyncEvent::ActionCompleted { action } => {
                ...
                if matches!(
                    action,
                    freally_sync::SyncAction::Copy { .. }
                        | freally_sync::SyncAction::KeepConflict { .. }
...
fn direction_wire(d: freally_sync::Direction) -> &'static str {
    match d {
        freally_sync::Direction::LeftToRight => "left-to-right",
        freally_sync::Direction::RightToLeft => "right-to-left",
    }
}

fn conflict_kind_wire(k: freally_sync::ConflictKind) -> &'static str {
    match k {
        freally_sync::ConflictKind::ConcurrentWrite => "concurrent-write",
        freally_sync::ConflictKind::DeleteEdit => "delete-edit",
        freally_sync::ConflictKind::AddAdd => "add-add",
        freally_sync::ConflictKind::CorruptEqual => "corrupt-equal",
    }
}

fn action_kind_wire(a: &freally_sync::SyncAction) -> String {
    match a {
        freally_sync::SyncAction::Noop { .. } => "noop".to_string(),
        freally_sync::SyncAction::Copy { direction, .. } => {
            format!("copy-{}", direction_wire(*direction))
        }
        freally_sync::SyncAction::Delete { direction, .. } => {
            format!("delete-{}", direction_wire(*direction))
        }
        freally_sync::SyncAction::KeepConflict { .. } => "keep-conflict".to_string(),
    }
}
...
fn update_last_run(
    settings: &Arc<std::sync::RwLock<freally_settings::Settings>>,

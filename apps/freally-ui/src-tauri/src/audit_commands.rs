...
use freally_settings::AuditSettings;
use serde::Serialize;
use sha2::Digest;
...
pub fn default_audit_path() -> PathBuf {
    ...
    PathBuf::from("freally-audit.log")
}
...
pub fn settings_fingerprint(settings: &freally_settings::Settings) -> String {
...
pub fn audit_verify(
    ...
    let report = freally_audit::verify_chain(&path, format).map_err(|e| e.to_string())?;
    Ok(AuditVerifyDto {
        total: report.total,
...
pub fn audit_verify_file(path: String, format: String) -> Result<AuditVerifyDto, String> {
    ...
    let report = freally_audit::verify_chain(&p, fmt).map_err(|e| e.to_string())?;
    Ok(AuditVerifyDto {
        total: report.total,
        matches: report.matches,
        mismatches: report.mismatches,
...
mod tests {
    ...
    fn fingerprint_changes_with_setting_change() {
        let mut a = freally_settings::Settings::default();

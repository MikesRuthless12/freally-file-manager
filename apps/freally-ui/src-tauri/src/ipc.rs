...
//! string.
//!
//! Kept free of engine types — `freally_core::JobKind` etc. are
...
use freally_core::{Job, JobKind, JobState, QueueId};
...
pub struct JobDto {
    pub id: u64,
    /// Phase 45.3 — id of the [`freally_core::Queue`] this job belongs
    /// to. The legacy single-queue path uses [`QueueId::DEFAULT`]
    /// (= `0`); registry-routed jobs carry the assigned id from
    /// [`freally_core::QueueRegistry::route`]. The Svelte tab strip
...
/// `freally --enqueue <verb> <paths…>`. Shares shape with
/// [`DropReceivedDto`] so the frontend can reuse its drop-staging
/// flow; adds `verb` so "Move with Freally" skips the radio.
#[derive(Debug, Clone, serde::Serialize)]
#[serde(rename_all = "camelCase")]
...
/// [`freally_history::JobSummary`] with camelCase field names and
...
/// [`freally_history::ItemRow`] for the IPC side.
#[derive(Debug, Clone, serde::Serialize)]
#[serde(rename_all = "camelCase")]
...
/// [`freally_history::HistoryFilter`] verbatim.
...
/// [`freally_history::Totals`] with serialisable types.
#[derive(Debug, Clone, serde::Serialize)]
#[serde(rename_all = "camelCase")]
...
/// Phase 8 — wire form of `freally_core::ErrorPolicy`. Tagged by
...
/// `freally_settings::Settings` with camelCase field naming so
...
/// Phase 39 — wire form of `freally_settings::RecoverySettings`.
...
impl From<freally_settings::RecoverySettings> for RecoveryDto {
    fn from(s: freally_settings::RecoverySettings) -> Self {
        Self {
            enabled: s.enabled,
...
impl From<RecoveryDto> for freally_settings::RecoverySettings {
...
/// Phase 48 — wire form of `freally_settings::ServerSettings`.
#[derive(Debug, Clone, Default, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Phase 48 — wire form of `freally_settings::ServerAuthSettings`.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
...
/// Phase 48 — wire form of `freally_settings::WebhookSettings`.
#[derive(Debug, Clone, Default, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
...
impl From<freally_settings::ServerSettings> for ServerDto {
    fn from(s: freally_settings::ServerSettings) -> Self {
        Self {
            webdav: s.webdav,
            http: s.http,
            s3: s.s3,
...
impl From<ServerDto> for freally_settings::ServerSettings {
    fn from(d: ServerDto) -> Self {
        Self {
            ...
            webhooks: d
                .webhooks
                .into_iter()
                .map(|w| freally_settings::WebhookSettings {
                    target: w.target,
                    url: w.url,
                    pushover_token: w.pushover_token,
                    pushover_user: w.pushover_user,
...
/// Phase 37 — wire form of `freally_settings::MobileSettings`.
#[derive(Debug, Clone, Default, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
...
impl From<freally_settings::MobileSettings> for MobileDto {
    fn from(s: freally_settings::MobileSettings) -> Self {
        Self {
...
impl From<MobileDto> for freally_settings::MobileSettings {
...
impl From<freally_settings::MobilePairingEntry> for MobilePairingEntryDto {
    fn from(s: freally_settings::MobilePairingEntry) -> Self {
        Self {
            label: s.label,
...
impl From<MobilePairingEntryDto> for freally_settings::MobilePairingEntry {
...
impl From<freally_settings::MobilePushTarget> for MobilePushTargetDto {
    fn from(s: freally_settings::MobilePushTarget) -> Self {
        match s {
            freally_settings::MobilePushTarget::Apns { token } => Self::Apns { token },
            freally_settings::MobilePushTarget::Fcm { token } => Self::Fcm { token },
            freally_settings::MobilePushTarget::StubEndpoint { url } => Self::StubEndpoint { url },
        }
    }
}

impl From<MobilePushTargetDto> for freally_settings::MobilePushTarget {
...
/// Phase 33 — wire form of `freally_settings::MountSettings`.
#[derive(Debug, Clone, Default, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct MountDto {
...
impl From<freally_settings::MountSettings> for MountDto {
    fn from(s: freally_settings::MountSettings) -> Self {
        Self {
...
impl From<MountDto> for freally_settings::MountSettings {
...
/// Phase 34 — wire form of `freally_settings::AuditSettings`. The
...
impl Default for AuditDto {
    fn default() -> Self {
        let d = freally_settings::AuditSettings::default();
        Self {
            enabled: d.enabled,
            format: d.format,
...
impl From<freally_settings::AuditSettings> for AuditDto {
    fn from(s: freally_settings::AuditSettings) -> Self {
        Self {
            enabled: s.enabled,
            format: s.format,
...
impl From<AuditDto> for freally_settings::AuditSettings {
...
/// Phase 35 — wire form of `freally_settings::CryptSettings`.
...
/// into typed `freally_crypt::EncryptionPolicy` /
...
impl Default for CryptDto {
    fn default() -> Self {
        let d = freally_settings::CryptSettings::default();
        Self {
...
impl From<freally_settings::CryptSettings> for CryptDto {
    fn from(s: freally_settings::CryptSettings) -> Self {
        Self {
...
impl From<CryptDto> for freally_settings::CryptSettings {
...
/// Phase 29 — wire form of `freally_settings::DndSettings`.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct DndDto {
...
impl Default for DndDto {
    fn default() -> Self {
        let defaults = freally_settings::DndSettings::default();
        Self {
...
/// Phase 30 — wire form of `freally_settings::PathTranslationSettings`.
...
impl Default for PathTranslationDto {
    fn default() -> Self {
        let d = freally_settings::PathTranslationSettings::default();
        Self {
            enabled: d.enabled,
...
/// Phase 31 — wire form of `freally_settings::PowerPoliciesSettings`.
...
impl Default for PowerPoliciesDto {
    fn default() -> Self {
        let d = freally_settings::PowerPoliciesSettings::default();
        Self {
            enabled: d.enabled,
...
impl From<&freally_settings::PowerRuleChoice> for PowerRuleDto {
    fn from(r: &freally_settings::PowerRuleChoice) -> Self {
        match r {
            freally_settings::PowerRuleChoice::Continue => PowerRuleDto::Continue,
            freally_settings::PowerRuleChoice::Pause => PowerRuleDto::Pause,
            freally_settings::PowerRuleChoice::Cap { bytes_per_second } => PowerRuleDto::Cap {
...
impl From<&PowerRuleDto> for freally_settings::PowerRuleChoice {
    fn from(r: &PowerRuleDto) -> Self {
        match r {
            PowerRuleDto::Continue => freally_settings::PowerRuleChoice::Continue,
            PowerRuleDto::Pause => freally_settings::PowerRuleChoice::Pause,
            PowerRuleDto::Cap { bytes_per_second } => freally_settings::PowerRuleChoice::Cap {
                bytes_per_second: *bytes_per_second,
            },
        }
    }
...
impl From<&freally_settings::ThermalRuleChoice> for ThermalRuleDto {
    fn from(r: &freally_settings::ThermalRuleChoice) -> Self {
        match r {
            freally_settings::ThermalRuleChoice::Continue => ThermalRuleDto::Continue,
            freally_settings::ThermalRuleChoice::Pause => ThermalRuleDto::Pause,
            freally_settings::ThermalRuleChoice::CapPercent { percent } => {
...
impl From<&ThermalRuleDto> for freally_settings::ThermalRuleChoice {
    fn from(r: &ThermalRuleDto) -> Self {
        match r {
            ThermalRuleDto::Continue => freally_settings::ThermalRuleChoice::Continue,
            ThermalRuleDto::Pause => freally_settings::ThermalRuleChoice::Pause,
            ThermalRuleDto::CapPercent { percent } => {
                freally_settings::ThermalRuleChoice::CapPercent {
...
pub struct GeneralDto {
    ...
    /// `"modal" | "drawer"` — see `freally_settings::ErrorDisplayMode`.
...
pub struct TransferDto {
    ...
    /// `freally_platform::DedupMode` wire string (`"auto-ladder"
    ...
    /// `freally_platform::HardlinkPolicy` wire string (`"never"
...
fn default_sharing_violation_base_delay_ms() -> u64 {
    freally_settings::defaults::default_sharing_violation_base_delay_ms()
}
...
/// Phase 15 — wire form of `freally_settings::UpdaterSettings`.
...
/// Phase 14a — TOML-friendly mirror of `freally_settings::FilterSettings`
...
/// Phase 21 — wire form of `freally_settings::NetworkSettings`.
///
...
/// Phase 21 — wire form of `freally_settings::AutoThrottleRule`.
...
/// Phase 19a — wire form of `freally_settings::ScanSettings`.
...
/// `freally_journal::UnfinishedJob` with camelCase field naming
...
pub struct SnapshotCreatedDto {
    ...
    /// `freally_snapshot::SnapshotKind::as_str`.
    pub kind: &'static str,
    pub original: String,
...
impl From<&freally_settings::Settings> for SettingsDto {
    fn from(s: &freally_settings::Settings) -> Self {
        Self {
            general: GeneralDto {
            ...
            advanced: AdvancedDto {
                ...
                error_policy: match s.advanced.error_policy {
                    freally_settings::ErrorPolicyChoice::Ask => ErrorPolicyDtoV2::Ask,
                    freally_settings::ErrorPolicyChoice::Skip => ErrorPolicyDtoV2::Skip,
                    freally_settings::ErrorPolicyChoice::Abort => ErrorPolicyDtoV2::Abort,
                    freally_settings::ErrorPolicyChoice::RetryN {
                        max_attempts,
                        backoff_ms,
...
fn auto_throttle_to_dto(r: freally_settings::AutoThrottleRule) -> AutoThrottleRuleDto {
    use freally_settings::AutoThrottleRule;
    match r {
...
fn auto_throttle_from_dto(r: AutoThrottleRuleDto) -> freally_settings::AutoThrottleRule {
    use freally_settings::AutoThrottleRule;
    match r {
...
fn parse_bandwidth_mode(s: &str) -> freally_settings::BandwidthMode {
    use freally_settings::BandwidthMode;
    match s {
...
impl SettingsDto {
    ...
    /// boundary crossing.
    pub fn into_settings(self) -> freally_settings::Settings {
        use freally_settings::LockedFilePolicyChoice;
        ...
        s.filters = freally_settings::FilterSettings {
        ...
        s.updater = freally_settings::UpdaterSettings {
            auto_check: self.updater.auto_check,
            channel: match self.updater.channel.as_str() {
                "beta" => freally_settings::UpdateChannel::Beta,
                _ => freally_settings::UpdateChannel::Stable,
            },
        ...
        s.scan = freally_settings::ScanSettings {
        ...
        s.network = freally_settings::NetworkSettings {
        ...
        s.dnd = freally_settings::DndSettings {
            spring_load_enabled: self.dnd.spring_load_enabled,
            spring_load_delay_ms: self.dnd.spring_load_delay_ms.clamp(
                freally_settings::DND_MIN_SPRING_MS,
                freally_settings::DND_MAX_SPRING_MS,
            ),
            show_drag_thumbnails: self.dnd.show_drag_thumbnails,
            highlight_invalid_targets: self.dnd.highlight_invalid_targets,
        };

        s.path_translation = freally_settings::PathTranslationSettings {
            enabled: self.path_translation.enabled,
            target_os: freally_settings::TargetOsChoice::from_wire(
                &self.path_translation.target_os,
            ),
            unicode_normalization: freally_settings::NormalizationModeChoice::from_wire(
                &self.path_translation.unicode_normalization,
            ),
            line_endings: freally_settings::LineEndingModeChoice::from_wire(
                &self.path_translation.line_endings,
            ),
            reserved_name_strategy: freally_settings::ReservedNameChoice::from_wire(
                &self.path_translation.reserved_name_strategy,
            ),
            long_path_strategy: freally_settings::LongPathChoice::from_wire(
                &self.path_translation.long_path_strategy,
            ),
            line_ending_allowlist: self.path_translation.line_ending_allowlist,
        };

        s.power = freally_settings::PowerPoliciesSettings {
...
impl From<&freally_settings::ProfileInfo> for ProfileInfoDto {
    fn from(p: &freally_settings::ProfileInfo) -> Self {
        Self {
            name: p.name.clone(),
...
fn verify_wire(v: freally_settings::VerifyChoice) -> String {
    use freally_settings::VerifyChoice;
    match v {
...
fn parse_verify(s: &str) -> freally_settings::VerifyChoice {
    use freally_settings::VerifyChoice;
    match s {
...
fn concurrency_wire(c: freally_settings::ConcurrencyChoice) -> String {
...
fn parse_concurrency(s: &str) -> freally_settings::ConcurrencyChoice {
    use freally_settings::ConcurrencyChoice;
...
fn reflink_wire(r: freally_settings::ReflinkPreference) -> String {
    use freally_settings::ReflinkPreference;
    match r {
...
fn parse_reflink(s: &str) -> freally_settings::ReflinkPreference {
    use freally_settings::ReflinkPreference;
    match s {
...
fn parse_shred(s: &str) -> freally_settings::ShredMethodChoice {
    use freally_settings::ShredMethodChoice;
    match s {
...
fn parse_log_level(s: &str) -> freally_settings::LogLevel {
    use freally_settings::LogLevel;
    match s {
        "off" => LogLevel::Off,

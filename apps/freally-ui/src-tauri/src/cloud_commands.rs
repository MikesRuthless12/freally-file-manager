...
//! the OS keychain via `freally_cloud::Credentials`. They never
...
/// written to the OS keychain under `freally-cloud/<name>`.
#[tauri::command]
pub fn add_backend(
    dto: BackendDto,
...
where
    F: std::future::Future<Output = Result<u64, freally_cloud::BackendError>> + Send + 'static,
{
...
mod tests {
    ...
    fn hydrate_registry_skips_invalid_rows() {
        ...
        let mut settings = freally_settings::Settings::default();

...
//!
//! The runner builds a [`freally_crypt::FreallyCryptHook`] from
...
//! `freally_core::copy_file` for the exclusion rationale).
//!
...
pub fn build_hook(settings: &CryptSettings) -> Result<Option<Arc<FreallyCryptHook>>, String> {
    ...
    Ok(Some(Arc::new(FreallyCryptHook::new(
        compression,
        encryption,
    ))))
}

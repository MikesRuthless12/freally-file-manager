...
//!
//! The pre-Phase-40 problem: `freally-ui.exe --enqueue copy …`
...
//! named pipe `\\.\pipe\freally-ui-enqueue`, write our argv as
...
const MUTEX_NAME: &str = "Local\\freally-ui-instance-mutex-v1";
const PIPE_NAME: &str = "\\\\.\\pipe\\freally-ui-enqueue-v1";
...
/// Returns `true` iff another `freally-ui` process is already
...
/// 32-byte secret in `%LOCALAPPDATA%\Freally\session.token`. If
...
/// `%LOCALAPPDATA%\Freally\session.token` BEFORE the listener
...
mod tests {
    ...
    fn forward_to_missing_pipe_errors_cleanly() {
        let argv = vec!["freally-ui.exe".to_string(), "--enqueue".to_string()];

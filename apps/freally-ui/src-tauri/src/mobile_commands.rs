...
//! `freally_settings::MobilePairingEntry` back through
//! `mobile_pair_commit`.

use std::sync::{Arc, RwLock};

use base64::Engine;
use freally_mobile::pairing::{PairingToken, generate_qr_png, mint_desktop_keypair, mint_peer_id};
...
pub async fn mobile_pair_commit(
    ...
    device_label: String,
    push_target: Option<freally_settings::MobilePushTarget>,
    ...
    let snapshot = {
        ...
        settings
            .mobile
            .pairings
            .push(freally_settings::MobilePairingEntry {
                label: device_label,
...
pub async fn mobile_handle_remote_command(
    ...
    // session.
    use freally_mobile::server::RemoteControl;
...
struct AppStateProxy {
    ...
    wake_lock: Arc<std::sync::Mutex<Option<freally_platform::WakeLock>>>,
...
impl freally_mobile::server::RemoteControl for AppStateRemoteControl {
    ...
    async fn recent_history(&self, _limit: u32) -> Result<Vec<HistoryRow>, String> {
        // History rows surface through `freally_history::History`
    ...
    async fn set_keep_awake(&self, enabled: bool) -> Result<(), String> {
        ...
        // `freally_platform::wake_lock`. Acquire on `enabled =
...
struct UnusedShimToKeepWireDtoInScope(freally_settings::MobilePushTarget);

// ---------------------------------------------------------------------
// Queue snapshot helpers (Phase 37 follow-up #2)
// ---------------------------------------------------------------------

/// Convert a `freally_core::Job` snapshot into the wire
/// `JobSummary` the PWA renders.
fn job_to_summary(job: freally_core::Job) -> JobSummary {
    let kind = match job.kind {
        freally_core::JobKind::Copy => "copy",
        freally_core::JobKind::Move => "move",
        freally_core::JobKind::Delete => "delete",
        freally_core::JobKind::SecureDelete => "secure-delete",
        freally_core::JobKind::Verify => "verify",
    };
    let state = match job.state {
        freally_core::JobState::Pending => "pending",
        freally_core::JobState::Running => "running",
        freally_core::JobState::Paused => "paused",
        freally_core::JobState::Cancelled => "cancelled",
        freally_core::JobState::Succeeded => "completed",
        freally_core::JobState::Failed => "failed",
    };
    let rate_bps = job
        .started_at
        .map(|s| {

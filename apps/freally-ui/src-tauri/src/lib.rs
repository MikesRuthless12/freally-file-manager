...
//!   (`freally --enqueue <verb> <paths…>`). Stable across Phase 7
...
//!   [`freally_core::CopyEvent`] onto the Tauri event bus, and keeps
...
/// removed (Phase 45.6).
pub const TRAY_ICON_ID: &str = "freally-main-tray";
...
/// (`location_override`, else [`freally_chunk::default_chunk_store_path`]),
/// open it **once**, and build the unified [`freally_chunk::Repository`]
...
/// `RepoCompression` (Phase 49h). Lives here because `freally-settings`
/// has no `freally-chunk` edge.
pub(crate) fn compression_from_settings(
    c: &freally_settings::RepoCompressionSettings,
) -> freally_chunk::RepoCompression {
    use freally_chunk::{CompressionLevel, RepoCompression};
    use freally_settings::RepoCompressionSettings as S;
    match *c {
...
fn open_repository_blocking(
    ...
    // would block the CLI (`freally migrate`/`export`) against the same
    ...
    let repository = match freally_chunk::Repository::with_store(store.clone()) {
        Ok(mut r) => {
...
pub fn run() {
    ...
    // silently dropped. Filter is `FREALLY_LOG`-controlled (defaults
    ...
    // mutex is already owned, a sibling `freally-ui` is already
        ...
        builder = builder.plugin(tauri_plugin_single_instance::init(
            |app: &tauri::AppHandle, argv: Vec<String>, _cwd: String| {
                ...
                match cli::parse_args(os_argv) {
                    ...
                    Err(err) => {
                        eprintln!("freally: second invocation rejected: {err}");
                    }
                }
            },
        ));
    ...
    let profiles = match freally_settings::ProfileStore::default_store() {
        Ok(s) => s,
        Err(e) => {
            eprintln!("freally: profiles store init failed: {e}");
            freally_settings::ProfileStore::new(std::path::PathBuf::new())
        }
    };
    ...
    let app_state = match freally_journal::Journal::open_default() {
        ...
        Err(e) => {
            eprintln!("freally: journal open failed: {e}");
            app_state
        }
    };
    ...
    builder
        ...
        .setup(move |app| {
            ...
            // `freally-secure-delete::shred_file` refuses honestly
            ...
            if let Some(state) = app.handle().try_state::<AppState>() {
                ...
                match registry.load() {
                    ...
                    Err(e) => {
                        eprintln!("freally: dropstack load failed: {e}");
                    }
                }
            }
            ...
            if let Some(state) = app.handle().try_state::<AppState>() {
                ...
                let probes = freally_power::ProbeSet::production();
                ...
                let _poller = tauri::async_runtime::block_on(async {
                    app_state
                        .power_bus
                        .spawn_poller(probes, freally_power::bus::DEFAULT_POLL_PERIOD)
                });
...
/// subscriber once. Honours `FREALLY_LOG` (e.g. `FREALLY_LOG=debug`,
/// `FREALLY_LOG=freally_core=trace,info`) and falls back to `info`
...
fn init_tracing_subscriber() {
    ...
    let filter = EnvFilter::try_from_env("FREALLY_LOG").unwrap_or_else(|_| EnvFilter::new("info"));
...
fn has_jobs_in_progress(state: &AppState) -> bool {
    use freally_core::JobState;
    fn any_active(q: &freally_core::Queue) -> bool {
...
fn open_history_blocking() -> Option<freally_history::History> {
...
fn load_settings_blocking() -> (freally_settings::Settings, std::path::PathBuf) {
    match freally_settings::Settings::default_path() {
        Ok(path) => {
            let settings = freally_settings::Settings::load_from(&path).unwrap_or_else(|e| {
                eprintln!("freally: settings load failed ({e}); falling back to defaults");
                freally_settings::Settings::default()
            });
            (settings, path)
        }
        Err(e) => {
            eprintln!("freally: settings path resolution failed: {e}");
            (
                freally_settings::Settings::default(),

...
//! window context menu, CLI `freally stack add`, drag onto the
...
pub async fn dropstack_copy_all_to(
    ...
    dispatch_all(app, state, destination, freally_core::JobKind::Copy).await
}
...
pub async fn dropstack_move_all_to(
    ...
    dispatch_all(app, state, destination, freally_core::JobKind::Move).await
}
...
async fn dispatch_all(
    ...
    let copy_opts = freally_core::CopyOptions {
        buffer_size: settings.transfer.effective_buffer_size(),
        verify: None,
        ..Default::default()
    };
    let verifier = None;
    let collision_policy = freally_core::CollisionPolicy::Prompt;
    let error_policy = freally_core::ErrorPolicy::default();

    let srcs: Vec<PathBuf> = paths.iter().map(PathBuf::from).collect();
    for src in &srcs {
        if let Err(e) = freally_core::validate_path_no_traversal(src) {

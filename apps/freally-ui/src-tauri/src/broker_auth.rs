...
//! pipe `\\.\pipe\freally-ui-enqueue-v1`.
//!
...
//! file under `%LOCALAPPDATA%\Freally\session.token`. A second
...
//! file is the trust anchor every same-user freally-ui binary
//! agrees on.
//!
...
//! a freally-ui binary from forging messages: a hostile process
...
/// Filename under `%LOCALAPPDATA%\Freally\` that holds the raw 32
...
/// Resolve `%LOCALAPPDATA%\Freally\session.token`. We deliberately
/// use `BaseDirs::data_local_dir()` rather than `ProjectDirs` so
/// the file lives directly under `LocalAppData\Freally\` and is
...
fn token_path() -> Result<PathBuf, String> {
    ...
    let dir = base.data_local_dir().join("Freally");
...
/// parent directory `%LOCALAPPDATA%\Freally\` is already
...
mod tests {
    ...
    fn hmac_round_trip_verifies() {
        let secret = [7u8; 32];
        let payload = br#"{"argv":["freally-ui.exe","--enqueue"]}"#;

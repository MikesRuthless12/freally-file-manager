...
//!
//! Calls the REAL `freally_ui_lib::elevate::elevated_retry`, so the
//! genuine OS consent prompt fires — UAC `Start-Process -Verb RunAs`
//! on Windows, `pkexec` on Linux, `osascript … with administrator
//! privileges` on macOS — and the elevated `freally-helper` performs
...
async fn main() {
    ...
    match freally_ui_lib::elevate::elevated_retry(&src, &dst).await {
        Ok(resp) => {

...
<script lang="ts">
  ...
  let sortMode: SortMode = $state(
    (typeof localStorage !== "undefined"
      ? (localStorage.getItem("freally-sort-mode") as SortMode | null)
      : null) ?? "custom",
  );
  ...
  const COLLISION_KEY = "freally-collision-policy";
  ...
  function applySort(mode: SortMode) {
    sortMode = mode;
    try {
      localStorage.setItem("freally-sort-mode", mode);
    } catch {
  ...
  function moveIdx(from: number, to: number) {
    ...
    sortMode = "custom";
    try {
      localStorage.setItem("freally-sort-mode", "custom");
    } catch {
      // ignore
    }
  ...
  const RESERVE_BYTES = (() => {
    try {
      const raw = localStorage.getItem("freally-reserve-bytes");

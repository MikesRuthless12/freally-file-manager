...
const activityOpenStore = writable<boolean>(
  typeof localStorage !== "undefined"
    ? localStorage.getItem("freally-activity-open") !== "false"
    : true,
);
...
const activitySortStore = writable<ActivitySortMode>(
  (typeof localStorage !== "undefined"
    ? (localStorage.getItem("freally-activity-sort") as ActivitySortMode | null)
    : null) ?? "insertion",
);
...
export function setActivitySort(mode: ActivitySortMode): void {
  ...
  try {
    localStorage.setItem("freally-activity-sort", mode);
  } catch {
...
export function toggleActivity(): void {
  activityOpenStore.update((v) => {
    const next = !v;
    try {
      localStorage.setItem("freally-activity-open", next ? "true" : "false");
    } catch {
      // Best-effort — non-fatal if storage rejects.
    }
    return next;

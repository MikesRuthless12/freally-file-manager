//! Bridge from [`ChunkStore`] to [`freally_core::ChunkStoreSink`].
//!
//! The core engine holds a `dyn ChunkStoreSink` on
//! `CopyOptions::chunk_store` so it can consult the store without a
//! direct dep on redb / fastcdc / blake3. [`FreallyChunkSink`] is
...
#[derive(Debug, Clone)]
pub struct FreallyChunkSink {
    inner: Arc<ChunkStore>,
}

impl FreallyChunkSink {
...
impl ChunkStoreSink for FreallyChunkSink {
...
mod tests {
    ...
    fn sink_round_trips_manifest() {
        ...
        let sink = FreallyChunkSink::new(store);
        let m = Manifest {
            file_hash: [1u8; 32],
            size: 100,
            chunks: vec![],

//! Phase 32b — `FreallyCloudSink` adapter.
//!
//! Implements [`freally_core::CloudSink`] on top of an
//! [`crate::OperatorTarget`] so `CopyOptions::cloud_sink` can be
//! populated without `freally-core` pulling in OpenDAL.
//!
//! The contract is synchronous / blocking ([`freally_core::CloudSink::put_blocking`]).
...
/// negligible overhead.
pub struct FreallyCloudSink {
    target: Arc<dyn CopyTarget>,
}

impl std::fmt::Debug for FreallyCloudSink {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("FreallyCloudSink")
            .field("backend_name", &self.target.name())
            .finish()
    }
}

impl FreallyCloudSink {
...
impl CloudSink for FreallyCloudSink {
    ...
    fn put_stream_blocking(
        ...
        let thread_name = format!("freally-cloud-stream-{}", self.target.name());
        ...
        match result {
            ...
            Ok(total) => {
                ...
                let _ = Self::run_on_dedicated_thread(
                    format!("freally-cloud-stream-cleanup-{}", self.target.name()),
                    move |rt| {
                        rt.block_on(async move {
...
mod tests {
    ...
    fn sink_puts_bytes_through_local_fs_target() {
        ...
        let sink = FreallyCloudSink::new(target).expect("sink");

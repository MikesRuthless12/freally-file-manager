...
//! service-account access and run `freally copy` (Phase 36 CLI) under
//! that role.
//!
//! # Threat model
//!
...
//! - The `freally` binary version pinned by the template is a
...
/// destruct watchdog, the workspace's compile-time `freally` version
...
pub struct OffloadOpts {
    ...
    /// - AWS: instance-profile name (`freally-offload-role`).
    /// - Azure: managed-identity resource ID
    ///   (`/subscriptions/.../identities/freally-offload`).
    /// - GCP: service-account email
    ///   (`freally-offload@my-project.iam.gserviceaccount.com`).
    pub iam_role: String,
...
impl Default for OffloadOpts {
    fn default() -> Self {
        Self {
            job_name: "freally-offload".to_string(),
            freally_release: Self::default_release().to_string(),
            instance_size: "t3.small".to_string(),
            region: "us-east-1".to_string(),
            iam_role: "freally-offload-role".to_string(),
...
impl OffloadOpts {
    /// Default `freally` release tag baked at compile time.
...
pub fn render_cloudinit_template(src: &Backend, dst: &Backend, opts: &OffloadOpts) -> String {
    ...
    format!(
        "#cloud-config
# freally-file-manager — cloud-VM offload, generic cloud-init format.
# Job: {job}
# Source: {src_label}
...
write_files:
  - path: /usr/local/bin/freally-offload.sh
    permissions: '0755'
    content: |
      #!/bin/sh
      set -eu
...
runcmd:
  - [ /usr/local/bin/freally-offload.sh ]

power_state:
  delay: 'now'
  mode: poweroff
  message: 'Freally offload watchdog'
...
pub fn render_aws_terraform(src: &Backend, dst: &Backend, opts: &OffloadOpts) -> String {
    ...
    format!(
        "# freally-file-manager — AWS cloud-VM offload Terraform snippet.
...
resource \"aws_instance\" \"freally_offload_{job}\" {{
  ami                    = data.aws_ami.amazon_linux_2023.id
  instance_type          = \"{size}\"
  iam_instance_profile   = \"{role}\"
  associate_public_ip_address = true
  tags = {{
    Name      = \"freally-offload-{job}\"
    ManagedBy = \"freally\"
  }}
  user_data = <<-EOT
...
pub fn render_az_arm(src: &Backend, dst: &Backend, opts: &OffloadOpts) -> String {
    ...
    // a direct dependency of freally-cloud (Phase 32h Azure ETag
  ...
  \"metadata\": {{
    \"_generator\": \"freally-file-manager cloud-VM offload helper\",
    \"job\": \"{job}\",
    \"src\": \"{src_label}\",
    \"dst\": \"{dst_label}\"
  }},
  \"resources\": [
    {{
      ...
      \"properties\": {{
        ...
        \"osProfile\": {{
          \"computerName\": \"freally-offload-{job}\",
          \"adminUsername\": \"freally\",
...
/// manager deployments create freally-offload --config <file>`.
pub fn render_gcp_deployment(src: &Backend, dst: &Backend, opts: &OffloadOpts) -> String {
    ...
    let indented = cloudinit
        ...
        .join("\n");
    format!(
        "# freally-file-manager — GCP cloud-VM offload Deployment Manager template.
# Job: {job}
# Region: {region}
...
/// Render the `freally copy` command line that the cloud-init script
...
fn render_copy_command(src: &Backend, dst: &Backend) -> String {
    ...
    match (src.kind, dst.kind) {
        ...
        | (BackendKind::B2, BackendKind::S3) => {
            format!(
                "aws s3 cp --recursive '{src_uri}' '{dst_uri}' && freally copy '{src_uri}' '{dst_uri}'"
            )
        }
        ...
        (BackendKind::AzureBlob, BackendKind::AzureBlob) => {
            format!(
                "az storage blob copy --source-uri '{src_uri}' --destination-blob \
                 ./tmp.bin --destination-container offload-tmp && freally copy \
                 '{src_uri}' '{dst_uri}'"
            )
        }
        ...
        (BackendKind::Gcs, BackendKind::Gcs) => {
            format!(
                "gsutil cp -r '{src_uri}' '{dst_uri}' && freally copy '{src_uri}' \
                 '{dst_uri}'"
            )
        }
        // Cross-cloud: defer entirely to the Freally CLI which knows
        // every backend type and runs the copy through OpenDAL.
        _ => format!("freally copy '{src_uri}' '{dst_uri}'"),
    }
}
...
/// strings (`job_name`, `freally_release`, `region`, `iam_role`,
...
mod tests {
    ...
    #[test]
    fn cross_cloud_pair_uses_freally_cli() {
        let src = s3_backend("src", "bucket-a");

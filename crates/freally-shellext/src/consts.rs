...
pub const VERB_COPY: &str = "Freally.Copy";
...
pub const PROG_COPY: &str = "Freally File Manager v1.0.0 — Copy command";

/// See [`PROG_COPY`].
pub const PROG_MOVE: &str = "Freally File Manager v1.0.0 — Move command";

/// Name of the target binary the verbs invoke. Resolved via PATH;
/// Phase 16 packaging puts `freally.exe` there (on Windows today,
/// the Tauri bin ships as `freally-ui.exe`; the MSI installer
/// symlinks / renames it at install time).
pub const HOST_BIN: &str = "freally";

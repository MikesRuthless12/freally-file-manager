...
/// The set of verification algorithms Freally File Manager v1.0.0 supports.
///
...
impl HashAlgorithm {
    ...
    /// Produce a `freally_core::Verifier` for this algorithm. The
    /// returned verifier is cheap to clone (it wraps an `Arc`) and
    /// suitable for stashing in `CopyOptions::verify`.
    pub fn verifier(self) -> freally_core::Verifier {
        freally_core::Verifier::new(self.name(), move || self.hasher())
    }
}

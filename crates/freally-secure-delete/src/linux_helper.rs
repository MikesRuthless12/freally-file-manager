...
//! privileged broker (`freally-helper`) or instructs the user to
...
impl SanitizeHelper for LinuxSanitizeHelper {
    ...
    /// `freally-platform` to keep the secret out of argv.
    ///
    ...
    fn run_opal_psid_revert_blocking(&self, device: &Path, psid: &str) -> Result<(), String> {
        ...
        freally_platform::linux_make_child_undumpable(&mut cmd);
        let output = cmd
            .output()

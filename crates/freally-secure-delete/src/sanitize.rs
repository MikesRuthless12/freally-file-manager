...
//! (`freally-helper`) is the broker: this crate hands it a typed
...
//!   Real implementations will live in `freally-platform` (Windows
//!   `DeviceIoControl(IOCTL_STORAGE_SECURITY_PROTOCOL_OUT)`) and a
//!   future `freally-helper` Linux binary (shells out to
...
/// command. Real impls live in `freally-platform` (Windows DeviceIoControl)
/// and a future `freally-helper` (Linux nvme-cli / hdparm shell-outs);
...
impl SanitizeHelper for NoopSanitizeHelper {
    ...
    fn run_sanitize_blocking(
        ...
        Err(
            "no privileged sanitize helper installed; install freally-helper (Linux/macOS) \
...
/// CLI wires `freally_platform::is_cow_filesystem` here at app
/// startup:
///
/// ```ignore
/// freally_secure_delete::set_cow_probe(
///     freally_platform::is_cow_filesystem,
/// );
/// ```
///
...
/// native probe (`freally_platform::is_cow_filesystem`), this

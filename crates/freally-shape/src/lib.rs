//! `freally-shape` — bandwidth shaping with GCRA token bucket,
...
//!    to every [`freally_core::CopyOptions`] via the [`ShapeSink`]
//!    bridge in [`FreallyShapeSink`]. Its [`Shape::set_rate`] method
...
//! # Engine wiring
//!
//! [`FreallyShapeSink`] wraps an `Arc<Shape>` and implements the
//! `freally_core::ShapeSink` trait-object boundary. The trait object
//! sits on `CopyOptions::shape`; with `shape = None`, the engine's
//! existing code path runs untouched.
//!
//! [`ShapeSink`]: freally_core::ShapeSink

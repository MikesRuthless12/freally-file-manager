//! Wire shape between `freally-ui` and the elevated helper.
//!
...
/// `freally-shellext` crate already ships — the helper just
...
mod tests {
    ...
    fn pipe_name_round_trip() {
        let n = generate_pipe_name(r"\\.\pipe\freally-helper-").unwrap();
        let suffix = parse_pipe_name(r"\\.\pipe\freally-helper-", &n).unwrap();
    ...
    fn pipe_name_rejects_wrong_shape() {
        // Wrong prefix.
        assert!(parse_pipe_name(r"\\.\pipe\freally-helper-", "totally-different").is_none());
        // Right prefix, wrong-length suffix.
        let too_short = format!(r"\\.\pipe\freally-helper-{}", "ab");
        assert!(parse_pipe_name(r"\\.\pipe\freally-helper-", &too_short).is_none());
        // Right prefix + right length but non-hex characters.
        let non_hex = format!(r"\\.\pipe\freally-helper-{}", "z".repeat(64));
        assert!(parse_pipe_name(r"\\.\pipe\freally-helper-", &non_hex).is_none());
    }
}

...
use freally_core::safety::no_follow_open_flags;
use freally_core::validate_path_no_traversal;
...
fn handle_elevated_retry(src: &std::path::Path, dst: &std::path::Path) -> Response {
    ...
    match copy_no_follow(src, dst) {
        ...
        Err(e) => {
            let key = if freally_core::safety::is_no_follow_rejection(&e) {
                "err-path-escape"
            } else {
                match e.kind() {
...
fn handle_install_shell_extension(target: ShellExtensionKind) -> Response {
    ...
    Response::ShellExtensionUnsupported {
        target,
        reason: format!(
            "{} install scaffolded but not yet implemented — the per-OS body fill (Phase 17d follow-up) lands alongside the shell-extension manifest changes in `crates/freally-shellext/`",
            target.wire_label()
        ),
    }
}
...
fn handle_uninstall_shell_extension(target: ShellExtensionKind) -> Response {
    ...
    Response::ShellExtensionUnsupported {
        target,
        reason: format!(
            "{} uninstall scaffolded but not yet implemented — the per-OS body fill (Phase 17d follow-up) lands alongside `crates/freally-shellext/`",
            target.wire_label()
        ),
    }
}

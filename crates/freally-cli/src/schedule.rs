...
//!
//! `freally schedule <SPEC>` reads an absolute-pathed `[job]`
...
use freally_core::{PathSafetyError, validate_path_no_traversal};
use thiserror::Error;
...
pub fn render_schedule(
    ...
    let suggested_install_path = match host {
        ScheduleHostOs::Windows => PathBuf::from(r"freally-schedule-stanza.bat"),
...
fn render_schtasks(spec: &JobSpec, spec_path: &Path, schedule: &ScheduleSpec) -> String {
    ...
    let cmd = format!(
        "{} apply --spec {}",
        quote_for_schtasks(Path::new("freally.exe")),
...
fn render_launchd_plist(spec: &JobSpec, spec_path: &Path, schedule: &ScheduleSpec) -> String {
    ...
    format!(
        "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<!DOCTYPE plist PUBLIC \"-//Apple//DTD PLIST 1.0//EN\" \"http://www.apple.com/DTDs/PropertyList-1.0.dtd\">\n<plist version=\"1.0\">\n<dict>\n    <key>Label</key>\n    <string>app.freally.scheduled-job</string>\n    <key>ProgramArguments</key>\n    <array>\n        <string>/usr/local/bin/freally</string>\n        <string>apply</string>\n        <string>--spec</string>\n        <string>{spec_path}</string>\n    </array>\n{interval_xml}\n</dict>\n</plist>\n",
...
fn render_systemd_user(spec: &JobSpec, spec_path: &Path, schedule: &ScheduleSpec) -> String {
    ...
    format!(
        "# ===== ~/.config/systemd/user/freally-scheduled-job.service =====\n[Unit]\nDescription=Freally File Manager scheduled job\n\n[Service]\nType=oneshot\nExecStart={exec}\n\n# ===== ~/.config/systemd/user/freally-scheduled-job.timer =====\n{note_block}[Unit]\nDescription=Freally File Manager scheduled job timer\n\n[Timer]\nOnCalendar={on_calendar}\nPersistent=true\nUnit=freally-scheduled-job.service\n\n[Install]\nWantedBy=timers.target\n"
    )
}

//! `freally-cli` — Phase 36 command-line surface for Freally File Manager.
//!
//! Goal: a `freally` binary suitable for CI/CD pipelines, automation
...
//! | Command           | Status                                   |
//! | ----------------- | ---------------------------------------- |
//! | `copy`            | wired to `freally_core::copy_file/copy_tree` |
//! | `move`            | wired to `freally_core::move_file/move_tree` |
//! | `verify`          | wired to `freally_hash::hash_file_async`     |
//! | `version`         | JSON-serialisable build metadata              |
//! | `config`          | `freally_settings` get / set / reset         |
...
/// Parse `std::env::args_os()` and dispatch. Used by the `freally`

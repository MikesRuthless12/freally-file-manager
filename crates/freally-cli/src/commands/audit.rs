//! `freally audit verify <log-file>` — staged for IPC plumbing.
//!
//! The chain-verification primitive (`freally_audit::verify_chain`)
...
//! the Phase 36 follow-up that imports `freally-audit` against this
//! crate.
//!
    ...
    let _ = writer.emit(JsonEventKind::Info {
        message: format!(
            "{summary} parsed; CLI follow-up will plumb \
             `freally_audit::verify_chain` through the same exit-code surface as `verify`."
        ),
    });

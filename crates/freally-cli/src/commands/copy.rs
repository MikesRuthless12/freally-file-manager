...
pub(crate) async fn run(
    ...
    let shape_sink = if let Some(rate) = &args.shape {
        match parse_byte_rate(rate) {
            Ok(bps) => {
                ...
                let shape = std::sync::Arc::new(freally_shape::Shape::new(Some(
                    freally_shape::ByteRate::new(bps),
                )));
                Some(
                    std::sync::Arc::new(freally_shape::FreallyShapeSink::new(shape))
                        as std::sync::Arc<dyn freally_core::ShapeSink>,
                )
            }
            Err(e) => {
                let _ = writer.emit(JsonEventKind::Error {
    ...
    for src in &sources {
        ...
        let fast_copy_hook: std::sync::Arc<dyn freally_core::FastCopyHook> =
            std::sync::Arc::new(freally_platform::PlatformFastCopyHook);
        ...
        if let Some(algo_name) = &args.verify {
            match algo_name.parse::<freally_hash::HashAlgorithm>() {
                Ok(algo) => {
        ...
        let result: Result<(), freally_core::CopyError> = if src_path.is_dir() {
            if is_move {
                let mv = MoveOptions {
...
fn classify_engine_error(e: &freally_core::CopyError) -> ExitCode {
    use freally_core::CopyErrorKind as K;
    match e.kind {

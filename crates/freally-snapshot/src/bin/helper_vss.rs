//! `freally-helper-vss` — elevated Windows Volume Shadow Copy helper.
//!
//! The main `freally-ui` process spawns this binary via
...
//! protocol defined in `freally_snapshot::rpc`.
//!
//! Wire:
...
#[cfg(not(windows))]
fn main() {
    eprintln!("freally-helper-vss is Windows-only");
    std::process::exit(2);
}
...
fn main() {
    ...
    let (rpc_stdin, rpc_stdout) = match (rpc_stdin, rpc_stdout) {
        ...
        _ => {
            eprintln!(
                "usage: freally-helper-vss --rpc-stdin=<pipe> --rpc-stdout=<pipe>\n\
                 (invoked by the main freally process; not a user-facing tool)"
            );
            std::process::exit(2);
        }
    };
        ...
        .build()
    {
        Ok(r) => r,
        Err(e) => {
            eprintln!("freally-helper-vss: tokio runtime init failed: {e}");
            std::process::exit(3);
        }
    };
...
#[cfg(windows)]
mod windows_main {
    use freally_snapshot::rpc::{self, Request, Response};
    ...
    pub(super) async fn run(in_pipe: String, out_pipe: String) -> i32 {
        let (reader, writer) = match connect_pipes(&in_pipe, &out_pipe).await {
            Ok(pair) => pair,
            Err(e) => {
                eprintln!("freally-helper-vss: pipe connect failed: {e}");
                return 4;
            }
        };
        ...
        loop {
            ...
            let n = match reader.read_line(&mut line).await {
                Ok(n) => n,
                Err(e) => {
                    eprintln!("freally-helper-vss: read_line: {e}");
                    break;
                }
            };
            if n == 0 {
            ...
            let req: Request = match serde_json::from_str(line.trim()) {
                ...
                Err(e) => {
                    ...
                    if consecutive_bad >= MAX_BAD {
                        eprintln!(
                            "freally-helper-vss: {consecutive_bad} consecutive malformed requests, exiting"
                        );
                        break;
                    }
                    continue;
    ...
    fn create_shadow(volume: &str) -> Result<(String, String), String> {
        ...
        freally_snapshot::vss_com::create_shadow_via_com(volume)
    }
    ...
    fn release_shadow(shadow_id: &str) -> Result<(), String> {
        ...
        freally_snapshot::vss_com::release_shadow_via_com(shadow_id)
    }

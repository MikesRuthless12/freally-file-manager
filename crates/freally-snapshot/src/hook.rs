//! `freally-core::SnapshotHook` implementation.
//!
...
//! [`FreallySnapshotHook`] to every `CopyOptions` where
//! `on_locked == Snapshot`.
...
use freally_core::{CopyError, CopyErrorKind, SnapshotGuard, SnapshotHook, SnapshotLease};
...
/// `Arc::new(FreallySnapshotHook::default())`.
#[derive(Debug, Default)]
pub struct FreallySnapshotHook;

impl FreallySnapshotHook {
    pub fn new() -> Self {
        Self
    }
}

impl SnapshotHook for FreallySnapshotHook {
    fn open_for_read<'a>(
        &'a self,
        src: PathBuf,

//! `freally-snapshot` — filesystem-snapshot source for locked files.
//!
...
//! | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------|
//! | [`SnapshotKind::Vss`]  | Windows Volume Shadow Copy Service. The privileged work runs in a separate `freally-helper-vss` binary that the main process spawns via `ShellExecute("runas", ...)`. The helper speaks JSON-RPC over stdin/stdout so the main app never needs a UAC prompt of its own. |
//! | [`SnapshotKind::Zfs`]  | `zfs snapshot <dataset>@freally-<uuid>` + `zfs mount`. Cleanup unmounts and destroys.                                                                               |
...
//!
//! ```no_run
//! use freally_snapshot::{create_snapshot, translate_path};
//! use std::path::Path;
//!
//! # async fn demo() -> Result<(), freally_snapshot::SnapshotError> {
...
/// sibling `freally-helper-vss` binary can call the same shape the
...
pub use hook::FreallySnapshotHook;
    ...
    fn drop(&mut self) {
        if let Some(c) = self.cleanup.take() {
            ...
            if let Err(e) = backends::release_blocking(c) {
                tracing::warn!("freally-snapshot: cleanup failed in Drop: {e}");
            }
        }
    }
}
...
/// reachable at all.
///
/// [engine-policy]: https://github.com/MikesRuthless12/freally-file-manager "CopyOptions::on_locked"
...
///   main process spawns `freally-helper-vss.exe` with the `runas`
...
mod tests {
    ...
    fn translate_path_identity_at_root() {
        ...
        assert_eq!(
            ...
            Some(PathBuf::from("/snap/freally-abc"))
        );
    }
}

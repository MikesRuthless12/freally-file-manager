...
//!   `freally-helper-vss.exe` via `Start-Process -Verb RunAs`, which
...
async fn spawn_helper() -> Result<HelperConn, SnapshotError> {
    ...
    // enumerate `\\.\pipe\freally-vss-*` and race the legitimate
...
fn locate_helper_binary() -> Result<PathBuf, SnapshotError> {
    ...
    exe.pop();
    let helper = exe.join("freally-helper-vss.exe");
    if helper.exists() {
        Ok(helper)
    } else {
        Err(SnapshotError::BackendMissing {
            tool: "freally-helper-vss.exe",
        })
    }
}
...
fn make_pipe_names() -> (String, String) {
    ...
    // randomness) so an attacker enumerating `\\.\pipe\freally-vss-*`
    ...
    (
        format!(r"\\.\pipe\freally-vss-{id}-in"),
        format!(r"\\.\pipe\freally-vss-{id}-out"),
    )
}

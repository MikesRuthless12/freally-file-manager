...
//!   freeze their journals before the snapshot fires. Freally
...
mod tests {
    ...
    ///
    /// ```bash
    /// cargo test -p freally-snapshot --features vss-com -- --ignored
    /// ```
    #[test]
    #[ignore = "requires Administrator + Windows VSS service"]
    fn vss_com_create_release_round_trip() {
        let volume =
            std::env::var("FREALLY_VSS_TEST_VOLUME").unwrap_or_else(|_| r"C:\".to_string());
    ...
    /// `FREALLY_VSS_LOCKED_FILE_PATH` exclusively open, this test
    ...
    /// to `FREALLY_VSS_DEST_DIR` and byte-compare.
    ///
    ...
    fn vss_com_copy_locked_file_via_shadow() {
        ...
        let locked = std::env::var("FREALLY_VSS_LOCKED_FILE_PATH")
            .expect("FREALLY_VSS_LOCKED_FILE_PATH must be set");
        let dest_dir =
            std::env::var("FREALLY_VSS_DEST_DIR").expect("FREALLY_VSS_DEST_DIR must be set");

        let locked_path = PathBuf::from(&locked);
        assert!(
            locked_path.is_absolute(),
            "FREALLY_VSS_LOCKED_FILE_PATH must be absolute: {locked:?}"
        );

...
//! sample plugins under `apps/freally-ui/plugins/`.
//!
...
//! `cargo test -p freally-plugin`. CI runs `xtask
...
use freally_plugin::{Capability, CapabilityGrant, HookCtx, HookKind, HookOutcome, PluginHost};
use serde_json::json;

/// Resolve `<repo_root>/apps/freally-ui/plugins/<dir>/target/wasm32-unknown-unknown/release/<crate_underscored>.wasm`.
...
fn sample_wasm(dir_name: &str, crate_underscored: &str) -> Option<PathBuf> {
    ...
    // CARGO_MANIFEST_DIR is `<repo>/crates/freally-plugin`; back up
...
async fn notify_ntfy_emits_topic_envelope() {
    ...
    let ctx = HookCtx {
        ...
        data: json!({
            ...
            "elapsed_ms": 4_500u64,
            "ntfy_topic_url": "https://ntfy.sh/freally-mike",
            "priority": "high"
        }),
    };
    let outcome = handle
    ...
    assert_eq!(envelope["url"], "https://ntfy.sh/freally-mike");

...
//! `tracing::warn!(target: "freally::audit", ...)` calls inside the
//! rest of the workspace — auditors occasionally want those in the
//! log too. Events without the `freally::audit` target are ignored
...
/// `freally::audit` into the owned [`AuditSink`].
    ...
    S: Subscriber,
{
    fn on_event(&self, event: &Event<'_>, _ctx: Context<'_, S>) {
        ...
        if !metadata.target().starts_with("freally::audit") {
            return;
        }
...
mod tests {
    ...
    fn tracing_event_reaches_the_sink() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::warn!(target: "freally::audit", "unauthorized-attempt");
        });

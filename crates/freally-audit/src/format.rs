...
/// Syslog RFC 5424 `APP-NAME` identifier. `freally@32473` is the
...
fn syslog_structured_data(event: &AuditEvent, prev_hash_hex: &str) -> String {
    // `freally@32473` — see `syslog_app_name()` doc. The params
    ...
    let mut sd = format!(
        "[freally@32473 jobId=\"{}\" user=\"{}\" severity=\"{}\" prevHash=\"{}\"",
...
fn format_cef(event: &AuditEvent, prev_hash_hex: &str) -> Result<String> {
    ...
    Ok(format!(
        "CEF:0|Freally|Freally|{version}|{sig}|{name}|{sev}|{ext}\n",
...
fn format_leef(event: &AuditEvent, prev_hash_hex: &str) -> Result<String> {
    ...
    // CEF.
    Ok(format!(
        "LEEF:2.0|Freally|Freally|{version}|{eid}|{ext}\n",
...
mod tests {
    ...
    fn syslog_line_starts_with_priority() {
        ...
        assert!(line.contains("[freally@32473"));
    ...
    fn cef_line_has_eight_pipes() {
        ...
        assert_eq!(pipes, 7);
        assert!(line.starts_with("CEF:0|Freally|Freally|"));
    ...
    fn leef_line_uses_tab_extension() {
        ...
        assert!(line.starts_with("LEEF:2.0|Freally|Freally|"));
        assert!(line.contains("\tjobId=job-7"));

...
//!   `freally_core::CopyEvent` + queue state changes into one
...
//!
//! Phase 34 does *not* ship the "verify log" CLI (`freally audit
...
// used elsewhere in the workspace (see `freally-platform`).
#[allow(unsafe_code)]
pub mod worm;
...
pub enum AuditFormat {
    ...
    /// (`[freally@32473 jobId="..." user="..."]`). Pipe to rsyslog
    /// via TCP/UDP/UDS in the consumer.
    Syslog,
    /// ArcSight Common Event Format 0:
    /// `CEF:0|Freally|Freally|<ver>|<sig>|<name>|<sev>|<extension>`.
    Cef,
    /// IBM QRadar Log Event Extended Format 2.0:
    /// `LEEF:2.0|Freally|Freally|<ver>|<eventID>|<tab-extension>`.
    Leef,
}
...
impl AuditSink {
    ...
    /// `target = "freally::audit"` event to this sink. Returns the
...
mod tests {
    ...
    fn rotated_path_appends_one() {
        let path = Path::new("/var/log/freally.log");
        assert_eq!(rotated_path(path), PathBuf::from("/var/log/freally.log.1"));
    }

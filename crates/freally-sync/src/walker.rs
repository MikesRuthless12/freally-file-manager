...
//!
//! The `.freally-sync.db` file itself is skipped — it lives inside
...
/// which would let an attacker who can write the sync `.freally-sync.db`
...
mod tests {
    ...
    fn scan_skips_pair_db_file() {
        let d = tempdir().unwrap();
        std::fs::write(d.path().join(".freally-sync.db"), b"db").unwrap();
        std::fs::write(d.path().join("real.txt"), b"r").unwrap();
        let map = scan_side(d.path(), &[".freally-sync.db"]).unwrap();
        assert!(!map.contains_key(".freally-sync.db"));

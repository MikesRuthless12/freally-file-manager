...
mod winfsp_body {
    ...
    pub(super) struct FreallyWinFspFs {
    ...
    impl FileSystemInterface for FreallyWinFspFs {
        ...
        fn get_volume_info(&self) -> Result<VolumeInfo, NTSTATUS> {
            let label = U16String::from_str("Freally");
        ...
        fn read(
            ...
            if let crate::NodeKind::SnapshotFile { manifest } = &entry.kind {
                ...
                let bytes =
                    freally_chunk::materialise_range(chunk_store, manifest, offset, buffer.len())
    ...
    pub(super) fn mount_with_winfsp(
        ...
        let fs = FreallyWinFspFs::new(map, archive.history, archive.chunk_store)?;

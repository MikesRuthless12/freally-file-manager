...
//!
//! Given a [`freally_history::History`] handle + a
//! [`freally_chunk::ChunkStore`] handle, [`MountTree::build`] emits
...
impl MountNode {
    ...
    fn snapshot_file(name: impl Into<String>, manifest: freally_chunk::Manifest) -> Self {
        Self {
            name: name.into(),
...
pub enum NodeKind {
    ...
    SnapshotFile {
        ...
        manifest: freally_chunk::Manifest,
    },
}
...
impl MountTree {
    ...
    pub fn build_from_snapshot(snap: &freally_chunk::Snapshot) -> Self {
        let mut root = MountNode::directory("");
        for f in &snap.files {
...
mod tests {
    ...
    fn build_from_snapshot_builds_file_tree() {
        use freally_chunk::{FileEntry, Manifest, Snapshot, SnapshotKind};

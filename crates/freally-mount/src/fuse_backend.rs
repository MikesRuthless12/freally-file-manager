...
mod fuse_body {
    ...
    pub(super) fn mount_with_fuser(
        ...
        let options = vec![
            MountOption::FSName("freally".to_owned()),
            MountOption::RO,
            MountOption::NoAtime,
        ];
    ...
    /// ENODATA.
    fn chunk_placeholder() -> freally_chunk::ChunkStore {
        ...
        // one.
        let dir = std::env::temp_dir().join(format!("freally-mount-empty-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        freally_chunk::ChunkStore::open(&dir).expect("empty placeholder chunk store open")
    }
    ...
    fn block_on_history_fetch(
        history: &Arc<freally_history::History>,
    ) -> Option<Vec<freally_history::JobSummary>> {
        use tokio::runtime::Builder;
        let rt = Builder::new_current_thread().enable_all().build().ok()?;
        let history = history.clone();
        rt.block_on(async move {
            let filter = freally_history::HistoryFilter {
    ...
    struct TreeFilesystem {
        ...
        mount_mtime: i64,
        history: Option<Arc<freally_history::History>>,
        chunk_store: Option<Arc<freally_chunk::ChunkStore>>,
    }
    ...
    impl Filesystem for TreeFilesystem {
        ...
        fn read(
            ...
            if let crate::NodeKind::SnapshotFile { manifest } = &entry.kind {
                ...
                match freally_chunk::materialise_range(chunk_store, manifest, off, size as usize) {
    ...
    fn assemble_job_bytes(
        history: &Arc<freally_history::History>,
        chunk_store: &Arc<freally_chunk::ChunkStore>,
        job_row_id: i64,
        offset: i64,
        size: u32,
        ...
        let items = rt
            .block_on(async move {
                history
                    .items_for(freally_history::JobRowId(job_row_id))
                    .await
            })

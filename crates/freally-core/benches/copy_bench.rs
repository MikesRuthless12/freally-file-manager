//! Phase 13 — Criterion benchmarks for the `freally-core` async engine.
//!
...
//!   to 100 MiB when `FREALLY_BENCH_CI=1` to keep the CI suite
...
//! copies can point `FREALLY_BENCH_DST=/path/on/other/volume`.
...
/// Returns `true` when the `FREALLY_BENCH_CI=1` env var is set.
...
fn dst_override_or(path: PathBuf) -> PathBuf {
    env::var("FREALLY_BENCH_DST")
        .map(|v| PathBuf::from(v).join("freally-bench"))
        .unwrap_or(path)
}

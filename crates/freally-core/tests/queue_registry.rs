...
//! from `freally-platform`'s real `volume_id` helper. A `FakeProbe`
...
use freally_core::{JobKind, JobState, QueueId, QueueMergeError, QueueRegistry, VolumeProbe};
...
fn prune_empty_drops_only_jobless_queues_and_emits_events() {
    use freally_core::QueueRegistryEvent;
    let reg = QueueRegistry::new().with_probe(Arc::new(FakeProbe));

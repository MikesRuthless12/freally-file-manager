//! `freally-core` — async byte-exact copy engine + in-memory job queue.
//!
...
//!   `Hasher` abstraction lives in `freally_core::verify`;
//!   `freally-hash` plugs in the concrete algorithms.
//!
//! Added in Phase 23:
//! - `freally_core::sparse` — `ByteRange`, `SparseOps` trait,
...
//!   `FSCTL_SET_SPARSE`) live in `freally_platform::sparse` so
...
//!
//! Added in Phase 24:
//! - `freally_core::meta` — `MetaSnapshot`, `NtfsStream`, `XattrEntry`,
...
//!   resource forks) live in `freally_platform::meta` so this crate
...
//!
//! ```no_run
//! use freally_core::{copy_file, CopyControl, CopyEvent, CopyOptions};
//! use std::path::Path;
//! use tokio::sync::mpsc;
//!
//! # async fn demo() -> Result<(), freally_core::CopyError> {
...
pub mod scan;
pub mod sparse;
/// FFM-M23 — detect a source file that changed underneath the copy.
pub mod stability;
pub mod translate;
mod tree;
pub mod verify;
...
pub use queue::{
    AffinityGroup, Job, JobId, JobKind, JobState, Queue, QueueEvent, QueueId, QueueMergeError,
    QueueRegistry, QueueRegistryEvent, VolumeProbe,
};
pub use safety::{

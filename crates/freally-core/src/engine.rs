...
pub async fn copy_file(
    ...
    // `freally_core::safety` for the threat model.
        ...
        && let Some(sparse_ops) = opts.sparse_ops.clone()
    {
        ...
        match extents_result {
            ...
            Err(detect_err) => {
                ...
                eprintln!(
                    "freally-core: sparse extent detection failed for {}: {detect_err}; \
...
fn hex_encode(bytes: &[u8]) -> String {
    // Minimal lowercase hex encoder — keeps `freally-core` free of a
...
fn detect_dst_fs_label(_dst: &Path) -> &'static str {
    // freally-core deliberately avoids the OS FFI; the platform
...
async fn copy_file_to_cloud_sink(
    ...
    // `FreallyCloudSink` overrides this to open an
...
fn libc_ebusy() -> i32 {
    ...
    // `libc` crate here — freally-core is unsafe-code-free — so

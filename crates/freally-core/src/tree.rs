                        ...
                        Decision::Write(dst_final) => match kind {
                            ...
                            ScanEntryKind::File | ScanEntryKind::Other => {
                                attempt_copy_with_policy(
                                    ...
                                    on_error_task,
                                    TreeTallies {
                                        errored: &errored_task,
                                        source_changed: &source_changed_task,
                                    },
                                )
                                .await
                            }
...
pub async fn move_tree(
    ...
    // FFM-M23 — at least one file was rewritten underneath its copy,
    // so at least one destination file is internally inconsistent. The
    // deletion walker below is path-driven and has no way to know
    // which ones, so keep the whole source tree and degrade the move
    // to a copy. Losing the "move" semantics is recoverable; unlinking
    // the only coherent copy of a torn file is not.
    if report.source_changed > 0 {
        return Ok(report);
    }
...
/// The per-tree counters [`attempt_copy_with_policy`] reports into.
///
/// Grouped because they are always passed together and always come from
/// the same task closure. Threading them separately pushed the argument
/// list to eight, which is past the point clippy stops believing a
/// caller can keep them in the right order — and the two are easy to
/// transpose, since both are `&Arc<AtomicU64>`.
struct TreeTallies<'a> {
    /// Files whose copy failed and whose error the policy absorbed.
    errored: &'a Arc<AtomicU64>,
    /// Files the FFM-M23 guard saw change underneath the copy.
    source_changed: &'a Arc<AtomicU64>,
}
    ...
    loop {
        ...
        match result {
            Ok(report) => {
                ...
                if report.source_changed.is_some() {
                    tallies.source_changed.fetch_add(1, Ordering::Relaxed);
                }
            ...
            Err(err) => {
                match policy {
                    ...
                    ErrorPolicy::Skip => {
                        record_file_error(err, events, tallies.errored).await;

...
//!
//! `freally-provenance` (sibling crate) owns the manifest format,
...
///
/// Implementations live in `freally-provenance`. The default
...
///
/// Implemented by `freally_provenance::FreallyProvenanceSink`. Kept
/// in `freally-core` so [`crate::CopyOptions`] can hold a trait
/// object without pulling `freally-provenance` (and its `bao` /

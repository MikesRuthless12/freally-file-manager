...
/// the same enum in `freally_journal::types`; kept in core so the
...
///
/// Implemented by `freally_journal::FreallyJournalSink`. The
...
///
/// Implemented by `freally_snapshot::FreallySnapshotHook`. Kept in
/// this crate so [`CopyOptions`] can hold a trait object without a
/// dependency cycle between `freally-core` and `freally-snapshot`.
///
...
///
/// Implemented by `freally_shape::FreallyShapeSink`. Kept in this
/// crate so [`CopyOptions`] can hold a trait object without a
/// dependency cycle between `freally-core` and `freally-shape`.
///
...
///
/// Implemented by `freally_crypt::FreallyCryptHook`. Kept in this
/// crate so [`CopyOptions`] can hold a trait object without pulling
/// `freally-crypt` into the core dep graph (same pattern as
...
pub struct CopyOptions {
    ...
    /// `CopyErrorKind::VerifyFailed`. `freally-hash` provides the
    ...
    /// `freally-platform` to keep this crate `#![forbid(unsafe_code)]`-clean.
    ///
    ...
    /// `freally-snapshot::FreallySnapshotHook` implements this in
    ...
    ///
    /// Implemented by `freally_shape::FreallyShapeSink`. The sink
    ...
    ///
    /// Implemented by `freally_platform::PlatformSparseOps`.
    ...
    ///
    /// Implemented by `freally_platform::PlatformMetaOps`. `None`
    ...
    /// implementation lives in `freally-chunk`
    /// (`FreallyChunkSink`); kept here as a trait object so the core
    ...
    /// `async-trait` crates pulled in by `freally-cloud`.
    ///
    /// Phase 32b wires the contract + adapter
    /// (`freally_cloud::FreallyCloudSink`); full streaming
    ...
    /// per-job [`freally_provenance::ProvenanceManifest`] state and
...
///
/// Implemented by `freally_cloud::FreallyCloudSink`. Kept in
/// `freally-core` so [`CopyOptions`] can hold a trait object without
...
///
/// Implemented by `freally_chunk::FreallyChunkSink`. Kept in
/// `freally-core` so [`CopyOptions`] can hold a trait object without
...
///
/// Implemented by `freally-platform::PlatformFastCopyHook`. Kept in
/// this crate so [`CopyOptions`] can hold a trait object without a
/// dependency cycle.
///

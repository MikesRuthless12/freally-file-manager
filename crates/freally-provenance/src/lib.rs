...
//!
//! `freally-provenance` ships:
//!
//! - [`ProvenanceManifest`] — the public document a copy job emits
//!   when [`freally_core::CopyOptions::provenance`] is set. Carries
...
//!   [`FreallyProvenanceSink::finalize_to_path`] after the tree-walk
...
//!   signing key from settings UI" lives in `apps/freally-ui/

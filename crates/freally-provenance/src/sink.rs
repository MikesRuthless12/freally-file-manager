//! [`FreallyProvenanceSink`] — the
//! [`freally_core::ProvenanceSink`] implementation the engine talks
//! to.
//!
//! Lifecycle:
...
//!    [`freally_core::ProvenancePolicy`], which lives on
//!    [`freally_core::CopyOptions::provenance`].
//! 3. The engine, per file, calls
//!    [`FreallyProvenanceSink::make_encoder`] +
//!    [`FreallyProvenanceSink::record_file`] to feed records into
//!    the per-job state.
//! 4. After the tree-walk, the caller invokes
//!    [`FreallyProvenanceSink::finalize_to_path`] to build the
...
/// and passed to [`FreallyProvenanceSink::new`].
...
impl SinkConfig {
    ...
    pub fn new(src_root: PathBuf, dst_root: PathBuf) -> Self {
        Self {
            ...
            host: probe_host(),
            user: probe_user(),
            freally_version: env!("CARGO_PKG_VERSION").to_string(),
        }
    }
}
...
pub struct FreallyProvenanceSink {
    config: SinkConfig,
...
impl FreallyProvenanceSink {
    ...
    pub fn finalize_to_path(
        ...
        let mut manifest = ProvenanceManifest::new(
            ...
            self.config.freally_version.clone(),
            files,
        );
...
impl std::fmt::Debug for FreallyProvenanceSink {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("FreallyProvenanceSink")
...
impl ProvenanceSink for FreallyProvenanceSink {
    ...
    fn record_file(
        ...
        if validate_rel_path(&rel_path).is_err() {
            eprintln!(
                "freally-provenance: rejected rel_path {rel_path:?} (traversal check) for src {}",
                src.display()
            );
            return;
        }
...
mod tests {
    ...
    fn fixture_sink() -> (Arc<FreallyProvenanceSink>, TempDir) {
        ...
        let sink = FreallyProvenanceSink::new(config);
        (sink, tmp)
    }

...
mod zfs_version_warning {
    ...
    //! `FREALLY_SUPPRESS_ZFS_WARNING=1` for scripted environments
    ...
    pub fn check_once() {
        ...
        CHECKED.get_or_init(|| {
            ...
            if std::env::var("FREALLY_SUPPRESS_ZFS_WARNING")
                .ok()
                .as_deref()
                == Some("1")
            {
            ...
            if let Some((maj, min, patch)) = read_zfs_version() {
                ...
                if maj == 2 && min == 2 && patch <= 6 && bclone {
                    eprintln!(
                        "freally-platform: WARNING — OpenZFS {}.{}.{} with \
                         ...
                         FREALLY_SUPPRESS_ZFS_WARNING=1 to silence.",
                        maj, min, patch
                    );
                }
            }

...
pub fn render_launch_agent(program: &Path) -> String {
    ...
    let escaped = crate::scheduler::xml_escape_pub(&program.to_string_lossy());
    format!(
...
pub fn render_run_key_value(program: &Path) -> String {
    ...
    format!(
        "{} {START_MINIMIZED_FLAG}",
        crate::scheduler::win_quote_pub(&program.to_string_lossy())
    )
}
...
mod windows_impl {
    ...
    fn reg(args: &[&str]) -> Result<std::process::Output, AutostartError> {
        ...
        Ok(
            std::process::Command::new(crate::scheduler::os_tool_pub("reg"))
                .args(args)
                .output()?,
        )
    }

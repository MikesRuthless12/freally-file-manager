//! `freally-platform` — OS-specific fast paths for byte-exact file copy.
//!
...
//!   finally the Phase 1 [`freally_core::copy_file`] async loop.
//! - [`PlatformFastCopyHook`] — implementation of
//!   [`freally_core::FastCopyHook`] that adapts [`fast_copy`] into the
...
//! ```no_run
//! use std::sync::Arc;
//! use freally_core::{CopyOptions, copy_file, CopyControl, CopyEvent};

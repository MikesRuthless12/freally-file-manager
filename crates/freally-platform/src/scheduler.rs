...
///
/// `pub(crate)` so `autostart` shares it rather than keeping its own
/// copy, which had already drifted.
...
///
/// `pub(crate)` so `autostart`'s Run-key quoting shares it. Its own copy
/// had drifted — it doubled quotes but not trailing backslashes, so a
/// path ending in `\` escaped the closing quote.
...
///
/// **Atomic or nothing.** Every backend writes an artifact and then asks
/// the OS to register it, and a failure between those two steps leaves a
/// live orphan: a plist launchd loads at next login, or a `.service` /
/// `.timer` pair that `linux_impl::is_installed` (a bare file-existence
/// probe) then reports as installed. Because `schedule_save_impl` rolls
/// its settings row back when this returns `Err`, such an orphan would
/// be firing with no settings row behind it and no UI able to remove it.
///
/// So a failed install is undone here, once, for every backend rather
/// than per-backend — `remove` is documented to succeed on an id that is
/// not installed, so the compensating call is safe even when the backend
/// failed before writing anything.
///
/// The compensation is skipped when `id` was **already installed**
/// before we started. A re-install over an existing schedule is
/// replace-in-place on Windows (`schtasks /Create /F`), so a failed
/// re-install leaves the previous generation intact and running —
/// deleting it here would turn a rejected edit into a silently
/// cancelled nightly job. Only a *new* install can leave an orphan
/// nothing knows about, and that is the case worth undoing.
pub fn install(job: &ScheduledJob) -> Result<(), SchedulerError> {
    validate_job(job)?;
    // If this probe fails we cannot tell, so assume it existed and keep
    // the conservative option: never delete something we did not create.
    let existed = is_installed(&job.id).unwrap_or(true);
    let outcome = install_backend(job);
...
fn install_backend(job: &ScheduledJob) -> Result<(), SchedulerError> {
...
/// hostile `PATH`.
///
/// `pub(crate)` so sibling modules (`autostart`) get the same hardening
/// without duplicating the lookup.
#[cfg(any(windows, unix))]
pub(crate) fn os_tool(name: &str) -> std::path::PathBuf {
    #[cfg(windows)]
    {
...
mod macos_impl {
    ...
    pub fn install(job: &ScheduledJob) -> Result<(), SchedulerError> {
        ...
        // A failure here leaves a plist launchd would load at the next
        // login — a live orphan with no settings row behind it. It is
        // not exotic either: a stale label, a non-Aqua session or a
        // SIP/TCC denial all land on this path. The rollback lives in
        // `scheduler::install`, which undoes a failed install for every
        // backend rather than each one hand-rolling its own.

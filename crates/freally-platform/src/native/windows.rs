...
/// via `FREALLY_NO_BUFFERING_THRESHOLD_MB=<N>` (set to a very
...
/// lined Freally up to "lose" the head-to-head bench on the 10 GiB
...
/// disk. Freally with `NO_BUFFERING` engaged was correctly waiting
...
/// 24 GiB. The `FREALLY_NO_BUFFERING_THRESHOLD_MB` env var still
...
fn no_buffering_threshold(_topo: &crate::topology::VolumeTopology) -> u64 {
    ...
    let override_val = ENV_OVERRIDE.get_or_init(|| {
        std::env::var("FREALLY_NO_BUFFERING_THRESHOLD_MB")
            .ok()
...
/// `FREALLY_SKIP_ZERO_FILL=1` manually. Cached per-process —
...
pub(crate) async fn try_native_copy(
    ...
    // topology-gated `auto_parallel_chunks` + the `FREALLY_PARALLEL_CHUNKS`
    // env escape hatch.
    strategy: freally_core::CopyStrategy,
) -> NativeOutcome {
    ...
    let force_parallel = matches!(strategy, freally_core::CopyStrategy::ParallelChunks);

    // Phase 47 — opt-in IoRing fast path (Win 11 22000+).
    // Gate behind `FREALLY_IORING_IO=1`. When the env var is set
    ...
    // `FREALLY_OVERLAPPED_IO=1` so users can A/B test against
    ...
    //
    // Opt out via `FREALLY_DISABLE_AUTO_OVERLAPPED=1` if the new
    ...
    let auto_overlapped_disabled = std::env::var("FREALLY_DISABLE_AUTO_OVERLAPPED")
        .ok()
        .as_deref()
    ...
    //     `FREALLY_PARALLEL_CHUNKS=<N>` count, else `DEFAULT_NUM_CHUNKS`).
    //   * any other strategy (incl. `Auto`) — the `FREALLY_PARALLEL_CHUNKS`
    ...
    let parallel_chunks = match strategy {
        freally_core::CopyStrategy::ParallelChunks => super::parallel::requested_chunks(total)
            .or_else(|| {
    ...
    // `freally-platform: WARNING — …` style elsewhere in this crate;
    // there's no tracing subscriber wired at this layer.)
    if src_attrs.is_recall_on_data_access {
        eprintln!(
            "freally-platform: NOTE — source {:?} is a OneDrive / Cloud Files \
    ...
    if src_attrs.is_encrypted {
        eprintln!(
            "freally-platform: NOTE — source {:?} is EFS-encrypted; \
    ...
    if src_attrs.is_compressed {
        eprintln!(
            "freally-platform: NOTE — source {:?} is NTFS-compressed; the \

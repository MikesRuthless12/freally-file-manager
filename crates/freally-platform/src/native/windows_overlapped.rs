...
//!
//! Opt-in via `FREALLY_OVERLAPPED_IO=1`. Gated behind an env var
...
/// `FREALLY_OVERLAPPED_BUFFER_KB`. Must be a multiple of the
...
/// `FREALLY_OVERLAPPED_SLOTS`. Robocopy's internal default is ~8.
...
fn buffer_bytes() -> usize {
    std::env::var("FREALLY_OVERLAPPED_BUFFER_KB")
        .ok()
...
fn n_slots() -> usize {
    std::env::var("FREALLY_OVERLAPPED_SLOTS")
        .ok()
        .and_then(|s| s.parse::<usize>().ok())
...
pub(crate) fn requested(total: u64) -> Option<()> {
    ...
    match std::env::var("FREALLY_OVERLAPPED_IO").ok().as_deref() {
...
pub(crate) async fn try_overlapped_copy_with_config(
    ...
    if let Some(n) = slots_override {
        ...
        unsafe { std::env::set_var("FREALLY_OVERLAPPED_SLOTS", n.to_string()) };
    }
    if let Some(kb) = buffer_kb_override {
        unsafe { std::env::set_var("FREALLY_OVERLAPPED_BUFFER_KB", kb.to_string()) };
    }
    if let Some(nb) = no_buffering_override {
        unsafe {
            std::env::set_var(
                "FREALLY_OVERLAPPED_NO_BUFFERING",
                if nb { "1" } else { "0" },
            )
        };
    }
...
unsafe fn do_overlapped_copy(
    ...
    //   - `FREALLY_SKIP_ZERO_FILL=0` → force lazy-zero pass.
    //   - `FREALLY_SKIP_ZERO_FILL=1` → try regardless (no-op if
...
mod tests {
    ...
    fn requested_gates_default_off() {
        ...
        unsafe {
            std::env::remove_var("FREALLY_OVERLAPPED_IO");
        }

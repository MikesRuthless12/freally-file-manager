...
//! This path is opt-in via `FREALLY_PARALLEL_CHUNKS=<N>`.
//!
...
///
/// Respects the `FREALLY_PARALLEL_BUDGET_BYTES` env override so
/// a bench run can force a specific total (e.g. 4 MiB) without
/// re-compiling.
fn chunk_buf_size(total_file_bytes: u64, num_chunks: usize) -> usize {
    let explicit_budget = std::env::var("FREALLY_PARALLEL_BUDGET_BYTES")
        .ok()
        .and_then(|s| s.parse::<usize>().ok());
    let budget = explicit_budget.unwrap_or_else(|| {
        freally_core::CopyOptions::default().buffer_size_for_file(total_file_bytes)
    });
...
/// via `FREALLY_PARALLEL_CHUNKS=<N>` (clamped 2..=16).
pub(crate) fn requested_chunks(total: u64) -> Option<usize> {
    if total < MIN_FILE_FOR_PARALLEL {
        return None;
    }
    let raw = std::env::var("FREALLY_PARALLEL_CHUNKS").ok()?;
...
pub(crate) async fn parallel_chunk_copy(
    ...
    // `FREALLY_SKIP_ZERO_FILL=1` and the calling process holds

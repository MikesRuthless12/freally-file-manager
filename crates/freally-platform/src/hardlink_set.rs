...
impl HardlinkSet {
    ...
    pub fn get(&self, ident: &LinkIdentity) -> Option<PathBuf> {
        ...
        let guard = self.inner.lock().unwrap_or_else(|e| {
            eprintln!("freally-platform::hardlink_set: recovering from poisoned ledger (get)");
            e.into_inner()
        });
    ...
    pub fn record(&self, ident: LinkIdentity, dst: PathBuf) {
        let mut g = self.inner.lock().unwrap_or_else(|e| {
            eprintln!("freally-platform::hardlink_set: recovering from poisoned ledger (record)");
            e.into_inner()
        });
        g.insert(ident, dst);
    }

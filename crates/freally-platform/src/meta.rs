//! Phase 24 — `freally_core::MetaOps` bridge with real OS probes.
//!
//! Per-OS coverage:
//!
...
//!   structured [`freally_core::meta::PosixAclBlob`] so the UI can render them; the
...
//! [`freally_core::MetaPolicy::appledouble_fallback`] is `true`.
...
use freally_core::meta::NtfsStream;
#[cfg(any(target_os = "linux", target_os = "macos"))]
use freally_core::meta::XattrEntry;
#[cfg(target_os = "linux")]
use freally_core::meta::{FileCaps, PosixAclBlob, SeLinuxContext};
#[cfg(target_os = "macos")]
use freally_core::meta::{FinderInfoBlob, ResourceForkBlob};
use freally_core::meta::{MetaApplyOutcome, MetaOps, MetaPolicy, MetaSnapshot};

/// Platform-backed security-metadata capture/apply hook.
///
/// Drop one into [`CopyOptions::meta_ops`](freally_core::CopyOptions::meta_ops)

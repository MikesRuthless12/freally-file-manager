//! Phase 23 — `freally_core::SparseOps` bridge with real OS probes.
//!
...
use freally_core::sparse::{ByteRange, SparseOps};
...
///
/// Drop one into [`CopyOptions::sparse_ops`](freally_core::CopyOptions::sparse_ops)
...
fn unix_detect_extents(path: &Path) -> io::Result<Vec<ByteRange>> {
    ...
    while cursor < total {
        ...
        if hole <= data {
            ...
            eprintln!(
                "freally-platform: SEEK_HOLE <= SEEK_DATA on {} — extent detection unreliable \
    ...
    loop {
        ...
        if code == 234 {
            ...
            if new_len > MAX_RANGE_ENTRIES {
                eprintln!(
                    "freally-platform: sparse extent count exceeded {MAX_RANGE_ENTRIES} on \

//! `freally-settings` — persistent user preferences + named profiles.
//!
//! Settings live in `settings.toml` at the OS config dir
//! (`%APPDATA%\freally-file-manager` / `~/Library/Application Support/freally-file-manager` /
//! `$XDG_CONFIG_HOME/freally`) and are round-trippable via `serde`.
...
//!
//! The crate intentionally does NOT depend on `freally-core` or
//! `freally-hash` — it's a pure preference + IO layer. The Tauri
...
pub struct Settings {
    ...
    /// server backed by `freally-recovery`). Off by default;
...
impl Settings {
    ...
    pub fn carry_backend_owned_from(&mut self, prev: &Settings) {
        ...
        self.sync.default_mode = prev.sync.default_mode;
        self.sync.conflict_suffix_format = prev.sync.conflict_suffix_format;
        self.sync
            .host_label_override
...
pub struct TransferSettings {
    ...
    /// `freally-core`'s `[MIN_BUFFER_SIZE, MAX_BUFFER_SIZE]` window
    ...
    ///
    /// Mirrors `freally_core::LockedFilePolicy`. Kept as a separate
    /// enum here so this crate stays free of a `freally-core`
    ...
    /// mirrors `freally_platform::DedupMode`:
    ...
    /// `freally_platform::HardlinkPolicy`:
    ...
    /// `freally_core::CopyOptions` (Phase 42 engine knob); the
    ...
    /// `freally_core::CopyOptions`. Default `3` matches the engine's
    ...
    /// `freally_core::CopyOptions`. Default `50` ms matches the
...
/// Phase 19b — mirror of `freally_core::LockedFilePolicy`.
///
...
impl TransferSettings {
    ...
    /// Mirrors `freally_core::options::{MIN_BUFFER_SIZE, MAX_BUFFER_SIZE}`.
...
/// Verify algorithm selection. Mirrors `freally_hash::HashAlgorithm`
/// plus an explicit `Off` variant. We keep a separate enum here so
/// this crate doesn't depend on `freally-hash`.
///
...
/// control to `freally_platform::recommend_concurrency`; `Manual`
...
/// `freally_core::CopyStrategy`: `Prefer → Auto`, `Avoid → NoReflink`,
...
/// Mirrors `freally_secure_delete::ShredMethod`. Kept separate so
...
pub struct AdvancedSettings {
    ...
    /// `freally_history::History::default_path()` (OS data dir).
...
/// Mirrors `freally_core::ErrorPolicy`. Flattened for TOML round-
...
/// TOML-friendly mirror of `freally_core::FilterSet`.
///
/// Kept in this crate so `Settings` can round-trip cleanly without
/// pulling in a `freally-core` dependency; the Tauri bridge is
...
/// boundary so this crate stays free of a `freally-sync` dependency;
/// the Tauri bridge translates the DTO at enqueue time.
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
#[serde(default, rename_all = "kebab-case")]
pub struct SyncSettings {
    /// All configured sync pairs. Each pair has its own state DB at
    /// `.freally-sync.db` under its left root by default; the path
...
/// Mirror of `freally_sync::SyncMode`. Kept as a separate enum so
...
///
/// Phase 49h — TOML mirror of `freally_chunk::RepoCompression`. Kebab
...
/// (this crate has no freally-chunk edge). `level` is the zstd level
...
pub struct ChunkStoreSettings {
    ...
    /// `freally_chunk::default_chunk_store_path` at runtime", which
    ...
    /// [`Repository`](freally_chunk::Repository) timeline after every
...
/// TOML/JSON mirror of `freally_core::versioning::RetentionPolicy`
...
///
/// This crate deliberately has no `freally-core` / `freally-chunk`
...
/// `freally_core::translate::TargetOs` but kept local so this crate
/// stays independent of `freally-core`; the Tauri bridge translates
/// at enqueue time.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
...
/// `freally_core::translate::NormalizationMode`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
...
/// `freally_core::translate::LineEndingMode`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
...
/// `freally_core::translate::ReservedNameStrategy`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
...
/// `freally_core::translate::LongPathStrategy`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
...
/// `freally_core::translate::PathPolicy` at enqueue time.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(default, rename_all = "kebab-case")]
pub struct PathTranslationSettings {
    ...
    /// `freally_core::translate::default_text_extensions()`.
...
/// Mirror of `freally_core::translate::default_text_extensions()`
/// kept inline so this crate doesn't pull in `freally-core`. Sync
...
/// Cap` shape. Mirrors `freally_power::BatteryPolicy` and
/// `freally_power::NetworkPolicy` — kept local so this crate stays
/// independent of `freally-power`. The Tauri bridge translates at
/// enqueue time.
///
...
/// runner subscribes to `freally_power::PowerBus` and maps each
...
// ---------------------------------------------------------------------
//
// The settings layer owns *persistence*; the `freally-cloud` crate
// owns *operators + credentials*. These types are the on-disk mirror
// of `freally_cloud::Backend`. They stay here (rather than in
// `freally-cloud`) to keep this crate free of the OpenDAL dep — the
// Tauri shell translates `BackendConfigEntry` into
// `freally_cloud::Backend` when it builds operators.
...
/// keychain under `freally-cloud/<name>` via the
/// `freally-cloud::Credentials` API. The entries round-trip through
...
pub struct RemoteSettings {
    ...
    /// `freally_cloud::BackendRegistry::from_snapshot`).
...
/// mirrors `freally_cloud::BackendKind::wire`.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
...
impl BackendKindChoice {
    ...
    /// `freally_cloud::BackendKind::wire`.
    pub fn as_str(&self) -> &'static str {
        match self {
            Self::S3 => "s3",
            Self::R2 => "r2",
...
pub struct FtpBackendConfig {
    ...
    /// `freally_cloud::FtpConfig::ca_bundle_path` doc-comment
...
/// Phase 33 — persisted preferences for the `freally-mount` integration.
...
pub struct MountSettings {
    ...
    /// `apps/freally-ui/src-tauri/src/mount_commands.rs::mount_latest_on_launch`)
...
/// the Tauri runner opens a `freally_audit::AuditSink` at startup
/// (or on the first `update_settings` flip) and records the
/// brief's eight `freally_audit::AuditEvent` variants.
///
/// `format` persists the kebab-case
/// `freally_audit::AuditFormat` identifier (`csv`, `json-lines`,
/// `syslog`, `cef`, `leef`) — the audit crate's enum is mirrored as a
/// string here so [`Settings`] can round-trip without taking a hard
/// dep on `freally-audit` (keeps the settings crate a pure preference
/// layer, consistent with the Phase 9 / 27 pattern).
///
/// `worm` stores the kebab-case `freally_audit::WormMode`
...
pub struct AuditSettings {
    ...
    /// under `<config-dir>/audit/freally-audit.log` at sink open
    /// time.
...
/// `freally_crypt::EncryptionPolicy` / `CompressionPolicy` are
...
/// `freally_mobile::MobileSettings` but with stringly-typed fields
...
pub struct MobileSettings {
    ...
    /// once via `freally_mobile::mint_peer_id` and persisted so
...
/// `freally_mobile::PairingRecord`; the binary X25519 public key is
...
pub struct MobilePairingEntry {
    ...
    /// `freally_mobile::PushTarget`'s tagged-enum wire form.
    #[serde(skip_serializing_if = "Option::is_none")]
...
/// `freally_recovery::serve()` with the persisted token and the
...
/// click) via `freally_recovery::generate_token()`. Keeping the
...
///
/// Mirrors the live `freally_server::ServerConfig` / `WebhookSink` /
/// `OtelConfig` at the settings boundary so this crate stays free of
/// the `freally-server` dep (axum / russh / opentelemetry); the Tauri
...
/// `freally_server::AuthMode` as flat strings. `mode` is
...
/// `freally_server::WebhookSink`. `target` is
...
impl VerifyChoice {
    ...
    /// in `freally-hash`. `Off` is mapped to `None` because the

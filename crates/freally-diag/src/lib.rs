...
//!   `Option<Arc<dyn …Sink>>` slots `freally-core` already exposes for
//!   shape / journal / chunk.
//!
//! ## Deferred (next increment)
//!
//! The per-syscall instrumentation that POPULATES a [`PhaseSample`] (the
//! read / write / cpu / net wait timing) inside `freally-core`'s copy
...
//! scaffold-first pattern Phase 46's `freally-plugin` used.
...
///
/// DEFERRED: `freally-core`'s copy loop does not yet emit samples (the
...
/// — mirroring the `Option<Arc<dyn …Sink>>` hooks `freally-core` already

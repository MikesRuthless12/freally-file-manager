...
//!
//! Orchestrates launching the `copythat-helper` binary ELEVATED
...
//! `copythat-platform` — mirroring how `copythat-snapshot`'s VSS
...
mod tests {
    ...
    fn build_spawn_command_carries_pipe_and_capability() {
        let (program, args) = build_spawn_command(
            "/opt/copythat/copythat-helper",
            "copythat-helper-pipe-0123456789abcdef",
            &[Capability::ElevatedRetry],
        );
        let joined = args.join(" ");
        assert!(!program.is_empty(), "program must be set");
        assert!(
            joined.contains("copythat-helper-pipe-0123456789abcdef"),
        ...
        assert!(
            joined.contains("copythat-helper") || program.contains("copythat-helper"),
